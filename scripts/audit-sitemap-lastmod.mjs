 ...
 *   ① 크롤러가 다시 와 볼지를 정할 근거가 없다 (구글은 lastmod 를 본다. 정직했던 이력까지)
 *   ② `scripts/indexnow-submit.mjs` 가 lastmod 로 최근에 바뀐 것을 고른다 →
...
/* 주소가 하나도 안 잡히면 전부 통과가 아니라 못 쟀다다. 형식이 바뀌었거나 빈 파일이다. */
if (blocks.length === 0) {
...
/* ★ **오지 마라와 여기 있다를 같이 말하지 않는다** (2026-08-16).
   robots.txt 가 막아 둔 자리가 사이트맵에 실려 있으면 크롤러는 그걸 막혔는데 색인됨으로
...
if (disallowedUrls.length > 0) {
  ...
  console.error('  → 오지 마라와 여기 있다를 같이 말하는 것이다. 둘 중 하나를 고쳐라.');
  process.exit(1);
}

/* ★ **`noindex` 인 장도 사이트맵에 싣지 않는다** (2026-08-20).
   바로 위 robots.txt 검사와 같은 말인데 막는 자리만 다르다. 그쪽은 들어오지 마라,
   이쪽은 들어와도 되는데 색인은 하지 마라다. 둘 다 사이트맵에 실릴 이유가 없다.
...
if (noindexUrls.length > 0) {
  ...
  console.error('  → 색인하지 마라와 여기부터 봐 달라를 같이 말하는 것이다.');
