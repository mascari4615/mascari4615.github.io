 ...
 * 그 파일에 이미 한글 식별자가 있어서 **「주변 코드와 같은 관용을 따르라」는 일반 지침을 따른
 * 결과** 위반했다. 사람이 매번 눈으로 잡을 수 없으니 기계가 막는다.
 * 룰 정본 = `memo/rules/code-style.md § 코드 방향성` (식별자는 영문).
 *
 * 왜 「0 이면 통과」가 아니라 래칫인가: 지금 존량이 수천 곳이다. 0 을 요구하면 오늘 당장 모든
 * 세션의 push 가 막힌다. 그래서 **늘면 빨강, 줄면 초록**으로 둔다. 존량을 비워 0 이 되면
 * 베이스라인도 0 이 되고, 그때부터는 그냥 「하나라도 있으면 빨강」이다.
 *
...
/* 볼 파일이 하나도 없으면 「전부 통과」가 아니라 「못 쟀다」다 — 걸러내기가 과했다는 뜻이다. */
if (files.length === 0) {
    ...
    `${JSON.stringify(
      {
        ...
        count,
        files: byFile.size,
        /* 파일별로도 적어 둔다 — 총계만 있으면 「늘었다」고만 알려 줄 뿐 **어디가** 늘었는지를 못 짚는다.
