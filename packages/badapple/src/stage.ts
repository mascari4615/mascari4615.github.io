 ...
 *
 * 여기가 도구가 활발하게 바뀐다를 견디는 자리다. 무대는 표면 **목록을 들고 있지 않다** . 
 * 매 프레임 붙어 있는 것들에게 지금 너 어디에 몇 칸이냐를 물어서, 그 답으로 배치를 새로 짠다.
...
function makePaint(frame: Frame, region: Rect, cols: number, rows: number): Paint {
	...
	// 하나라도 켜져 있으면 켠다로 본다. 실루엣은 가늘어지는 것보다 굵어지는 게 안전하다.
...
export class Stage {
	...
	present(frame: Frame): void {
		...
		// 모자이크: 지금 화면에 있는 것들을 전부 감싸는 사각형이 곧 이번 프레임의 액정 크기다.
