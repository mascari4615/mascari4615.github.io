...
/**
 * 최신 그림을 받아 두고, 지금 줄의 높이를 하나씩 내주는 표면.
 *
 ...
 * 바탕 위에 남은 자리를 구한다.
 *
 * 여기가 조용한 기계에서만 된다를 없애는 자리다. 바탕이 30% 면 30~천장 사이에 그리면 되고,
 * 바탕이 천장에 가까우면 **그릴 자리가 없다고 말해야 한다**. 억지로 그리면 전부 100% 에 붙어
 * 나와서, 코드가 멀쩡한데도 안 따라간다로 보인다 (실제로 그렇게 헤맸다).
 *
 ...
 *
 * 바닥과 천장을 두는 이유: 0 은 그래프에서 아무 일 없음과 구별이 안 되고, 1 은 기계를
...
export class LoadDriver {
	...
	async run(columns?: number): Promise<number> {
		...
		// 그래서 한 줄도 안 태우고 0 을 낸다. 부르는 쪽이 지금은 못 그린다를 알 수 있게.
		if (!band) return 0;
