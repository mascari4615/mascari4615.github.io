 ...
 *
 * 왼쪽 = 빌려온 골격의 뼈, 오른쪽 = 이 몸의 뼈. 이름이 같으니 그냥 먹이면 된다가
...
function bakeClip(clip, pairs, sourceRoot, mixer, fps = 30) {
  ...
  // 붙었다와 붙었는데 안 움직인다를 화면만 보고는 구분 못 한다.
...
export async function mountModel(canvas, modelName, onFail) {
  ...
  try {
    ...
    // 제 모습을 찍어 낼 수 있어야 자세가 이상하다를 사람 눈 없이도 확인한다.
  ...
  // 바닥이 없어 그림자가 맺힐 데도 없다. 그래서 바닥인 척하는 얼룩이 맞다.
   ...
   * 얘가 화면에서 실제로 차지하는 네모.
   *
   * 그림판은 창만큼 넓지만 얘는 그 안 일부만 채운다. 그림판 전체를 누를 자리로
