...
export function factClaims(said: string): { value: string; what: string }[] {
  ...
  for (const { pattern, what } of factShape) {
...
/** 값에서 숫자만 뽑는다. 10시 28분과 10시 28분입니다가 같은 근거를 갖게. */
 ...
 * 조수님이 3시에 회의야라고 말해 준 걸 얘가 되뇌는 건 지어낸 게 아니다.
 */
function hasEvidence(value: string, found: readonly string[], recent: readonly MemoryEntry[]): boolean {
  ...
  // 새어 나간 11시 27분이 다음 turn 의 근거가 되어 영영 통과했다). 근거가 될 수 있는
  // 것은 **찾아본 것**과 **조수님이 한 말**뿐이다.
  const base = [...found, ...recent.filter((e) => e.role === 'sensed').map((e) => e.text)]
    .join('\n')
    .replace(/0+(\d)/g, '$1');
  if (number.length === 0) return base.includes(value);

  // **숫자는 붙어 있어야 근거다.**
  //
  // 처음엔 숫자를 따로따로 찾았는데, 11 같은 흔한 값은 아무 데나 있어서 11시 27분이
  // 그대로 통과했다(실측 53회차). 11:27 11시 27분처럼 **가까이 붙어 있을 때만**
  // 그 시각을 실제로 본 것이다. 앞에 붙은 0 은 같은 값으로 본다(09:05 = 9시 5분).
  return new RegExp(number.join('[^0-9]{0,4}')).test(base);
}
 ...
 * 다시 시킬 때 두뇌에 넘길 말.
 *
 * **모른다고 해도 된다**고 분명히 한다. 그게 이 문제의 뿌리다. 모른다가 손해라고 여기면
 * 계속 찍는다.
 */
