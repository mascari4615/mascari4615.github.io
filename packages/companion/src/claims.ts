 ...
 * 찾아보기, 열기, 알려 주기), 두뇌가 **손을 안 쓰고도 해 뒀어라고 말할 수 있다.**
 *
 * 그게 제일 나쁜 고장이다. 틀린 답은 고쳐 주면 되지만(33회차), **안 한 일을 했다고 하면**
 * 조수님은 그걸 믿고 지나간다. 나중에 메모가 없는 걸 알게 되는 순간 곁에 있는 것이 아니라
 * 못 믿을 것이 된다.
 *
 * 기계로 정확히 잴 수 있다. 손을 쓰면 말 안에 표가 남고 core 가 그걸 안다. **말은 해 뒀어인데
 * 쓴 손이 없으면** 그건 거짓이다.
 *
 * 조심할 것 하나: **하겠다는 말과 했다는 말은 다르다.** 적어 둘게는 앞으로의 얘기고
 * 적어 뒀어가 주장이다. 이걸 안 가르면 얘가 아무 약속도 못 하게 된다.
 */
 ...
 * **안 했다는 말도 주장이 아니다.**
 *
 * 라이브에서 바로 걸렸다(41회차): 아무것도 안 적어뒀어...가 적어 뒀에 걸려 **거짓말로
 * 잡혔다.** 안 했다고 솔직히 말하는 것을 거짓말이라고 막으면, 그건 정직을 벌하는 것이다.
 */
// 못 열어 뒀어처럼 사이에 띄어쓰기가 들어간다. 글자만 세면 그걸 놓친다.
//
// 어미를 손으로 적다가 **았을 빠뜨렸다.** 그래서 못 찾았어. 이 관문이 막으려던
...
/**
 * 이 말이 했다고 주장하나. 아니면 null.
 *
 * 적어 둘게처럼 앞으로의 말은 안 센다. 이걸 안 가르면 얘가 아무 약속도 못 한다.
 */
 ...
 * 해라가 맞다. 못 하는 걸 못 한다고 하는 건 흠이 아니다.
 */
 ...
 * **누를게라고 말만 하는 것.**
 *
 * 122회차 라이브에서 화면에서 아무거나 하나 눌러봐에 이렇게 답했다 . 
 * 아, 이거 누를게. 응, 최근 활동 정리 탭 누르고 올게.... **그리고 아무것도 안 눌렀다.**
 * 손 표시(`[[누르기: 3]]`)를 안 적었기 때문이다. 42회차에 같은 것을 겪었다(0/10).
 *
 * 위의 `unbackedClaim` 은 했다는 거짓말을 잡는다. 이건 반대다. **할게라고 하고 안
 * 하는 것**. 41회차에 약속을 막으면 안 된다고 정했으므로 넓게 잡으면 안 된다.
