...
export interface ScreenSenseOptions {
  ...
  okToLook?: () => boolean;
  /**
   * 지금 화면이라고 부를 수 있는 나이. 이보다 묵었으면 물어볼 때 다시 찍는다.
   *
   * 화면 보기 간격(`everyMs`)과 다른 값이다. 저건 스스로 볼 주기고, 이건 **누가 물었을 때**
   * 내주는 그림의 신선도다. 90초 전 화면을 두고 지금 뭐 보여에 답하면 그건 거짓말이다.
   */
  freshMs?: number;
...
export interface ScreenElement {
   ...
   * 탭 닫기가 넷이었다(120회차 실측). 탭 닫기 눌러로는 어느 것인지 못 집는다.
 ...
 * 두뇌는 전부 화면이 비었다로 읽는다. 그리고 상한은 120 인데 msedge 는 화면 안에만
...
export function screenSense(options: ScreenSenseOptions = { everyMs: 90_000 }): ScreenEye {
  ...
  /** 마지막으로 읽어 낸 것. 지금 보이는 것을 물으면 이걸 글로 편다. */
  ...
  async function look(emit: (sensation: Sensation) => void): Promise<void> {
    // 앞의 촬영이 안 끝났으면 건너뛴다. 밀린 화면이 쌓이면 지금이 아니게 된다.
    if (busy) return;
 ...
 * 성공률을 +5.1%p 올렸다. 그 세 단계 중 하나가 중복 지우기다. 그래서 **먼저 쟀다**
...
/**
 * 목록에 **무엇을 넣을까**. 자르는 기준이 나무 순서였다 (TASK-KAR-246).
 *
