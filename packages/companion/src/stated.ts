 ...
 *
 * 우리 아는 것(16회차)은 전부 **두뇌가 졸여서 만든 짐작**이다. 조수님이 나 커피 좋아해라고
 * 직접 말해도, 나중에 졸이기가 커피를 싫어함이라고 써 버리면 그만이다. 33회차에 말로
 * 고치는 길을 냈지만 **고쳐 놓은 게 다시 덮이는 것**은 못 막았다.
 *
 * 그래서 **직접 들은 것을 따로 쌓는다.** 자기상(23회차)이 얘가 저에 대해 한 말이라면
 * 이건 그 짝인 **조수님이 저에 대해 한 말**이다.
 *
 * 뽑는 것도 좁게 잡는다. 아무 말이나 사실로 쌓으면 그건 그냥 대화 기록이다. **자기에
 ...
 *
 * 오늘 피곤해는 사실이 아니라 오늘 기분이다. **그때뿐인 말은 뺀다**. 안 빼면 피곤함이
 * 영구 사실로 굳는다.
 */
