 ...
 * 언제부터 알던 것인가.
 *
 * 아는 것은 한 장의 목록이라 **전부 똑같은 무게**로 놓여 있었다. 방금 들은 것과 몇 주
 * 함께 지내며 굳어진 것이 나란히 적혀 있고, 얘는 그 둘을 구분할 방법이 없었다.
 *
 * 그래서 이런 게 나온다. 2분 전에 처음 들은 걸 예전부터 알았잖아라고 말한다. 사람은
...
export class KnownStamps {
  ...
  /**
   * 지금의 아는 것과 맞춘다. 새 줄은 오늘부터, 있던 줄은 마지막 본 날만 갱신.
   *
   * **사라진 줄은 지우지 않는다.** 졸이는 쪽이 한 번 빠뜨렸다고 처음 본 날을 잃으면,
   * 다음에 돌아왔을 때 몇 주 알던 것이 갑자기 오늘 알게 된 것이 된다.
   */
  sync(known: string | null): void {
    const now = (this.options.now ?? Date.now)();
    // **새 줄이 생겼을 때만 파일에 남긴다.** 마지막 본 날이 바뀔 때마다 다시 쓰면 이
  ...
  /** 잘못 쌓인 줄을 지운다. 아는 것에서 지울 때 같이 부른다. */
