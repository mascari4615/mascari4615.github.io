 ...
 * 그런데 **조수님이 직접 말할 방법이 없었다.** 회의 들어가기 전에 좀 있다 얘기해라고
 * 하고 싶어도 방법이 창을 닫는 것뿐이었다.
 *
 * 핵심 하나: **조용히 하라고 해도 말을 걸면 답한다.** 조용히 있으라는 건 먼저 걸지 마라지
 * 벙어리가 되라가 아니다. 이걸 헷갈리면 얘가 말을 걸어도 대답을 안 하는 고장처럼 보인다.
 */
...
/**
 * 시간을 읽는다. 숫자만 보면 한 시간을 놓친다. 사람은 그렇게 말한다.
 *
 * 바쁘만 넣으면 바빠를 못 알아듣는 것과 같은 부류다(ㅂ 불규칙). 실제로 둘 다 놓쳤다.
 */
 ...
 * 이제 됐어로 풀리는 것은 부탁 쪽뿐이다. 한마디로 밤을 없앨 수는 없다.
 */
export class Quiet {
  private until = 0;
...
export function quietNote(quiet: Quiet): string {
  ...
  return quiet.hushed
    // **부탁받아 조용한 것과 그냥 밤인 것은 다르다.**
    //
    // 둘 다 아주 짧게라고 시켰더니, 밤 열한 시가 넘자 무엇을 물어도 ...졸려...
    // 한마디만 돌아왔다. 대화가 통째로 죽었다(실측: 재료를 눈으로 보고 찾았다).
    // 조용히 있어 달라고 *부탁받았을 때*는 짧은 게 맞다. 그냥 밤인 것은 **먼저 말을
    // 안 거는 것**이지, 물어본 말에 성의 없이 답하라는 뜻이 아니다.
    ? `조수님이 조용히 있으라고 했다 (${quiet.leftSay()}). 물으면 답하되 **아주 짧게**, 먼저 말 걸지 마라.`
    : '지금은 밤이라 먼저 말 걸지 않는다. 다만 물어보면 평소처럼 제대로 답하라. 밤이라고 성의 없이 굴 이유는 없다.';
}
