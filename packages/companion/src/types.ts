...
export interface Sensation {
   ...
   * 사람의 말로 쌓였다. 졸이고 나니 아는 것이 통째로 이렇게 됐다:
...
export interface MemoryEntry {
   ...
   *
   * 옛 기록에는 없다. 없으면 모른다로 다루고 함부로 두뇌 것으로 치지 않는다.
   */
  via?: 'brain' | 'reflex';
...
export interface ThinkInput {
  ...
  /**
   * 지금 눈에 보이는 것. 방금 찍은 화면 그림의 자리.
   *
   * `sensation.meta.imagePath` 와 다르다. 그건 화면을 봤다는 감각이 스스로 들고 온
   * 그림이라 **화면 감각이 만든 turn 에만** 있다. 사람이 화면에 뭐 보여?라고 물은
   * turn 에는 없었고, 그래서 얘는 창 제목만 보고 답했다(99회차 라이브: npm ci 돌고
   * 있네... = TITLE 값 그대로).
   *
