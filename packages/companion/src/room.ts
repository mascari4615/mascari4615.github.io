 ...
 * 값어치는 무슨 말을 하나보다 언제 입을 여나에서 먼저 갈린다.
 *
 ...
 * 를 보는 데 쓰고 있던 것뿐이다.
 *
 * **모르면 모른다고 한다.** 억지로 갈래를 붙이면 엉뚱한 자리에서 입을 다물거나 연다.
 */
...
const kindTable: readonly point[] = [
  /* **우리 창.** 78회차에 떠 있는 창을 세어 보니 얘는 자기 창조차 모름이었다. 그런데
     사람이 얘를 보고 있는 건 아무 자리도 아닌 게 아니라 **말 걸기 가장 좋은 때**다.
     우리가 지은 제목이라 여기는 표가 맞다. 두뇌한테 물어볼 일이 아니다. */
  { slot: '나를보는중', word: ['동반자'] },
  // 디스코드는 제목이 #방이름 | 서버이름 - Discord로 온다(실측). 앞에 붙임표를
  ...
  {
    slot: '읽는중',
    // 브라우저 제목은 ... - 개인 - Microsoft Edge처럼 사이에 뭐가 낀다. 앞에 붙임표를
