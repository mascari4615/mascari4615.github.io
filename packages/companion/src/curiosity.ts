 ...
 * 가끔을 정하는 값도 밖에서 넣게 뒀다. 시험할 수 없는 무작위는 두지 않는다.
 */
 ...
 * 조사를 뗀다. 회의가 → 회의.
 *
 * 안 떼면 같은 말이 회의가 회의를 회의는으로 흩어져 매번 새로 궁금해한다.
 */
 ...
 * 이 낱말이 **물어볼 만한 것**인가.
 *
 * 레퍼런스가 짚는 척도는 몇 개를 뽑았나가 아니라 **그중 실제로 쓸 만한 게 몇 개인가**다.
 * 실측(30, 31회차): 쌓인 궁금증 30개 중 쓸 만한 게 **0개**였다. 어때 에 대해 더
 * 알려주세요 에 대해 더처럼 사람이 읽어도 무슨 말인지 모르는 것들이었다.
 *
 * 원인은 단순했다. **가장 긴 낱말**을 골랐던 것이다. 한국어에서 긴 낱말은 대개 **활용된
 * 서술어**(힘들었어 알려주세요)라, 길이로 고르면 동사만 골라진다. 궁금할 만한 것은
 * 거의 언제나 **name 붙은 것**. 사물, 일, 사람이다.
 *
 * 그래서 서술어 꼴을 걸러낸다. 완벽한 품사 가르기는 못 하지만, **못 쓸 것을 안 담는 쪽이
 * 많이 담는 것보다 낫다.**
 *
 * 못 거르는 것도 있다. 만드는 같은 관형형은 는을 떼면 서술어 표시가 사라지는데,
 * 회의는도 똑같이 떼야 하므로 규칙으로 가를 수 없다. 흔한 것만 손으로 막아 뒀다.
 */
export function worthWondering(word: string, skip: ReadonlySet<string> = commonWords): boolean {
  ...
  //
  // 31회차 규칙은 마지막 한 글자만 봐서 이것들을 통째로 놓쳤다. 실제로 좋아해가
...
const commonWords = new Set([
  ...
  // 묻는 말 자체. 무엇 에 대해 더 어때 에 대해 더가 실제로 쌓였다(실측).
  '무엇', '어때', '어떻', '언제', '어디', '누구', '얼마', '어느', '몇개', '뭔가', '뭐가',
  '하자', '하나', '가지', '자기', '소개', '기능', '목록', '방법', '대해', '경우',
  // 만드는 같은 관형형은 기계로 못 가린다. 는을 떼면 만드가 되어 서술어 표시가
  // 사라지고, 회의는도 똑같이 회의가 되기 때문이다. 흔한 것만 손으로 막는다.
