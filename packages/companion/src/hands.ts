 ...
 * 실행하게 열어두지 않는다. 지금 쥐여준 것은 적어두기와 알려주기 둘뿐이고,
...
export interface Hand {
   ...
   *
   * **안 적으면 못 되돌린다로 친다.** 손을 새로 만든 사람이 표시를 잊었을 때 조용히
   * 위험한 쪽으로 열리면 안 된다.
   */
  ...
  for (const request of requests) {
    ...
    if (hand === undefined) {
      ...
      /* **못 한 것도 결과다.** 빈손으로 돌아가면 두뇌는 막힌 줄도 모르고, 그러면 얘는
         못 했어라고 말할 수가 없다. 사람 눈에는 그냥 침묵이다(121회차 라이브).
 ...
 * 기억 찾기. 옛 대화를 뒤진다.
 *
 * 두뇌에게 넘어가는 건 최근 몇 마디뿐이라, 저번에 말한 그거를 물으면 얘는 모른다.
 * 뉴로사마가 기억을 좀 찾아봐야겠다며 실제로 뒤지는 것과 같은 자리다.
 *
