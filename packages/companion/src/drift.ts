 ...
 * 방금 이렇게 샜다고 알려 준다. 고리를 끊는 건 지적 한 줄이면 된다.
 */
...
const defaultBan: readonly RegExp[] = [
  /* 존댓말. 곁에 있는 사이에 갑자기 격식이 끼면 그게 표류다.
     **처음엔 몇 개만 적어 놨다가 통째로 놓쳤다**(87회차 실측). 얘가 ...없는 상태예요
     답답하신 게 있어요?라고 말하는데 검사가 조용했다. 해요만 막아 놓으니
     예요, 어요, 으세요가 다 빠져나갔다. **한국말 높임은 끝에서 난다.** 끝을 본다.
     반말에는 요가 안 붙으므로 이렇게 넓혀도 얘 결은 안 걸린다. */
  ...
  /* 위 목록은 **여전히 표였다.** 소설/스토리 캐릭터의 대사인가요? 다른 프로젝트인가요?가
     ...
     낱말을 하나 더 적는 건 다음 낱말을 기다리는 일이다. **끝이 요/죠면 높임**으로 본다
...
export function checkDrift(said: string, rules: DriftRules = {}): Drift {
  ...
  /* **제가 누구라고 말하나.**
     ...
     캐릭터 카드에 너는 개발 도구가 아니다가 적혀 있지만 말로 시킨 것은 안 지켜진다. */
 ...
 * 따로 있다. 레퍼런스가 무난한 메꿈이라 부르는 것. 확신이 없을 때 바꿔 말하기 + 뻔한
 ...
 * 제 정체를 도구라고 말하나.
 *
 * **나는/저는이 앞에 붙은 것만** 본다. 우리 대화에는 도구 이름이 늘 나오고
 ...
 * **나가기 전에 막아야 하는 표류.**
 *
 * 표류 감시는 원래 샜으면 **다음 번에** 일러 준다로 만들었다(이 파일 첫 주석). 말투가
