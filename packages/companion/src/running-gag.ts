 ...
 * 좋다가 아니라 **같이 지냈다**는 뜻이 된다. 레퍼런스 쪽 재미의 큰 축도 이것이다. 몇 달째
 * 이어지는 농담, 서로 아는 얘기.
 *
 * 우리 얘는 옛 대화를 찾아 붙이긴 한다(12회차). 그런데 그건 **지금 얘기와 관련된 것**을
 * 찾는 것이고, **자꾸 되풀이되는 것**은 모른다. 우리 사이에 자주 나오는 얘기라는 게 없다.
 *
...
export interface RecurringOptions {
  ...
  /** 몇 개까지 들고 올지. */
  keep?: number;
  /** 하루에 몰아 나온 것도 셀까. 기본은 **아니오**. 여러 날 걸쳐야 우리끼리다. */
  needDays?: number;
}
 ...
 * 자꾸 나오는 얘깃거리를 찾는다.
 *
 * **조수님이 한 말만** 본다. 얘가 한 말까지 세면 제가 자주 쓰는 낱말을 우리끼리 그거로
 * 착각한다. 그리고 **닿은 것**(쿡 찌르기 같은)은 뺀다. 그건 대화가 아니라 몸짓이고,
 * 조수님이 나를 쿡 찔렀다가 스물아홉 번 쌓여 있으면 그게 1등이 된다(실측 48회차).
 */
 ...
 * 나온다(39회차에서 배웠다). 우리끼리 자주 나오는 얘기라는 사실만 준다.
 */
