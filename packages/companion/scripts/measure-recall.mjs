 ...
 * 옛말을 얼마나 잘 불러오나. **점수판**.
 *
 * TASK-KAR-236 § 재는 판. 지금까지 회상은 되는 것 같다/안 되는 것 같다로만 말했다.
 * 105, 106, 107, 110, 111회차가 전부 회상 재료를 손댔는데, 그게 나아졌는지 **숫자가 없었다.**
 *
 * 밖에서는 이걸 LoCoMo, LongMemEval 로 잰다(원장 2026-08-21). 그건 영어 판이라 그대로
 * 못 쓴다. 그래서 **우리 판**을 둔다. 짝 하나는 옛날에 오간 말과 나중에 그걸 가리키는
 * 물음이고, 물음에는 **옛말의 낱말이 되도록 안 겹치게** 넣었다. 낱말이 겹치면 낱말 회상만으로
 * 맞으니 잰 값이 부풀려진다.
 *
 * 쓰는 법:
...
if (withMeaning) {
  ...
  /* **점수판은 기다린다.** 뜻 재는 자리는 기다리게 하지 않는다가 설계라(94회차)
...
if (withMeaning) {
  ...
  /* 둘 중 하나라도 찾으면 사람은 기억한다고 느낀다. 그게 체감치다. */
