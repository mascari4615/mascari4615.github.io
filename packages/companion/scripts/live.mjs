 ...
 * 지킴이. 동반자를 계속 살려둔다.
 *
 * 내가 실행하는 프로그램과 그냥 거기 있는 존재를 가르는 건 이것 하나다.
 ...
 * 검은 창이 남고, 그 창을 닫으면 얘도 같이 죽는다. 그냥 거기 있는 존재인데 창 하나를
...
function fromEnd() {
  ...
  // 이어 붙인다. 다시 띄울 때마다 지난 판이 사라지면 어제 왜 죽었나를 못 본다.
...
function supervise() {
  ...
  if (built !== null) {
    ...
    if (process.env.COMPANION_AUTOBUILD === '1') {
      ...
      console.log('창 프로그램이 없다. KarmoLab 데스크톱 앱 설치에서 동반자 창을 깔면 다음부터 그 창으로 뜬다.');
 ...
 * (격리 빌드용 `target-claude` 등) 엉뚱한 데 구워 놓고 없다고 하게 된다.
 */
function build() {
...
function keepAlive(label, command, args, options, onCleanExit) {
  ...
  const start = () => {
    ...
    child.on('exit', (code) => {
      ...
      // 되살리지 않는다. 안 그러면 닫아도 곧바로 다시 떠서 안 꺼진다가 된다.
