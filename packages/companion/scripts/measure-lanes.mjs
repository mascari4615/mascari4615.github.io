...
const heavyWorkMs = Number(process.env.LANE_HEAVY_MS ?? '3000');
...
const measure = async () => {
  ...
  const body = {
    ...
    voice: {
      name: 'web:voice',
      speak(u) {
        producedText.push({ text: u.text, at: Date.now() });
      },
    },
  };
  ...
  return { waitedMs: reply === undefined ? null : reply.at - sentAt, producedText: producedText.map((m) => m.text) };
};

const r = await measure();
console.log(`[계통] 무거운 일 ${heavyWorkMs}ms 가 도는 중에 말을 걸었다`);
console.log(`[계통] 대답까지 ${r.waitedMs === null ? '(대답 없음)' : `${r.waitedMs}ms`}`);
console.log(`[계통] 나온 말 순서: ${r.producedText.join(' → ')}`);
