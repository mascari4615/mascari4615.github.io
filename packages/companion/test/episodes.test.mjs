...
test('화면 곁눈질은 안 센다', () => {
  const s = new EpisodeStore();
  s.learn([person('진짜 너무 속상해 완전 망했어', 1, 'screen')]);
  assert.equal(s.all.length, 0);
});

test('같은 말은 두 번 안 담는다', () => {
  const s = new EpisodeStore();
  s.learn([person('오늘 발표 진짜 망했어 속상해')]);
  s.learn([person('오늘 발표 진짜 망했어 속상해')]);
...
test('자리가 모자라면 기운 약한 것부터 버린다. 오래된 큰일이 먼저 사라지면 안 된다', () => {
  ...
  s.learn([person('완전 신나고 뿌듯하고 행복해 드디어 해냈어 진짜 너무 기뻐', 3)]);
...
test('이어지는 일이 있으면 그때 말을 그대로 보여 준다', () => {
  const s = new EpisodeStore();
  s.learn([person('오늘 발표 진짜 망했어 너무 속상해', 1000)]);
  const note = episodeNote(s, '발표 얘기 나왔는데 속상하더라', 1000 + day * 3);
...
test('얼마나 지났는지 사람 말로', () => {
  const now = 100 * day;
  assert.equal(roughlyWhen(now - day * 40, now), '한참 전에');
  assert.equal(roughlyWhen(now - day * 9, now), '지난주쯤');
  assert.equal(roughlyWhen(now - day, now), '어제');
  assert.equal(roughlyWhen(now - 1000, now), '아까');
});
...
test('파일에 남겨 두면 껐다 켜도 이어진다', () => {
  ...
  try {
    new EpisodeStore({ path }).learn([person('오늘 발표 진짜 망했어 너무 속상해')]);
