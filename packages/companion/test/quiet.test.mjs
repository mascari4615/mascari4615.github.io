...
test('조용히 해 달라는 말을 알아듣는다', () => {
  for (const text of ['좀 조용히 해', '지금 바빠', '이따 얘기하자', '나중에', '집중해야 해', '말 걸지 마']) {
    assert.notEqual(asksForQuiet(text), null, `${text} 은 조용히 해 달라는 말이다`);
  }
});

test('그냥 하는 말은 부탁이 아니다', () => {
  for (const text2 of ['오늘 회의 길었어', '셰이더 못 고쳤어', '뭐 하고 있었어']) {
    assert.equal(asksForQuiet(text2), null, `${text2}`);
  }
});

test('시간을 말하면 그만큼 조용히 있는다', () => {
  assert.equal(asksForQuiet('한 시간만 조용히 해').ms, 3600_000);
  assert.equal(asksForQuiet('20분만 조용히').ms, 20 * minutes);
});
...
test('평소에는 먼저 말을 걸어도 된다', () => {
  assert.equal(make().q.maySpeakFirst, true);
});
