...
test('준비 안 됐으면 대타가 말한다 — 조용해지지 않는다', async () => {
  const t = 판();
  const 목소리 = 필요할때({ 진짜: 가짜목소리('흉내'), 대타: 가짜목소리('인터넷'), 기동: t.기동 });
  assert.equal((await 목소리.synthesize('안녕')).toString(), '인터넷');
  await 기다림();
  // 뒤에서 떴으니 이제 진짜가 말한다.
  assert.equal((await 목소리.synthesize('안녕')).toString(), '흉내');
});

test('떠 있다고 봤는데 실패해도 이번 말은 나간다', async () => {
  const t = 판();
  t.켜두기();
  const 목소리 = 필요할때({ 진짜: 가짜목소리('흉내', true), 대타: 가짜목소리('인터넷'), 기동: t.기동 });
  assert.equal((await 목소리.synthesize('안녕')).toString(), '인터넷');
});
