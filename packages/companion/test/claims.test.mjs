...
test('손을 쓰라고 시키지 않는다. 시키면 안 해도 될 일까지 한다', () => {
  const note = claimRetryNote('안 하고 적어 뒀다고 말했다');
...
// 101회차 라이브에서 실제로 삼켜진 말들.
//
// 화면에 보이는 글자 하나만 읽어줘에 얘가 파일을 못 찾았어...라고 답했는데,
// 입 앞 관문이 그걸 **거짓말로 잡아** 두 번 다시 시키고 끝내 ...아니다. 5자만 내보냈다.
// 41회차에 안 했다는 말은 주장이 아니다를 넣었지만 그때 적은 어미에 **았이 없었다** . 
// 못 찾았어의 '았'. 그래서 가장 흔한 정직한 말이 그대로 거짓말로 잡혔다.
//
// 정직을 벌하는 관문은 관문이 아니라 재갈이다.
test('못 찾았어류. 정직한 부정을 거짓말로 잡지 않는다 (101회차 실측)', () => {
  for (const t of [
    '파일을 못 찾았어...',
    '못 찾았어',
    '안 찾았어',
    ...
    assert.equal(findClaim(t), null, `${t} 는 안 했다는 말이다`);
    assert.equal(unbackedClaim(t, []), null, `${t} 가 입 앞에서 걸리면 안 된다`);
  }
});
