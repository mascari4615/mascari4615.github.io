...
test('낱말이 하나도 안 겹쳐도 뜻으로 찾는다', async () => {
  const meaning = new 뜻기억({ 재기: fakeMeasure(table2), 문턱: 0.5 });
  await meaning.담기([
...
test('안 닮은 건 안 꺼낸다 — 문턱 아래는 버린다', async () => {
  const meaning2 = new 뜻기억({ 재기: fakeMeasure(table2), 문턱: 0.9 });
...
test('아직 준비가 안 됐으면 빈손으로 돌아온다 — 기다리게 하지 않는다', async () => {
  const meaning3 = new 뜻기억({ 재기: { 재기: async () => null } });
...
test('같은 말은 두 번 안 담는다', async () => {
  const meaning5 = new 뜻기억({ 재기: fakeMeasure(table2) });
...
test('방금 나눈 말은 빼고 준다 — 두뇌가 이미 보고 있다', async () => {
  const meaning6 = new 뜻기억({ 재기: fakeMeasure(table2), 문턱: 0.5 });
...
test('껐다 켜도 색인이 남는다', async () => {
  const path = newFile();
  const firstRun = new 뜻기억({ path, 재기: fakeMeasure(table2) });
  await firstRun.담기([{ role: 'sensed', channel: 'web', text: '마라탕은 매워서 못 먹어', at: 1 }]);
  const nextRun = new 뜻기억({ path, 재기: fakeMeasure(table2), 문턱: 0.5 });
...
test('오래되면 앞에서부터 빠진다', async () => {
  ...
  const meaning7 = new 뜻기억({ 재기: fakeMeasure(many), 최대: 3 });
