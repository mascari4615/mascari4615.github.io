...
test('자기에 대해 대놓고 말한 것을 뽑는다', () => {
  const chosen = statedFacts([person('나 커피 진짜 좋아해')]);
...
test('얘가 한 말은 안 센다', () => {
  assert.deepEqual(statedFacts([companion('나 커피 좋아해')]), []);
});

test('화면 곁눈질도 안 센다', () => {
  assert.deepEqual(statedFacts([person('나 커피 좋아해', 1, 'screen')]), []);
});

test('너무 짧거나 너무 긴 말은 안 센다', () => {
  assert.deepEqual(statedFacts([person('나 좋아')]), []);
  assert.deepEqual(statedFacts([person(`나 커피 좋아해 ${'가'.repeat(100)}`)]), []);
});
...
test('쌓인다', () => {
  ...
  s.learn([person('나 커피 진짜 좋아해')]);
  s.learn([person('나 매운 거 못 먹어')]);
...
test('너무 많이 들고 있지 않는다', () => {
  ...
  for (let i = 0; i < 8; i += 1) s.learn([person(`나 ${i}번 좋아해`, i)]);
...
test('직접 들은 것과 아는 것이 어긋나면 잡는다', () => {
  const drift = findConflicts([{ said: '나 커피 진짜 좋아해', at: 1 }], '- 커피를 싫어함\n- 한국어를 씀');
  assert.equal(drift.length, 1);
  assert.match(drift[0], /커피/);
});
...
test('너무 많이 늘어놓지 않는다', () => {
  const many = Array.from({ length: 10 }, (_, i) => ({ said: `나 ${i}번 좋아해`, at: i }));
...
test('파일에 남겨 두면 껐다 켜도 이어진다', async () => {
  ...
  try {
    new StatedStore({ path }).learn([person('나 커피 진짜 좋아해')]);
