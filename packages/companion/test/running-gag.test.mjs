...
test('얘가 한 말은 안 센다. 제가 자주 쓰는 낱말을 우리끼리 그거로 착각한다', () => {
  const es = [companion('셰이더 얘기였나', twoDaysAgo), companion('셰이더', yesterday), companion('셰이더', today)];
  assert.deepEqual(recurringThings(es), []);
});

test('닿은 것은 대화가 아니다. 안 빼면 조수님이 나를 찔렀다가 1등이 된다', () => {
...
test('서술어는 안 센다. 좋아해가 단골 얘기가 되면 안 된다', () => {
...
test('여러 날에 걸친 것을 먼저 둔다. 오래 이어진 게 더 우리끼리다', () => {
  const es = [
