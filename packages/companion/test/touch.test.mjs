...
test('계속 찌르면 결이 옮겨 간다. 같은 자극에 같은 소리만 내면 버튼이다', () => {
  ...
  assert.notEqual(times2, first);
  assert.notEqual(keepGoing, times2);
  assert.match(keepGoing, /그만/);
});

test('끌고 다니면 어지러워한다', () => {
  assert.match(touchReply('흔듦', { times: 1, roll: firstOne }), /어/);
  assert.match(touchReply('흔듦', { times: 9, roll: firstOne }), /(멀미|그만|아무 데나)/);
});
...
test('바로 전에 한 대꾸를 또 하지 않는다', () => {
  const first2 = touchReply('쿡', { times: 1, roll: firstOne });
...
test('닿은 것에는 두뇌를 부르지 않는다. 2초 뒤 문장은 반응이 아니라 답변이다', async () => {
  ...
  let thought = 0;
  const said = [];
  const mouth = { name: 'v', speak(u) { said.push(u.text); } };
  const companion = new Companion({
    bodies: [
      { name: 'web', sense: { name: 's', start() {} }, voice: mouth },
      { name: TOUCH_CHANNEL, sense: { name: '닿음', start() {} }, voice: mouth },
    ],
    ...
    reflex: (s) => {
      ...
      return kind === null ? null : touchReply(kind, { times: 1, roll: firstOne });
    },
  });
...
test('닿은 것도 기억에는 남는다. 곁에서 있었던 일이니까', async () => {
  ...
  const companion = new Companion({
    bodies: [
      { name: 'web', sense: { name: 's', start() {} }, voice: mouth2 },
      { name: TOUCH_CHANNEL, sense: { name: '닿음', start() {} }, voice: mouth2 },
    ],
...
test('닿은 것에는 눈치를 보지 않는다. 나를 찔렀는데 바쁘신 것 같아 참았다는 말이 안 된다', () => {
  ...
  assert.equal(
    hint.shouldRespond({ sensation: { channel: 'screen', kind: 'text', text: '화면을 봤다', at: Date.now() }, recent: recent2 }).respond,
    false,
    '곁눈질은 여전히 눈치를 본다',
  );
});
...
test('다 쓰면 비우고 다시 돈다. 말이 떨어져서 멈추면 안 된다', () => {
  ...
  const produced2 = [];
  let last;
  for (let i = 0; i < 9; i += 1) { last = touchReply('쿡', { times: 9, last, roll: () => 0 }); produced2.push(last); }
  assert.equal(produced2.length, 9);
