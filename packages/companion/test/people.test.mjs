...
test('부르는 말까지 통짜로 잡는다 — 「김」으로 줄이면 김 과장과 뒤섞인다', () => {
  ...
  assert.equal(one.includes('김'), false);
});
...
test('너무 많이 들고 있지 않는다', () => {
  ...
  for (let i = 0; i < 8; i += 1) p.learn([person(`사람${i} 씨랑 만났어`, i)]);
  assert.ok(p.known.length <= 3);
});

test('잘못 주운 사람은 지울 수 있다', () => {
  const p = new People({ needTimes: 1 });
  p.learn([person('김 대리랑', 1)]);
...
test('한동안 얘기 안 나온 사람에게 안부를 묻는다', () => {
  const p = new People();
  p.learn([person('김 대리랑', 0), person('김 대리 또', 1000)]);
...
test('방금 나온 사람에게는 안 묻는다 — 「아까 그 김 대리는 요즘 어때?」는 이상하다', () => {
  ...
  assert.equal(p.whoToAskAbout(now), null);
});

test('가장 오래 안 나온 사람을 고른다', () => {
  const p = new People();
  p.learn([person('김 대리랑', 0), person('김 대리 또', 1)]);
  p.learn([person('박 과장이랑', 10 * day), person('박 과장 또', 10 * day + 1)]);
  assert.equal(p.whoToAskAbout(30 * day)?.name, '김대리');
});

test('아직 인정 안 된 사람에게는 안부를 안 묻는다', () => {
  const p = new People();
  p.learn([person('김 대리랑', 0)]);
  assert.equal(p.whoToAskAbout(30 * day), null);
});
...
test('너무 많이 늘어놓지 않는다', () => {
  const many = Array.from({ length: 10 }, (_, i) => ({ name: `사람${i}`, times: 2, firstAt: 1, lastAt: i }));
  const note = peopleNote(many, 3);
...
test('파일에 남겨 두면 껐다 켜도 이어진다', async () => {
  ...
  try {
    const first = new People({ path });
    first.learn([person('김 대리랑', 1)]);
    first.learn([person('김 대리 또', 2)]);
