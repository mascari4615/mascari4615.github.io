...
test('할 말이 없는 재료는 안 꺼낸다 — 빈 걸 꺼내라고 하면 지어낸다', () => {
  const emptyOnly = { ...base, material: [{ name: '궁금', text: '   ', weight: 5 }], heldFor: () => 9 };
  assert.equal(topicFirst(emptyOnly), null);
});

test('꺼진 재료도 안 꺼낸다 — 지금 자리에 없는 얘기다', () => {
  const off = { ...base, material: [{ name: '놀리기', text: '놀려라', weight: 5, when: false }], heldFor: () => 9 };
...
test('실제로 밀린 것을 세어 꺼낸다 — 표를 따로 들지 않는다', () => {
  ...
  for (let i = 0; i < 4; i += 1) { pressed.write('단골얘기', '밀림'); pressed.다음턴(); }
  pressed.write('기분', '실림');
...
test('말하고 나면 풀려서 또 안 꺼낸다', () => {
  ...
  for (let i = 0; i < 4; i += 1) { pressed2.write('단골얘기', '밀림'); pressed2.다음턴(); }
  pressed2.write('단골얘기', '실림');
