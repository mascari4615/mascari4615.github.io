...
const brain = (text, at = 1) => ({ role: 'said', channel: 'web', text, at });
const marked = (text, at = 1) => ({ ...brain(text, at), via: 'brain' });
const reflex = (text, at = 1) => ({ role: 'said', channel: 'web', text, at, via: 'reflex' });

// ── 말라 가나 ───────────────────────────────────────────────────────

test('짧은 답만 이어지면 마른 것이다', () => {
  assert.equal(isDrying([marked('소파...'), marked('음...'), marked('고양이...')]), true);
});

test('되묻고 있으면 안 마른 것이다. 짧아도 대화는 살아 있다', () => {
  assert.equal(isDrying([marked('소파...'), marked('넌 어때?'), marked('음...')]), false);
});

test('길게 말하고 있으면 안 마른 것이다', () => {
  assert.equal(isDrying([
    marked('음 그건 어제도 비슷했는데 오늘은 좀 다르네'),
    marked('아까 그 창 계속 켜 있더라'),
    marked('셰이더 그거 아직인가'),
  ]), false);
});

test('말이 몇 마디 없으면 판단하지 않는다. 두 마디로 마른 대화라 부를 수 없다', () => {
  assert.equal(isDrying([marked('음...'), marked('소파...')]), false);
});

test('고정 대꾸는 안 센다. ...어?가 짧다고 대화가 마른 건 아니다', () => {
  assert.equal(isDrying([reflex('...어?'), reflex('왜.'), reflex('...그만.')]), false);
});

test('어디서 왔는지 모르는 옛 기록도 안 센다', () => {
  assert.equal(isDrying([brain('소파...'), brain('음...'), brain('고양이...')]), false);
});

test('얼마나 짧아야 마른 것인지 정할 수 있다. 인격마다 다르다', () => {
  const es = [marked('그럭저럭 지냈어'), marked('나도 그랬는데'), marked('음 그러네')];
...
test('억지로 이어 붙이지 말라고 못 박는다. 그건 그렇고는 대화가 아니다', () => {
...
test('마를 때만 한 줄이 나온다', () => {
  const dried = [marked('소파...'), marked('음...'), marked('고양이...')];
  assert.notEqual(tangentFor(dried, { wondering: '그 게임' }), '');
});

test('안 마르면 조용하다. 잘 굴러가는 대화에 끼어들지 않는다', () => {
  const alive = [marked('소파...'), marked('넌 어때?'), marked('음...')];
  assert.equal(tangentFor(alive, { wondering: '그 게임' }), '');
});

test('말라도 꺼낼 게 없으면 조용하다', () => {
  const dried2 = [marked('소파...'), marked('음...'), marked('고양이...')];
  assert.equal(tangentFor(dried2, {}), '');
});
