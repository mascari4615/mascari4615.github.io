...
test('너무 많이 들고 있지 않는다', () => {
  ...
  for (let i = 0; i < 20; i += 1) w.saw(`창 ${i}`, i * minutes);
...
test('얼마나 붙들고 있는지 잰다', () => {
  const w = new Watching();
  w.saw('유니티', 0);
  assert.equal(w.heldFor(30 * minutes), 30 * minutes);
});

test('오래 붙들면 붙들고 있는 것으로 본다', () => {
  const w = new Watching({ stuckAfterMs: 40 * minutes });
  w.saw('유니티', 0);
  assert.equal(w.isStuck(30 * minutes), false);
  assert.equal(w.isStuck(41 * minutes), true);
});

test('바꾸면 다시 처음부터 센다', () => {
  const w = new Watching({ stuckAfterMs: 40 * minutes });
  w.saw('유니티', 0);
  w.saw('브라우저', 50 * minutes);
  assert.equal(w.isStuck(60 * minutes), false);
});

test('본 게 없으면 붙들 것도 없다', () => {
  assert.equal(new Watching().isStuck(999 * minutes), false);
  assert.equal(new Watching().heldFor(999 * minutes), 0);
});
...
test('두 창을 되풀이해 오가면 왔다갔다다', () => {
  ...
  const order = ['유니티', '브라우저', '유니티', '브라우저', '유니티'];
  order.forEach((t, i) => w.saw(t, i * minutes));
...
test('몇 번 안 오갔으면 왔다갔다가 아니다', () => {
  ...
  w.saw('유니티', 0);
  w.saw('브라우저', minutes);
  assert.equal(w.isFlipping(2 * minutes), false);
});

test('오래전 일은 왔다갔다로 안 센다', () => {
  const w = new Watching({ flipWindowMs: 10 * minutes, flipsAtLeast: 4 });
  ['유니티', '브라우저', '유니티', '브라우저'].forEach((t, i) => w.saw(t, i * minutes));
...
test('아무 일 없으면 조용하다', () => {
  const w = new Watching({ stuckAfterMs: 40 * minutes });
  w.saw('유니티', 0);
  assert.equal(watchNote(w, 5 * minutes), '');
});

test('오래 붙들고 있으면 알려 주되 함부로 끊지 말라고 한다', () => {
  const w = new Watching({ stuckAfterMs: 40 * minutes });
  w.saw('WitchMendokusai - Unity', 0);
  const note = watchNote(w, 50 * minutes);
...
test('아는 척하지 말라고 한다', () => {
  const w = new Watching({ flipWindowMs: 10 * minutes, flipsAtLeast: 4 });
  ['가', '나', '가', '나', '가'].forEach((t, i) => w.saw(t, i * minutes));
  assert.match(watchNote(w, 5 * minutes), /아는 척은 하지 마라/);
});
...
test('씬 이름만 바뀐 건 같은 것으로 본다 — 안 그러면 두 시간을 붙들어도 안 잡힌다', () => {
  ...
  w.saw('WitchMendokusai - Arena - Unity', 40 * minutes);
