import assert from 'node:assert/strict';
import test from 'node:test';

import { clonedSpeech, moodSpeed } from '../dist/index.js';

/** 진짜로 안 보내고, 무엇을 보내려 했는지만 가로챈다. */
function intercept() {
  const sent = [];
  const original = globalThis.fetch;
  globalThis.fetch = async (url, init) => {
    sent.push(JSON.parse(init.body));
    return { ok: true, arrayBuffer: async () => new ArrayBuffer(8) };
  };
  return { sent: sent, restore: () => { globalThis.fetch = original; } };
}

const voice = () => clonedSpeech({ refAudioPath: 'ref.wav', refText: '참고' });

test('결이 없으면 늘 하던 속도', async () => {
  const { sent: sent, restore: restore } = intercept();
  try {
    await voice().synthesize('안녕');
    assert.equal(sent[0].speed_factor, 1);
  } finally { restore(); }
});

test('처진 결은 느리게 — 결을 붙여 놓고 안 받으면 붙인 적 없는 것과 같다', async () => {
  const { sent: sent, restore: restore } = intercept();
  try {
    await voice().synthesize('안녕', 'cloned@droop');
    assert.ok(sent[0].speed_factor < 1, `느려야 하는데 ${sent[0].speed_factor}`);
  } finally { restore(); }
});

test('들뜬 결은 빠르게', async () => {
  const { sent: sent, restore: restore } = intercept();
  try {
    await voice().synthesize('안녕', 'cloned@들뜸');
    assert.ok(sent[0].speed_factor > 1);
  } finally { restore(); }
});

test('모르는 결은 늘 하던 속도 — 엉뚱한 값이 들어와도 안 튄다', async () => {
  const { sent: sent, restore: restore } = intercept();
  try {
    await voice().synthesize('안녕', 'cloned@없는결');
    assert.equal(sent[0].speed_factor, 1);
  } finally { restore(); }
});

test('네 결이 모두 이어져 있다 — 하나만 빠지면 그 마음만 안 들린다', async () => {
  for (const tone of Object.keys(moodSpeed)) {
    const { sent: sent, restore: restore } = intercept();
    try {
      await voice().synthesize('안녕', `cloned@${tone}`);
      assert.notEqual(sent[0].speed_factor, 1, `${tone} 이 안 이어졌다`);
    } finally { restore(); }
  }
});
