...
test('한참 있다 말하면 새 뭉치다 — 이어 말한 게 아니다', () => {
  ...
  assert.notEqual(b.heard(60 * seconds + 500), null, '그 다음 마디에는 친다');
});

test('얼마나 빨라야 이어 말한 것인지 정할 수 있다', () => {
  const narrow = agreement({ withinMs: 500 });
  narrow.heard(0);
  assert.equal(narrow.heard(900), null, '0.9초는 이어 말한 게 아니다');

  const wide = agreement({ withinMs: 5000 });
  wide.heard(0);
...
test('같은 소리를 연달아 내지 않는다', () => {
  const b = agreement();
  b.heard(0);
  const firstTurn = b.heard(900);
  b.answered();
  b.heard(2000);
  const secondTurn = b.heard(2500);
