...
const companion = (text, at = 2) => ({ role: 'said', channel: 'web', text, at });
...
test('물음과 그때 한 답을 짝지어 뽑는다', () => {
  const chosen = selfMoments([person('너 뭐 좋아해?', 1), companion('...조용한 거. 시끄러운 건 좀.', 2)]);
  assert.equal(chosen.length, 1);
  assert.equal(chosen[0].asked, '너 뭐 좋아해?');
  assert.equal(chosen[0].answered, '...조용한 거. 시끄러운 건 좀.');
});

test('호응이나 회피는 자기를 말한 게 아니다. 응으로 노트를 채우면 안 된다', () => {
  for (const answer of ['응', '...어', '음...', '그래', '글쎄...', '...모르겠어']) {
    assert.deepEqual(selfMoments([person('너 괜찮아?', 1), companion(answer, 2)]), [], `${answer} 은 자기 얘기가 아니다`);
  }
});

test('짧아도 알맹이가 있으면 센다. 길이로 재면 이 얘한테는 영영 안 쌓인다', () => {
  const chosen2 = selfMoments([person('너 뭐 좋아해?', 1), companion('소파...', 2)]);
  assert.equal(chosen2.length, 1);
  assert.equal(chosen2[0].answered, '소파...');
});

test('묻지 않은 자리의 말은 안 센다. 놀이 잡담이 쏟아진다', () => {
  const game = [person('바나나', 1), companion('석로 시작해야지. 내가 이겼다.', 2)];
  assert.deepEqual(selfMoments(game), []);
});

test('화면에서 주워 온 것은 물음이 아니다', () => {
  const es = [person('화면을 봤다. 너 앞에 있는 창은 동반자.', 1, 'screen'), companion('...또 그거네.', 2)];
  assert.deepEqual(selfMoments(es), []);
});

test('답이 아니라 다른 사람 말이 오면 안 센다', () => {
  assert.deepEqual(selfMoments([person('너 뭐 좋아해?', 1), person('아니 됐어', 2)]), []);
});
...
test('물어본 것들이 쌓인다', () => {
  ...
  s.learn([person('너 뭐 좋아해?', 1), companion('...조용한 거.', 2)]);
  s.learn([person('넌 뭐가 싫어?', 3), companion('...시끄러운 거.', 4)]);
...
test('같은 물음은 옛 답을 지킨다. 새것으로 덮으면 흔들림을 따라가는 것이다', () => {
  const s = new SelfImage();
  s.learn([person('너 뭐 좋아해?', 1), companion('...조용한 거.', 2)]);
  s.learn([person('너 뭐 좋아해?', 3), companion('...음악? 잘 모르겠는데.', 4)]);
...
test('물음표나 띄어쓰기가 달라도 같은 물음으로 본다', () => {
  const s = new SelfImage();
  s.learn([person('너 뭐 좋아해?', 1), companion('...조용한 거.', 2)]);
  s.learn([person('너 뭐 좋아해', 3), companion('...딴 말.', 4)]);
  assert.equal(s.all.length, 1);
});

test('너무 많이 들고 있지 않는다. 스무 줄이면 인격이 아니라 서류다', () => {
  const s = new SelfImage({ keep: 3 });
  for (let i = 0; i < 8; i += 1) s.learn([person(`너 ${i}번은 어때?`, i * 2), companion(`...${i}번 답이야.`, i * 2 + 1)]);
...
test('배운 게 없으면 0 을 돌려준다', () => {
  const s = new SelfImage();
  assert.equal(s.learn([person('오늘 힘들었어', 1), companion('...그랬구나.', 2)]), 0);
});

test('잘못 쌓인 건 지울 수 있다', () => {
  const s = new SelfImage();
  s.learn([person('너 뭐 좋아해?', 1), companion('...라면 좋아하는데.', 2)]);
...
test('그때 한 말을 그대로 보여 준다. 요약하면 그걸 보고 또 새로 지어낸다', () => {
  const s = new SelfImage();
  s.learn([person('너 뭐 좋아해?', 1), companion('...조용한 거. 시끄러운 건 좀.', 2)]);
  const note = s.note();
...
test('파일에 남겨 두면 껐다 켜도 이어진다', async () => {
  ...
  try {
    const first = new SelfImage({ path });
    first.learn([person('너 뭐 좋아해?', 1), companion('...조용한 거.', 2)]);
    const again = new SelfImage({ path });
    assert.equal(again.all.length, 1);
    assert.equal(again.all[0].answered, '...조용한 거.');
  } finally {
