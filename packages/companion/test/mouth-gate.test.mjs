...
test('새는 말은 잡는다', async () => {
  const gate = mouthGate({ roll: firstOne });
...
test('새면 한 번 다시 시킨다. 그게 통과하면 그걸 쓴다', async () => {
  let times = 0;
  const gate = mouthGate({
    retry: async () => { times += 1; return '응... 뭔데.'; },
  });
...
test('다시 시키는 건 딱 한 번이다. 계속 다시 시키면 대답이 늦어진다', async () => {
  let times2 = 0;
  const gate = mouthGate({
    retry: async () => { times2 += 1; return '역시 도와드리겠습니다'; },
    fallbacks: ['...'],
  });
...
test('안 할 말은 기억에도 안 남는다. 남으면 다음 번 재료가 되어 굳는다', async () => {
  ...
  const said2 = memory.all().filter((e) => e.role === 'said').map((e) => e.text);
  assert.deepEqual(said2, ['...응.']);
  assert.equal(said2.includes('무엇을 도와드릴까요'), false);
});
...
test('아쉬울 뿐인 이유로 걸린 말은 버리지 않는다. 막는 자리가 답을 더 나쁘게 만들면 안 된다', async () => {
  ...
  assert.match(written.join(' '), /원래 말이 얼버무림보다는 낫다/);
});
