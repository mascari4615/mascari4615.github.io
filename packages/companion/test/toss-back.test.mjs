...
test('안 하는 이유를 갈래마다 다르게 말한다 — 「빔」만 보이면 못 고친다', async () => {
  ...
  assert.match(skipReason({ recent: [사람('안녕'), 얘('응')], 방금: '안녕' }), /몇 마디 안 오갔다/);
  assert.match(skipReason({ recent: 식는대화, 방금: '너는?' }), /물어본 turn/);
  assert.match(skipReason({ recent: [...식는대화.slice(0, -1), 얘('어떻게 됐어?')], 방금: '응' }), /방금 되물었다/);
});

test('돌려줄 자리면 이유가 없다', async () => {
  const { skipReason } = await import('../dist/index.js');
  assert.equal(skipReason({ recent: 식는대화, 방금: '응' }), null);
});
...
test('이유에 숫자가 들어간다 — 「안 식었다」만으로는 얼마나 모자란지 모른다', async () => {
  ...
  assert.match(skipReason({ recent: 살아있는, 방금: '그래도 다음엔 괜찮아' }), /\d+자/);
});
...
test('얘 말이 하나만 있어도 센다 — 판단하는 그 순간 지금 할 말은 아직 기억에 없다', async () => {
  ...
  assert.equal(skipReason({ recent: 한마디모자란, 방금: '응' }), null);
});
