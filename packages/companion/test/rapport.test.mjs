...
const baseline = Date.UTC(2026, 0, 31);
const now = () => baseline;
...
function overDays(days, perDay = 3, endsDaysAgo = 0) {
  ...
  for (let d = 0; d < days; d += 1) {
    const at = baseline - (days - 1 - d + endsDaysAgo) * day;
...
test('며칠 만나면 조금 편해진다', () => {
  const oneDay = readRapport(overDays(1), { now });
  const tenDays = readRapport(overDays(10), { now });
  assert.ok(tenDays.level > oneDay.level);
  assert.match(oneDay.note, /서먹/);
});
...
test('하루에 몰아 떠든 것으로는 오래 본 사이가 안 된다', () => {
  const hundredPerDay = readRapport(overDays(1, 100), { now });
  const twentyDaysLight = readRapport(overDays(20, 2), { now });
...
test('한참 못 보면 식는다 — 다만 없던 일이 되진 않는다', () => {
  ...
  assert.ok(untilMonthAgo.level > 0.2, '함께 지낸 시간이 사라지진 않는다');
...
test('사이는 사람이 건넨 말로 센다 — 얘 혼잣말로 가까워지지 않는다', () => {
  ...
  const r = readRapport(monologueOnly, { now });
  assert.equal(r.days, 0);
