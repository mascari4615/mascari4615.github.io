...
test('힘들다는 말을 알아본다', () => {
  for (const t of ['오늘 진짜 힘들었어', '너무 지쳤다', '좀 우울해', '막막하네', '불안해서']) {
    assert.equal(readTender([person(t)]).soft, true, `${t}`);
  }
});

test('평범한 말은 조심할 자리가 아니다', () => {
  for (const t of ['오늘 회의 있었어', '셰이더 고쳤어', '뭐 하고 있어']) {
    assert.equal(readTender([person(t)]).soft, false, `${t}`);
  }
});
...
test('그냥 힘든 건 무겁지 않다', () => {
  assert.equal(readTender([person('오늘 진짜 힘들었어')]).heavy, false);
});
...
test('조심할 자리에서는 가벼운 재료가 빠진다', () => {
  const caution = readTender([person('오늘 진짜 힘들었어')]);
  const picked = pickIngredients([
    material('조심', tenderNote(caution), 14),
    material('놀리기', '놀릴 거리', 5, caution.soft === false),
    material('이정표', '오늘 백일', 8, caution.soft === false),
    material('기분', '지금 상태', 9),
  ]);
  const names = picked.map((x) => x.name);
  assert.equal(names.includes('놀리기'), false);
  assert.equal(names.includes('이정표'), false);
  assert.equal(names.includes('조심'), true);
});

test('평소에는 가벼운 재료가 그대로 있다', () => {
  const caution2 = readTender([person('오늘 회의 있었어')]);
  const picked2 = pickIngredients([
    material('조심', tenderNote(caution2), 14),
    material('놀리기', '놀릴 거리', 5, caution2.soft === false),
    material('기분', '지금 상태', 9),
  ]);
...
test('조심하라는 줄은 가장 무거워 예산에 안 밀린다. 밀리면 아무 소용이 없다', () => {
  ...
  const picked3 = pickIngredients(
    [
      material('긴것1', '가'.repeat(200), 9),
      material('긴것2', '나'.repeat(200), 8),
      material('조심', tenderNote(caution3), 14),
    ],
    { maxChars: 300, maxLines: 5 },
  );
  assert.equal(picked3[0].name, '조심');
});
