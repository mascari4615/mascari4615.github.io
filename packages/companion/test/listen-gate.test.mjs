...
const playBack = (gate, { level, durationMs, fromMs = 0, stepMs = 50 }) => {
  const happened = [];
  for (let t = fromMs; t < fromMs + durationMs; t += stepMs) {
    const r = gate.heard(level, t);
    if (r !== null) happened.push({ what: r, at: t });
  }
  return { happened: happened, finishedAt: fromMs + durationMs };
};

const loud = defaultValue.threshold + 0.03;
const quiet = 0.005;

test('조용하면 안 열린다', () => {
  const { happened: happened } = playBack(new listenGate(), { level: quiet, durationMs: 5000 });
  assert.deepEqual(happened, []);
});

test('말소리가 이어지면 열린다', () => {
  const { happened: happened } = playBack(new listenGate(), { level: loud, durationMs: 2000 });
  assert.equal(happened[0]?.what, '열림');
});
...
test('한 번 튄 잡음으로는 안 열린다. 키보드 소리마다 열리면 못 쓴다', () => {
  ...
  gate2.heard(loud, 0);
  gate2.heard(loud, 50);
  assert.equal(gate2.heard(quiet, 100), null);
  assert.equal(gate2.open, false);
});

test('말 중간에 숨 쉬어도 안 닫힌다', () => {
  const gate3 = new listenGate();
  playBack(gate3, { level: loud, durationMs: 600 });
  assert.equal(gate3.open, true);
  playBack(gate3, { level: quiet, durationMs: 400, fromMs: 600 }); // 닫히는데(900ms)보다 짧은 숨
  assert.equal(gate3.open, true, '400ms 숨에 끊기면 한 문장도 못 받는다');
});

test('말이 끝나고 조용하면 닫힌다', () => {
  const gate4 = new listenGate();
  playBack(gate4, { level: loud, durationMs: 600 });
  const { happened: happened } = playBack(gate4, { level: quiet, durationMs: 2000, fromMs: 600 });
  assert.equal(happened.at(-1)?.what, '닫힘');
});

test('얘가 말하는 동안은 안 열린다. 제 목소리를 제가 받아쓰면 혼자 떠든다', () => {
  const gate5 = new listenGate();
  gate5.mouth(true, 0);
  const { happened: happened } = playBack(gate5, { level: loud, durationMs: 3000 });
  assert.deepEqual(happened, []);
});

test('얘가 말하기 시작하면 열려 있던 문도 닫는다', () => {
  const gate6 = new listenGate();
  playBack(gate6, { level: loud, durationMs: 600 });
  assert.equal(gate6.open, true);
  gate6.mouth(true, 600);
  assert.equal(gate6.heard(loud, 650), '닫힘');
});

test('얘가 말을 마쳐도 꼬리 여운 동안은 안 연다. 스피커 울림이 남는다', () => {
  const gate7 = new listenGate();
  gate7.mouth(true, 0);
  gate7.mouth(false, 1000);
  playBack(gate7, { level: loud, durationMs: 500, fromMs: 1000 }); // 꼬리여운(600ms) 안
  assert.equal(gate7.open, false);
  playBack(gate7, { level: loud, durationMs: 600, fromMs: 1700 }); // 여운 지난 뒤
  assert.equal(gate7.open, true);
});

test('켜 둔 TV 에 영영 매달리지 않는다. 너무 길면 그냥 닫는다', () => {
  const gate8 = new listenGate();
  const { happened: happened } = playBack(gate8, { level: loud, durationMs: 25000, step: 100 });
  assert.equal(happened.filter((x) => x.what === '닫힘').length >= 1, true);
});

test('설정을 바꾸면 그대로 먹는다. 방마다 시끄러운 정도가 다르다', () => {
  const gate9 = new listenGate({ threshold: 0.5 });
  playBack(gate9, { level: 0.2, durationMs: 3000 });
  assert.equal(gate9.open, false);
});
...
test('왜 안 넘겼는지 말할 수 있다. 조용히 버리면 왜 대답을 안 하지가 된다', () => {
...
test('말소리가 거의 없던 구간은 무슨 글이 와도 안 넘긴다. 조용한 3초에 안녕하세요가 나왔다', () => {
...
test('메아리만 들어올 때는 안 끊는다. 안 그러면 한 마디도 못 끝내고 자기를 끊는다', () => {
  ...
  gate11.mouth(true, 0);
  const happened3 = speakAndPlay(gate11, 0.05, 5000); // 스피커 소리가 마이크로 돌아온 level
...
test('메아리 바닥을 재서 배운다. 방마다 스피커마다 다르니 정해 두면 어디선가 틀린다', () => {
  ...
  assert.equal(gate14.echoFloor, 0);
  gate14.mouth(true, 0);
  speakAndPlay(gate14, 0.06, 1000);
  assert.ok(gate14.echoFloor >= 0.05, `메아리를 배워야 한다. 지금 ${gate14.echoFloor}`);
  assert.ok(gate14.bargeThreshold > 0.06, '배운 메아리보다 높아야 사람 목소리로 본다');
});
...
test('메아리가 너무 크면 끼어들기를 포기한다. 켜면 자기를 끊는 얘가 된다', () => {
  ...
  assert.equal(gate15.bargeGiveUpMs, true);
...
test('얘가 말하는 durationMs 받아쓰기 문이 열리지는 않는다. 끊는 것과 받아쓰는 것은 다르다', () => {
  const gate17 = new listenGate();
  gate17.mouth(true, 0);
  speakAndPlay(gate17, 0.5, 3000);
  assert.equal(gate17.open, false);
});
...
test('사람이 끼어든 소리를 메아리로 배우지 않는다. 배우면 문턱이 같이 뛰어 영영 안 걸린다', () => {
  ...
  const learned = gate18.echoFloor;
  speakAndPlay(gate18, 0.4, 600, 2000); // 사람이 끼어든 구간
  assert.ok(gate18.echoFloor < learned * 3, `잠깐 솟은 걸 메아리로 배우면 안 된다. ${learned} → ${gate18.echoFloor}`);
  assert.equal(gate18.bargeGiveUpMs, false, '한 번 끼어들었다고 포기하면 안 된다');
});
...
test('메아리가 내내 큰 방에서는 사람 목소리도 그만큼 커야 한다. 안 그러면 자기를 끊는다', () => {
  ...
  assert.ok(gate19.bargeThreshold > 0.3, `메아리 0.3 인 방에서 문턱이 ${gate19.bargeThreshold} 이면 제 소리에 끊긴다`);
});
...
test('한 번 끼어들었다고 끼어들기가 꺼지지 않는다. 사람 목소리가 바닥을 끌어올리면 안 된다', () => {
  ...
  assert.equal(gate20.bargeGiveUpMs, false, `바닥이 ${gate20.echoFloor} 까지 끌려 올라가면 다음부터 못 끊는다`);
  // 다시 끊을 수 있어야 한다
  gate20.mouth(false, 3800); gate20.mouth(true, 6000);
...
test('진짜 시끄러운 방이면 바닥이 곧 차올라 포기한다. 억지로 켜면 자기를 끊는다', () => {
  ...
  assert.equal(gate21.bargeGiveUpMs, true, `바닥 ${gate21.echoFloor}. 이 방에서는 켜면 안 된다`);
});
