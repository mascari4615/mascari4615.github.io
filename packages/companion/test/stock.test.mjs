...
test('한가할 때 채워 두면 그걸 꺼내 쓴다', async () => {
  const store = new 대사창고({ fetchBuilt: async () => '…또야?\n…손 치워.\n…그만해.' });
...
test('꺼낸 것은 없어진다 — 담아 둔 걸 다시 쓰면 또 도는 말이 된다', async () => {
  const store2 = new 대사창고({ fetchBuilt: async () => '하나\n둘' });
...
test('껐다 켜도 담아 둔 게 남는다', async () => {
  const path = newFile();
  const firstRun = new 대사창고({ path, fetchBuilt: async () => '하나\n둘' });
...
test('인격이 바뀌면 앞 인격이 지은 말은 안 쓴다', async () => {
  let who = '욘';
  const store6 = new 대사창고({ whom: () => who, fetchBuilt: async () => '…또야?' });
...
test('깨진 파일은 없는 셈 친다 — 그것 때문에 못 뜨면 안 된다', () => {
  ...
  const store7 = new 대사창고({ path, fetchBuilt: async () => null });
  assert.equal(store7.remaining('갈래'), 0);
});

test('닿음 대꾸가 창고 것을 먼저 쓰고, 비면 손으로 적은 표로 물러선다', async () => {
  clearReplyMemory();
  const store8 = new 대사창고({ fetchBuilt: async () => '…또야?' });
...
test('반사도 미리 지어 둔 것을 먼저 쓰고, 비면 손으로 적은 표로 물러선다', async () => {
  const store10 = new 대사창고({ fetchBuilt: async () => '어, 왔어' });
  await store10.fill(반사갈래('인사', '보통'), '인사 받는 말');
...
test('결이 다르면 그 자리 것을 안 꺼낸다 — 늘어진 애가 생생한 말을 하면 안 된다', async () => {
  const store11 = new 대사창고({ fetchBuilt: async () => '…어… 왔네' });
  await store11.fill(반사갈래('인사', '처짐'), '나른할 때 인사');
...
test('반사 아닌 말은 창고가 있어도 반사하지 않는다', async () => {
  const store12 = new 대사창고({ fetchBuilt: async () => '아무 말' });
  await store12.fill(반사갈래('인사', '보통'), '인사');
