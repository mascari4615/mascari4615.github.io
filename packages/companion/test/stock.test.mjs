...
import { lineStore, select, touchReply, touchKind, clearReplyMemory, reflexFor, reflexKind } from '../dist/index.js';

const newFile = () => join(mkdtempSync(join(tmpdir(), 'stock-')), '지은-대사.json');

test('한가할 때 채워 두면 그걸 꺼내 쓴다', async () => {
  const store = new lineStore({ fetchBuilt: async () => '…또야?\n…손 치워.\n…그만해.' });
  assert.equal(store.remaining('touch:쿡:1'), 0);
  const storedCount = await store.fill('touch:쿡:1', '쿡 찔렸을 때 대꾸');
  assert.equal(storedCount, 3);
  assert.equal(store.raise('touch:쿡:1'), '…또야?');
});

test('꺼낸 것은 없어진다 — 담아 둔 걸 다시 쓰면 또 도는 말이 된다', async () => {
  const store2 = new lineStore({ fetchBuilt: async () => '하나\n둘' });
  await store2.fill('갈래', '아무거나');
...
test('지어 온 것 중 설명·존댓말·긴 말은 안 담는다', async () => {
  ...
  const storedCount2 = await store3.fill('갈래', '아무거나');
...
test('쓸 게 하나도 안 남으면 아무것도 안 담는다', async () => {
  const store4 = new lineStore({ fetchBuilt: async () => '무엇을 도와드릴까요?\n필요하신 게 있으면 말씀해 주세요' });
  assert.equal(await store4.fill('갈래', '아무거나'), 0);
  assert.equal(store4.raise('갈래'), null);
});

test('두뇌가 못 지어도(널·예외) 얘는 안 죽는다', async () => {
  const nullValue = new lineStore({ fetchBuilt: async () => null });
  assert.equal(await nullValue.fill('갈래', '아무거나'), 0);
  const crash = new lineStore({
    fetchBuilt: async () => {
      throw new Error('두뇌가 없다');
    },
  });
  assert.equal(await crash.fill('갈래', '아무거나'), 0);
});

test('넉넉하면 두뇌를 안 부른다', async () => {
  let callCount = 0;
  const store5 = new lineStore({
    fetchBuilt: async () => {
      callCount += 1;
      return '하나\n둘';
    },
  });
  await store5.fill('갈래', '아무거나', 2);
  await store5.fill('갈래', '아무거나', 2);
...
test('껐다 켜도 담아 둔 게 남는다', async () => {
  const path = newFile();
  const firstRun = new lineStore({ path, fetchBuilt: async () => '하나\n둘' });
  await firstRun.fill('갈래', '아무거나');
  const nextRun = new lineStore({ path, fetchBuilt: async () => '안 불러야 한다' });
...
test('인격이 바뀌면 앞 인격이 지은 말은 안 쓴다', async () => {
  let who = '욘';
  const store6 = new lineStore({ whom: () => who, fetchBuilt: async () => '…또야?' });
  await store6.fill('갈래', '아무거나');
  who = '무명';
...
test('깨진 파일은 없는 셈 친다 — 그것 때문에 못 뜨면 안 된다', () => {
  ...
  const store7 = new lineStore({ path, fetchBuilt: async () => null });
  assert.equal(store7.remaining('갈래'), 0);
});

test('닿음 대꾸가 창고 것을 먼저 쓰고, 비면 손으로 적은 표로 물러선다', async () => {
  clearReplyMemory();
  const store8 = new lineStore({ fetchBuilt: async () => '…또야?' });
  await store8.fill(touchKind('쿡', 0), '쿡 찔렸을 때');
  assert.equal(touchReply('쿡', { times: 1, store: store8 }), '…또야?');
  // 비었으니 이제 기본 표. (기본 표에 있는 말이어야 한다)
  const next = touchReply('쿡', { times: 1, store: store8 });
...
test('골라내기는 겹치는 말을 한 번만 남긴다', () => {
  assert.deepEqual(select('…또야?\n…또야?\n…그만.'), ['…또야?', '…그만.']);
});

test('두뇌가 되물으며 낸 말은 대꾸로 안 담는다 (89회차 실측 — 창고에 실제로 담겼던 것들)', async () => {
  const store9 = new lineStore({
    fetchBuilt: async () => ['소설/스토리 캐릭터의 대사인가요?', '게임 대사 작성인가요?', '다른 프로젝트인가요?'].join('\n'),
  });
  assert.equal(await store9.fill('갈래', '아무거나'), 0);
});
...
test('인격을 주면 그 글이 부탁에 실린다 — 안 실으면 맨 두뇌가 짓는다 (89회차 실측)', async () => {
  let receivedRequest = '';
  const store13 = new lineStore({
    personaText: () => '너는 욘. 늘 나른하고 반말만 쓴다.',
    fetchBuilt: async (prompt) => {
      receivedRequest = prompt;
      return '어, 왔어';
    },
  });
  await store13.fill('갈래', '인사 받는 말');
