...
test('표가 아는 창은 안 물어본다 — 물어보는 값이 헛되이 든다', () => {
  const 배움 = new learnSlot({ 물어보기: async () => null });
...
test('물어보기가 없으면 담지도 않는다 — 아무도 안 볼 목록을 키우지 않는다', () => {
  const 배움 = new learnSlot();
  배움.읽기('NVIDIA GeForce Overlay');
  assert.equal(배움.밀린것, 0);
});

test('한 번 배우면 그 뒤로는 표처럼 쓴다', async () => {
  const 배움 = new learnSlot({ 물어보기: async (ts) => ts.map(() => '노는중') });
  배움.읽기('Overwatch');
...
test('모른다고 답한 것도 적어 둔다 — 안 적으면 같은 창을 영원히 다시 묻는다', async () => {
  let 부른수 = 0;
  const 배움 = new learnSlot({ 물어보기: async (ts) => { 부른수 += 1; return ts.map(() => null); } });
  배움.읽기('Windows 입력 환경');
  await 배움.되새기기();
  배움.읽기('Windows 입력 환경');
  await 배움.되새기기();
...
test('표가 두뇌를 이긴다 — 우리 창은 우리가 안다', async () => {
  const 배움 = new learnSlot({ 물어보기: async (ts) => ts.map(() => '통화') });
  await 배움.되새기기();
...
test('개수가 안 맞는 대답은 통째로 버린다 — 어긋나면 엉뚱한 창이 「통화」가 되어 입을 닫는다', async () => {
  const 적힌것 = [];
  const 배움 = new learnSlot({ 물어보기: async () => ['노는중'], log: (m) => 적힌것.push(m) });
...
test('두뇌가 죽어도 상황 파악이 멈추지 않는다 — 그리고 조용히 삼키지 않는다', async () => {
  const 적힌것 = [];
  const 배움 = new learnSlot({ 물어보기: async () => { throw new Error('두뇌 없음'); }, log: (m) => 적힌것.push(m) });
  배움.읽기('설정');
  assert.equal(await 배움.되새기기(), 0);
...
test('물음에 창 제목이 다 들어간다', async () => {
  let 본것 = '';
  const 묻기 = askSlot(async (p) => { 본것 = p; return '모름\n모름\n모름'; });
