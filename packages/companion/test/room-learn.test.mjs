...
test('표가 아는 창은 안 물어본다 — 물어보는 값이 헛되이 든다', () => {
  const learning = new learnSlot({ 물어보기: async () => null });
...
test('표가 모르는 창은 물어볼 것으로 담는다 — 모른다고 버리면 영영 모른다', () => {
  const learning2 = new learnSlot({ 물어보기: async () => null });
...
test('한 번 배우면 그 뒤로는 표처럼 쓴다', async () => {
  const learning4 = new learnSlot({ 물어보기: async (ts) => ts.map(() => '노는중') });
  learning4.읽기('Overwatch');
  assert.equal(await learning4.되새기기(), 1);
...
test('모른다고 답한 것도 적어 둔다 — 안 적으면 같은 창을 영원히 다시 묻는다', async () => {
  let callCount = 0;
  const learning5 = new learnSlot({ 물어보기: async (ts) => { callCount += 1; return ts.map(() => null); } });
  learning5.읽기('Windows 입력 환경');
  await learning5.되새기기();
  learning5.읽기('Windows 입력 환경');
  await learning5.되새기기();
...
test('표가 두뇌를 이긴다 — 우리 창은 우리가 안다', async () => {
  const learning6 = new learnSlot({ 물어보기: async (ts) => ts.map(() => '통화') });
  await learning6.되새기기();
  assert.equal(learning6.읽기('동반자'), '나를보는중');
});

test('개수가 안 맞는 대답은 통째로 버린다 — 어긋나면 엉뚱한 창이 「통화」가 되어 입을 닫는다', async () => {
  const written = [];
  const learning7 = new learnSlot({ 물어보기: async () => ['노는중'], log: (m) => written.push(m) });
  for (const t of unknownWindow) learning7.읽기(t);
  assert.equal(await learning7.되새기기(), 0);
...
test('두뇌가 죽어도 상황 파악이 멈추지 않는다 — 그리고 조용히 삼키지 않는다', async () => {
  const written2 = [];
  const learning8 = new learnSlot({ 물어보기: async () => { throw new Error('두뇌 없음'); }, log: (m) => written2.push(m) });
  learning8.읽기('설정');
  assert.equal(await learning8.되새기기(), 0);
