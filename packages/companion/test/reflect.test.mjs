...
test('짚은 것을 근거와 함께 담는다', async () => {
  const r = new reflection({ 물어보기: async () => [짚은것('조수님은 막힌 얘기를 밤에만 꺼낸다')] });
...
test('근거를 못 대면 버린다 — 되새김은 헛것이 가장 잘 나오는 자리다', async () => {
  const 적힌것 = [];
  const r = new reflection({ 물어보기: async () => [{ 무엇: '조수님은 사실 고양이를 싫어한다', 근거: [] }], log: (m) => 적힌것.push(m) });
...
test('같은 걸 또 짚지 않는다 — 글자가 달라도 같은 얘기면 안 담는다', async () => {
  const r = new reflection({ 물어보기: async () => [짚은것('조수님은 셰이더에 자꾸 막힌다')] });
  await r.되새기기(오간말);
  const r2 = new reflection({ 물어보기: async () => [짚은것('조수님은 셰이더에 계속 막힌다')] });
  r2.all.push(...r.all);
...
test('두뇌가 죽어도 대화는 안 멈춘다 — 그리고 조용히 삼키지 않는다', async () => {
  const 적힌것 = [];
  const r = new reflection({ 물어보기: async () => { throw new Error('두뇌 없음'); }, log: (m) => 적힌것.push(m) });
  assert.equal(await r.되새기기(오간말), 0);
...
test('지금 얘기와 이어질 때만 얹는다 — 늘 붙이면 사람을 계속 분석하는 꼴이다', async () => {
  const r = new reflection({ 물어보기: async () => [짚은것('조수님은 셰이더 얘기를 밤에만 꺼낸다')] });
  await r.되새기기(오간말);
