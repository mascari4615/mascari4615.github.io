...
- 자료 모양을 모른다: `{ id, hash, text }` 만 받고 **번호로** 답한다 (글이든 상품이든 같음)
- **곳간(cache)은 부른 쪽이 소유**함. 이 꾸러미는 읽고 채우기만, 파일은 안 만듦
- 곳간 열쇠에 **모델 이름**이 들어감. 모델을 갈면 옛 벡터를 안 집음
- **재는 연장은 부른 쪽이 건넨다** (`loadRunner`). 벤더를 이름에도 의존에도 안 넣음.
...
- **E5 계열은 앞말(`passage:`) 필수**. 안 붙이면 순서가 뒤집힘.
  e5-small 로 갈아타려다 되돌린 판: 닮은 글 15.1→63.7배(좋아짐)이나 정직도 0.886→0.735(나빠짐),
  사전 문턱 자 넷 중 셋을 못 넘었다
- **쏠림 빼기는 한 번으로 0 이 안 된다**. 뺀 뒤 길이를 1 로 다시 맞추면서 새 평균이 생김
  (0.7181 → 0.0708 → 0.0086). 한 번만 뺀다
- 벡터는 소수점 여섯 자리로 자름. 곳간이 세 배 작아지고 그 자리에서 뜻은 안 달라짐
