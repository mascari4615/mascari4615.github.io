...
function normalizeProviderName(raw) {
    ...
    if (s === 'claude-cli' || s === 'claude')
        return 'claude-cli';
    if (s === 'codex-cli' || s === 'codex')
        return 'codex-cli';
    if (s === 'openai' || s === 'gpt')
        return 'openai';
    if (s === 'ollama' || s === 'local')
        return 'ollama';
    if (s === 'openrouter' || s === 'or')
        return 'openrouter';
    if (s === 'gemini' || s === 'google' || s === 'vertex' || s === 'aistudio')
        return 'gemini';
    return null;
}
 ...
 */
function resolveProviderChain(env, primary) {
    ...
    const parts = raw
        ...
        .filter((p) => p != null && ALL_PROVIDERS.has(p));
...
// ─── per-provider 단일 호출 ───────────────────────────────────────────────
async function callOne(provider, env, prompt, opts) {
    if (provider === 'claude-cli') {
        const cwd = env.ASSISTANT_AGENT_REPO_PATH?.trim() || undefined;
        return (0, cli_claude_1.generateClaudeCliText)({ prompt, timeoutMs: opts.timeoutMs, cwd });
    }
    if (provider === 'codex-cli') {
        const cwd = env.ASSISTANT_AGENT_REPO_PATH?.trim() || undefined;
        return (0, cli_codex_1.generateCodexCliText)({ prompt, timeoutMs: opts.timeoutMs, cwd });
    }
    if (provider === 'openai') {
        const config = (0, openai_compat_1.openAiConfigFromEnv)(env);
        if (!config)
            throw new Error('openai: OPENAI_API_KEY 가 .env 에 필요합니다.');
        const messages = (0, openai_compat_1.geminiHistoryToOpenAiMessages)(opts.history, opts.systemInstruction, prompt);
        const { text } = await (0, openai_compat_1.generateOpenAiCompatText)({ config, messages });
        return text;
    }
    if (provider === 'openrouter') {
        const config = (0, openai_compat_1.openRouterConfigFromEnv)(env);
        if (!config)
            throw new Error('openrouter: OPENROUTER_API_KEY 가 .env 에 필요합니다.');
        const messages = (0, openai_compat_1.geminiHistoryToOpenAiMessages)(opts.history, opts.systemInstruction, prompt);
        const { text } = await (0, openai_compat_1.generateOpenAiCompatText)({ config, messages });
        return text;
    }
    if (provider === 'ollama') {
        const config = (0, ollama_1.ollamaConfigFromEnv)(env);
        if (!config)
            throw new Error('ollama: OLLAMA_BASE_URL 또는 OLLAMA_MODEL 이 .env 에 필요합니다.');
        const messages = (0, openai_compat_1.geminiHistoryToOpenAiMessages)(opts.history, opts.systemInstruction, prompt);
        const { text } = await (0, ollama_1.generateOllamaText)({ config, messages });
        return text;
    }
    ...
    if (opts.systemInstruction || (opts.history && opts.history.length > 0)) {
        ...
        const modelId = (0, text_1.resolveGeminiModelId)({ tier: opts.tier, env });
        const t0 = Date.now();
        if (surface === 'vertex') {
            ...
            const r = await (0, text_1.generateVertexText)({
                apiKey,
                projectId,
        ...
        const r = await (0, text_1.generateAiStudioChatText)({
            apiKey,
            modelId,
            systemInstruction: opts.systemInstruction,
            history: opts.history ?? [],
    ...
    const { text } = await (0, text_1.generateBlobTextFromEnvWithOptions)(env, prompt, {
        surface: 'inherit',
        tier: opts.tier,
        tag: opts.tag,
