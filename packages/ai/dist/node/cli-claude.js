        ...
        return new Promise((resolve, reject) => {
            ...
            child.on('close', (code) => {
                ...
                if (code === 0 && (stdout.trim() || opts.cwd)) {
                    ...
                    const err = new Error(`Claude CLI 종료 코드 ${code} (cwd=${opts.cwd ?? '<none>'}): stderr=${stderrSnip} | stdout=${stdoutSnip}`);
                    err.exitCode = code;
    ...
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            ...
            if (code === 0)
                ...
                const stderrSnip = (stderr || '').trim().slice(0, 1200) || '<stderr empty>';
                const stdoutSnip = (stdout || '').trim().slice(0, 600) || '<stdout empty>';
                reject(new Error(`Claude CLI(discovery) 종료 코드 ${code}: stderr=${stderrSnip} | stdout=${stdoutSnip}`));
            }
        });
        child.on('error', (err) => {
            clearTimeout(timer);
