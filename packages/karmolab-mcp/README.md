...
**Zero dependencies.** 50 tools. Everything runs locally; nothing is sent anywhere.
...
| **Daily games** | `daily_today` · `dailytype_today` | Puzzle date and seed for a daily game, fixed to KST — device timezone cannot split players onto different puzzles · today's Korean typing challenge, scored in 타/분 with the sentences drawn from this site's own tool descriptions |
...
**LLM 이 조용히 틀리는 것들을 대신 계산하는 MCP 서버.** 의존성 0개, 도구 50개, 전부 로컬에서 돈다.
