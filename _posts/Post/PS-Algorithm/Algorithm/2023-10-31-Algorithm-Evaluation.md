...
- O(log n) 로그 시간 Logarithmic Time : 이진탐색  
- O(n) 선형 시간 Linear Time (데이터 크기와 실행 횟수가 똑같은) : 단순탐색  
- O(n log n) : 퀵 같이 빠른 정렬 알고리듬
- O(n<sup>2</sup>) : 선택 정렬과 같이 느린 정렬 알고리듬
- O(n!) : 외판원(세일즈맨) 문제와 같이 정말 느린 알고리듬
