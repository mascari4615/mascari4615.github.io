...
/** 열쇠 재료와 색인과 휴지통. 파일이 늘 때마다 바뀐다 */
const FIXED = new Set(['hdr', 'idx', 'trash', 'trash.bak']);

/**
 * 화면이 **써도 되는** 키. 하나뿐이다.
 *
 * 왜 하나뿐인가: 색인이나 열쇠 재료를 화면에서 쓸 수 있게 하면, 세션 하나가 털렸을 때
 * 저장소 전체가 열리지 않는 상태로 갈 수 있다. 휴지통은 표시일 뿐이라
 * 최악이라야 파일이 목록에서 사라지고, 그건 되살리면 그만이다.
 */
export function writableKey(key) {
    return key === 'trash';
}
