...
**폰은 눕혀서 잡는다.** 세로로 들면 16:9 가 가운데 띠만 되므로 「돌려서 잡아라」 안내가
뜬다. 다만 **2초 넘게 세로로 머물 때만** 뜬다 — 카메라 워킹 중 롤이 세로를 스쳐 지나가는
건 의도된 동작이라 그때 안내가 튀면 방해만 된다. 눕히면 즉시 사라진다.

화면을 90° 돌려 채우는 길도 있었지만 안 했다. 그러면 폰을 오른쪽으로 밀 때 그림이 위로
가는 **거짓 뷰파인더**가 된다 (카메라 롤까지 같이 돌리는 「세로 그립」은 별개 모드다).
