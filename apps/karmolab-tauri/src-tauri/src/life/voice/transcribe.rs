...
use super::super::state::LifeScreenConfig;
...
fn cache_dir() -> Result<PathBuf, String> {
    let memo_root = LifeScreenConfig::resolve()
        .map(|c| c.memo_repo_root)
        .unwrap_or_else(|_| PathBuf::from("."));
    let dir = memo_root
        .join("life")
        .join(".models")
        .join("whisper-large-v3");
    std::fs::create_dir_all(&dir).map_err(|e| format!("model cache 디렉토리 생성 실패: {e}"))?;
    Ok(dir)
}

fn ensure_model_files() -> Result<ModelFiles, String> {
    let cache = cache_dir()?;
...
impl Decoder {
    ...
    fn run_mel(&mut self, mel: &Tensor) -> Result<Vec<LocalSegment>, String> {
...
pub fn transcribe(samples_16k: &[f32]) -> Result<TranscribeResult, String> {
    ...
    let local_segments = dec.run_mel(&mel)?;
    let mut full_text = String::new();
    let mut segments = Vec::with_capacity(local_segments.len());
    for ls in local_segments {
        full_text.push_str(ls.dr.text.trim());
        full_text.push(' ');
        segments.push(Segment {
            text: ls.dr.text.trim().to_string(),
            start_s: ls.start as f32,
            end_s: (ls.start + ls.duration) as f32,
        });
    }
    Ok(TranscribeResult {
