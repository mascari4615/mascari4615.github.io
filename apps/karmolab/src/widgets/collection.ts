 ...
 * 도구도 화면에는 똑같이 생겼다. 그래서 160개가 필요할 때 찾는 목록이지 내 것이 되지 않는다.
 ...
 *
 * 몇 번 썼나는 안 센다. 도감은 수집이지 성적표가 아니다. 횟수를 보여 주는 순간 적게 쓴
 * 칸이 부끄러운 칸이 된다.
 */
...
.cl-sec.is-on { border-style: solid; border-color: var(--accent); color: var(--accent); font-family: inherit; }`;
                ...
                build: function (container: HTMLElement): void {
                  void loadNamespace('collection').then(function () {
                    ...
                    const paint = (stamped: Set<string>, signedIn: boolean): void => {
                        ...
                        /* 로그인 안 한 사람에게 이 브라우저에만 남습니다를 말해 준다.
                    ...
                    /* 앱은 한 번 그린 화면을 그대로 다시 보여 준다. 도구를 쓰고 도감으로
                       ...
                       도감은 방금 쓴 것이 찍혔나를 보러 오는 화면이라 이게 곧 고장이다.
    ...
    /* 이 화면 자체도 도구다. 열었으면 도장이 찍힌다(예외를 두면 왜 이건 안 찍히지가 된다). */
