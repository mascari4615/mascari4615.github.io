...
export function waveformSvg(path: string, label: string, className = ''): string {
  return `<svg class="hu-wave-svg ${className}" viewBox="0 0 100 32" preserveAspectRatio="none" aria-label="${label} 파형"><path d="${path}"></path></svg>`;
}
...
export function clipHtml(input: ClipViewInput): string {
  ...
  if (clip.notes.length) {
    ...
    body = `<div class="hu-midi-notes">${preview.map((note) => `<i class="hu-midi-note" style="left:${note.beat / clip.duration * 100}%;width:${Math.max(1, note.duration / clip.duration * 100)}%;bottom:${(note.pitch - low) / (high - low) * 90}%"></i>`).join('')}</div>`;
  } else {
    body = audioBody();
  }
  return `<div class="hu-clip${selected ? ' is-selected' : ''}${clip.mute ? ' is-muted' : ''}${clip.locked ? ' is-locked' : ''}" data-clip="${clip.id}" data-track="${track.id}" style="--clip:${clip.color || track.color};left:${clip.start * pxPerBeat}px;width:${clip.duration * pxPerBeat}px"><div class="hu-clip-name">${esc(clip.name)}</div>${body}<div class="hu-handle" data-resize="1"></div></div>`;
}
