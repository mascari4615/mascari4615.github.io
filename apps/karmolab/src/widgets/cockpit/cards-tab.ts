...
export function buildCardsTab(container: HTMLElement): void {
  container.innerHTML = `
    <div style="display:flex;flex-direction:column;gap:.75rem;padding:1rem;font-size:.9rem;height:100%;overflow-y:auto;box-sizing:border-box">
      <header style="display:flex;align-items:center;gap:.75rem;flex-wrap:wrap">
        <h2 style="margin:0;font-size:1.05rem">${esc(t('cockpit.t57'))}</h2>
        <span class="ac-meta" style="opacity:.6;font-size:.78rem;flex:1"></span>
        <input class="ac-search" type="search" placeholder="${esc(t('cockpit.t56'))}" style="padding:.25rem .5rem;min-width:10rem" />
        <select class="ac-filter" style="padding:.25rem .4rem">
          <option value="all">${esc(t('cockpit.opt.all'))}</option>
          <option value="decision">${esc(t('cockpit.opt.decision'))}</option>
          <option value="fix">${esc(t('cockpit.opt.fix'))}</option>
          <option value="finding">${esc(t('cockpit.opt.finding'))}</option>
          <option value="incident">${esc(t('cockpit.opt.incident'))}</option>
          <option value="note">${esc(t('cockpit.opt.note'))}</option>
        </select>
        <button class="ac-refresh" type="button" style="padding:.3rem .7rem">${esc(t('cockpit.t58'))}</button>
      </header>
  ...
  function render(): void {
    ...
    meta.textContent =
      sq || filter !== 'all'
        ? t('cockpit.cards.filtered', { n: rows.length, total: allCards.length })
        : t('cockpit.cards.recent', { n: allCards.length });
    if (rows.length === 0) { list.innerHTML = t('cockpit.t59'); return; }
  ...
  async function load(): Promise<void> {
    ...
    try {
      ...
      errBox.style.display = 'block';
      errBox.textContent = t('cockpit.cards.loadFailed', { why: String(e) });
      meta.textContent = '';
    }
  }
