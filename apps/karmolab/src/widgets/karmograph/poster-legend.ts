 ...
 *
 * 지금 「그림으로 저장」은 판을 **있는 그대로** 오려 낸다. 그 그림을 받은 사람은
 * 「분홍 테두리는 뭐고 노란 점은 뭔가」를 물을 수밖에 없다 — 종류와 관계의 뜻이 그림 밖
 ...
 *
 * 규칙:
 *  - **판에 실제로 쓰인 것만.** 안 쓴 종류를 범례에 적으면 「이 그림에 없는 것」을 설명하게 된다.
 *  - **많이 쓰인 순.** 같은 수면 이름순 — 같은 판을 두 번 뽑으면 같은 그림이 나와야 한다.
 *  - **상한이 있다.** 스무 가지가 늘어서면 범례가 그림을 잡아먹는다. 넘치면 「그 밖 N가지」로 접는다.
 */
...
export interface LegendItem {
  kind: string;
  label: string;
  /** 이 종류가 판에 몇 개 있나 — 순서의 근거이자, 「인물 12」처럼 세어 보여 줄 때 쓴다. */
  count: number;
...
/**
 * 범례가 쓸모 있나. 종류가 **한 가지뿐**이면 설명할 것이 없다 — 「전부 인물입니다」는 그림을 보면 안다.
