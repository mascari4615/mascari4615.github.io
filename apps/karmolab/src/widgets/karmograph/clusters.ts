 ...
 *
 * 관계망 칸은 이미 「끊긴 덩어리」를 센다. 그런데 사람이 진짜 궁금한 건 **다 이어져 있는 판 안에서**
 * 「누가 누구랑 한 패인가」다 — 학교 무리와 가족 무리가 한 사람으로만 이어져 있어도, 눈으로는
 ...
 *   그러면 「어제 본 무리」와 「오늘 본 무리」가 달라져 아무도 안 믿는다. 그래서 순서를 못 박는다:
...
export function findClusters(
    ...
    for (const id of ids) {
      ...
      for (const [l, n] of votes) {
        ...
        /* 표가 많으면 이긴다. **표가 같으면 함께 아는 카드가 많은 쪽**이 이긴다 —
           이 한 줄이 「다리 하나로 붙은 두 패」를 안 뭉치게 한다(다리 건너편과는 아는 사람이 없다).
  ...
  // 큰 무리부터. 크기가 같으면 첫 이름표 순 — 순서까지 못 박아야 「같은 답」이다.
...
/**
 * 무리 이야기를 **할 만한가**.
 * 무리가 하나면 「다 한 패입니다」라 할 말이 없고, 카드마다 제 무리면(전부 혼자) 그것도 할 말이 아니다
 * — 그건 「아무도 안 이어져 있다」이고 관계망 칸이 이미 말한다.
 */
