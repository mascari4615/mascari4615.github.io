 ...
 *
 * 세계관을 짓는 일은 다 적었다로 끝나지 않는다. 카드마다 **익는 단계**가 있다:
 * 이름만 있는 씨앗, 반쯤 적은 것, 더 적을 게 없는 것. Heptabase 가 파는 것이 이 감각이다
 * (생각이 익는 단계를 도구가 안다).
 *
 * 새 자료를 만들지 않는다. **이미 있는 칸**에서 읽어 낸다. 사람이 따로 익음을 고르게 하면
 * 그 값도 관리해야 할 또 하나의 칸이 되고, 결국 아무도 안 고친다.
 *
 * 관계망 칸의 아직 안 적은 칸이 **판 전체**를 말한다면, 이건 **이 카드 한 장**이다.
 */
...
/** 공백만 적은 것은 **안 적은 것**이다. 안 적음과 띄어쓰기 한 칸을 다르게 세면 아무 말도 못 한다. */
...
/**
 * 이 카드에 대해 **말을 걸 만한가**.
 * 다 적은 카드에 다 적었어요라고 하는 건 잔소리다. 남은 것이 있을 때만 말한다.
 */
