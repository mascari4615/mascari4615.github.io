...
(function (): void {
  ...
  const KARMOGRAPH_CSS = `
    ...
    /* ★ 단추의 hidden 클래스만은 **한 줄로** 못 박는다. 없던 동안 접었다고 표시한 단추가 그대로
       보였다. 툴바의 ↑ 위로가 68×30 으로 살아 있었고(접기의 뜻이 사라진다), 서랍, 팔레트가
    ...
    /* ★ 탭은 **전부 이름을 단다** (2026-08-12 사용자 검토: 🔗 💾 🔍 만 보고는 공유, 내보내기가
       ...
       두 줄로 접는다: 옆 패널의 세로 12px 이 무엇인지 모르는 단추 아홉 개보다 싸다. */
    ...
    /* ★ 옆 패널 버튼은 **글자를 안 접는다**. 줄 안에서 버튼이 마지막이라 flex 가 여기부터 깎았고,
       그 결과 추가가 추 / 가로, 남기기가 남기 / 기로 세로로 쪼개져 찍혔다
    ...
    @media (max-width: 720px) {
      ...
      /* ★ 서랍은 폰에서 **아래 시트보다 위**여야 한다 (실측 2026-08-14: z-index 20 이라 시트(960)가
         ...
         서랍은 폰에서 접은 것들에 닿는 유일한 문인데 그 문의 절반이 죽어 있었던 셈이다).
    ...
    /* 안내는 **위쪽에** 앉힌다. 한가운데 두면 빈 곳 두 번 클릭이 카드에 먹혀
    ...
    /* 눈에는 안 보이고 **읽어 주는 도구에만** 들리는 자리 (KL-271). 고르는 일은 초점이 안
       움직여서, 여기 한 줄을 흘려 주지 않으면 무엇을 골랐는지가 아무 데도 안 남는다. */
    ...
    /* ★ 발표 줄은 **판 위에 떠야 한다.** z-index 를 안 주면(auto) 캔버스 svg 가 그 위에 깔려
       한 장으로 담기, 다음, 나가기가 **마우스로 안 눌렸다**. 실측 2026-08-12: 단추
    ...
    /* 조작 단추는 **오른쪽**에 모은다. 왼쪽 아래는 앱의 채팅 알약 자리라 이전이 그 밑에
    ...
    @media (max-width: 720px) {
      ...
      /* ★ 말 상자, 되물음, 링크 상자는 **아래 시트 위쪽 절반**(그림 자리)에 붙인다.
         ...
         지울래요가 화면에 있는데 눌러지지 않았다(실측 2026-08-14: 손가락이 시트 단추에 닿았다).
      ...
      /* ★ 손가락 규격(44). 폰에서 시점 줄 단추 다섯과 다른 목록이 24~38px 이었다
      ...
      /* ★ 옆으로 밀리는데 **밀린다는 표시가 없었다**. 스크롤막대를 감췄으니(폰에서는 원래 안 보인다)
         오른쪽 끝을 흐리게 해서 더 있다를 눈에 보이게 한다. 끝까지 밀면 흐림도 사라진다. */
...
;
  ...
  function buildKarmoGraph(container: HTMLElement): void {
    ...
    /* 옆 패널의 더 적기가 펼쳐져 있나. 카드마다가 아니라 **판 전체로 하나**다.
        ...
        <div class="km-body">
          <div class="km-canvas" data-km="canvas">
            ...
            <!-- ★ 저장 표시는 **툴바 밖**에 산다 (TASK-KL-271 F3). 툴바 흐름 안에 있던 동안엔
                 뜰 때마다 오른쪽 것들을 밀어서 + 새 판이 208px → 318px 로 움직였다(실측).
    ...
    function keepPickedCardVisible(): void {
      ...
      const step = (): void => {
        ...
        /* 시트는 0.18초에 걸쳐 미끄러져 올라온다. 올라오는 중에 위치를 재면 안 가렸다가
        ...
        /* ★ **두 자를 같은 자로 맞춘다** (2026-08-12, 계측 세 번째. 이게 진짜 원인이었다).
           ...
           내려온 만큼(실측 282px) 늘 덜 밀었다. 24px 만 밀고 이제 안 가림이라 멈췄고,
        ...
        /* ★ **곧바로 옮긴다. 애니메이션으로 부탁하지 않는다** (2026-08-12, 계측 두 번째).
           `animate=true` 는 그 자리로 미끄러져 가라는 예약이다. 이 되풀이는 프레임마다
     ...
     * 덮어쓰기는 되돌릴 수 없고, 사람은 지금 것도 살아 있겠지라고 믿는다. 이름이 겹치면 표시만 붙인다.
     */
    ...
    // 어떤 편집이든 되돌아간다가 코드 한 줄로 보장된다. 새 편집 기능을 붙일 때마다
    // undo 를 따로 안 짜도 된다. 그 대신 스냅샷 상한을 둬서 메모리를 묶는다.
    /** 되돌리기 더미. **판 수가 아니라 무게로** 자른다 (규칙은 `history.ts`, TASK-KL-271 M4). */
    const history: string[] = [];
    /** 다음 저장이 무엇을 한 것인지. 저장 직전에 아는 자리에서 적어 둔다. */
    let lastAction = '';
    /** 각 판이 무엇을 한 뒤인지. 되돌리기 단추의 이름이 된다(빈 문자열이면 그냥 되돌리기). */
    ...
    function snapshot(what = ''): void {
      ...
      history.push(json);
      histLabels.push(what);           // 무엇을 한 판인지. 되돌리기 단추가 이걸 말한다
     ...
     * (TASK-KL-271). 저장이 안 되는 판에서 필요한 건 경고문이 아니라 지금 파일로 빼기다.
     */
    /**
    ...
    /** 지금 맵 이름. 라이브러리 목록에서 어느 맵에서 온 글인가를 보여 주는 데 쓴다. */
     ...
     * 저장 버튼이 어디 있지?로 불안해하다가 창을 안 닫는다. 저장할 때마다 잠깐 저장됨을 켠다.
     */
    let savedTimer: ReturnType<typeof setTimeout> | null = null;
    let savedCount = 0;   // 처음 몇 번만 이 브라우저에까지 말한다. 매번이면 소음이 된다
    function flashSaved(): void {
      const el = root.querySelector('[data-km="saved"]') as HTMLElement | null;
      if (!el) return;
      // ★ 저장됨만으로는 **어디에** 저장됐는지 모른다. 방문기록을 지우면 사라지는 것을
    ...
    /* 툴바가 폰에서 옆으로 밀리는데 **밀린다는 표시가 없었다**(스크롤막대는 폰에서 안 보인다).
       오른쪽에 남은 것이 있으면 끝을 흐린다. 끝까지 밀면 흐림이 사라져 끝도 같이 말해 준다. */
     ...
     * (실측 2026-08-14). 화면이 터지진 않지만 ① 그 장이 몇 장을 담았나가 틀리고
     ...
     * 판마다 여기까지 줬다를 적어 두고 그 뒤 번호만 준다. 옛 판은 지금 쓰는 가장 큰 번호에서 잇는다.
     */
     ...
     * 인물이면 출신, 소속을 적는다는 이 판의 약속이 견본에서만 살아 있었다. 빈 칸이 곧 여기에 적어라다.
    ...
    function syncEmptyHint(): void {
      /* ★ **빈 판에서는 툴바도 접는다** (TASK-KL-271 F2 / S2).
         카드가 0장인데 전체 보기, 발표, 찾기가 다 켜져 있었다. 할 수 있는 게 하나뿐인
      ...
      /* ★ 폰에서는 **두 줄까지**다 (2026-08-12 사용자 검토). 넉 줄이면 아래 시트(시작 갈래)와
         겹쳐 문장이 중간에서 잘리고, 잘린 안내는 안 읽는다. 실측: ...고를 수 있어에서 끊겼다.
    ...
    function createGroup(): GroupDef {
      ...
      // 묶음 이름 후보도 갈래를 안 가린다. 가족이든 묘지든 이 맵에 없으면 후보다.
      ...
      const group: GroupDef = {
        ...
        label,
        color,
        // 겹치는 묶음이 흔한 도구다. 네모보다 멤버를 감싸는 윤곽이 누가 어디 속하는지를 덜 흐린다.
        shape: 'hull',
    ...
    function applyTerms(): void {
      ...
      // 내 용어가 이긴다. 방금 고친 색이 옛 정의에 덮이면 고쳤는데 그대로가 된다.
     ...
     * 노드가 늘면 하나씩 만지는 것이 곧 벽이 된다(Heptabase 도 골라서 Create Section이 기본 동작).
     */
    /**
     * 글 한 덩이 → 노드, 선. 글로 만들기와 예시 넣어 보기가 같은 길을 쓴다 . 
    ...
    function buildFromOutline(src: string, kind: string): number {
      ...
      /* ★ **같은 이름은 같은 카드다** (TASK-KL-271 P8). 글에서 소꿉친구가 아래에 또 나오는 것은
         새 사람이 아니라 그 사람에 대해 한 줄 더 적은 것이다. 예전에는 이름이 같아도 카드를 새로
         만들어서, 첫 화면 견본에 소꿉친구와 라이벌이 두 장씩 놓였다(사용자 중복 지적의 그 자리). */
      ...
      // (글에는 욘 -> 마을이라고만 적히지, 그 둘이 어디서 왔는지는 안 적힌다).
     ...
     * 패널이 아홉 가지로 늘었는데 서로 오가는 길이 없었다. 각 패널에 닫기만 있어서
     * 다른 패널로 가려면 툴바에서 그 아이콘을 **다시 찾아야** 했다. 탭을 항상 띄워
     * 지금 어디에 있고 어디로 갈 수 있는지를 한자리에서 보인다.
     */
     ...
     * 여덟은 어쩌다 한 번 쓰는 것인데 고른 것과 같은 크기를 먹었다. 이제 화면은 **고른 것이
    ...
    function sayPicked(): void {
      ...
      /* ★ **말이 달라졌을 때만** 다시 적는다. 카드 id 로만 견주면, 만들자마자(이름 없을 때) 한 번
         말하고는 이름을 친 뒤에도 입을 다문다. 실측 2026-08-14: (...) 골랐음에서 안 바뀌었다. */
    ...
    function renderSide(): void {
      ...
      if (sideEl.classList.contains('hidden')) {
        ...
        // 빈 판에서는 **묻는 말이 맨 위**다. 고르면 여기서 고칩니다를 위에 두면 정작 첫 할 일이
        ...
        if (spec.nodes.length > 0 && spec._meta?.tips !== 'off') {
          ...
          /* ★ **견본을 지우는 길** (TASK-KL-271 F6). 견본을 깔아 주는 것까지는 했는데, 이제 이걸
             지우고 내 걸로 시작하고 싶다는 길이 ⋯서랍 맨 밑 빨간 단추뿐이었다. 처음 온 사람이
          ...
          if (spec._meta?.sampleFp === `${spec.nodes.length}:${spec.edges.length}`) {
            ...
            wipe.onclick = () => {
              ...
              delete meta.sampleFp;   // 자국을 지운다. 빈 판에 견본 지우기가 또 뜨면 안 된다
              spec._meta = meta;
              applySpec();
              persistStructure();
              renderSide();
        ...
        sideEl.querySelectorAll('[data-km="intent"]').forEach((btn) => {
          (btn as HTMLButtonElement).onclick = () => {
            ...
            // 팩을 바꾸면 이미 놓인 것들의 기본 종류까지 흔들려 모든 주제가 공존이 깨진다.
            ...
            // 자국과 안 맞아 견본 지우기가 저절로 사라진다. 남의 작업을 지울 위험이 없다.
      ...
      // 👁 보기 전용에서도 **코멘트만은 열어 둔다**. 받은 사람이 여기 이상해요를 남길 유일한 칸이고,
     ...
     * 1부에서는 소꿉친구, 2부에서는 라이벌을 담으려면 먼저 **시점이 있어야** 한다.
     */
    ...
    function renderTimes(): void {
      ...
      bar.innerHTML = `<button class="btn btn-ghost" data-km="time-prev" title="${esc(t('karmograph.time.prev'))}"
        ...
        //   보이기만 해도 고쳐도 되나를 묻게 되고, 눌리면 받은 판이 말없이 달라진다.
     ...
     * 판은 누가 누구와 이어졌나에 강하고 빠짐없이 훑기에 약하다. 카드가 흩어져 있어
    ...
    function renderSideBody(): void {
      ...
      sideEl.innerHTML = `
        <!-- 머리에는 **이 카드가 무엇인지**를 적는다. 노드는 프로그램 말이지 사람 말이 아니고,
             바로 위 탭이 이미 고른 것이라 같은 말이 두 번 나오기도 했다. -->
        <h4>${kindIcon(node.kind)} ${esc(kindLabel(node.kind))}</h4>
        ${(() => {
          /* ★ **이 카드가 얼마나 익었나** (TASK-KL-271 L5, Heptabase 계보). 관계망 칸의 아직 안
             적은 칸이 판 전체를 말한다면 이건 이 카드 한 장이다. 고쳐 쓰는 그 자리에서 남은
        ...
        <!-- ★ 판이 커지면 전체 그림은 아무 말도 안 한다(KL-271 L1). 그렇다고 말없이 감추지는
             않는다. 적어 둔 카드가 소리 없이 사라지면 내 것이 없어졌다가 된다. 권하기만 한다. -->
      ...
      // 이 카드로 바로 오는 주소. 큰 그림을 보낼 때 어디를 보라는 건지를 말로 설명하지 않게.
      ...
      // 걸러진 사이에 고른 값이 사라지면 내가 뭘 골랐는지를 잃는다.
     ...
     * 처음 여는 사람이 이제 뭘 하지로 멈추는 자리가 바로 거기였다. 그래서 **다 익힐 때까지만**
    ...
    function placeMini(): void {
      ...
      /* 놓을 자리 후보는 둘. **카드 위** 아니면 **카드 아래**. 어느 쪽도 시점 줄과 카드를
         건드리면 안 된다.
         ★ 처음엔 시점 줄에 닿으면 그만큼 아래로 민다로만 고쳤다가 **카드를 덮었다**
    ...
    /* ★ 카드 위 작은 도구줄의 ↝ 선 잇기를 걷어냈다 (TASK-KL-271 R1).
    ...
    canvas = new GraphCanvas(canvasEl, {
      ...
      // 구별되지 않았다(2026-08-12 사용자 검토). 이름 첫 글자는 누구만 말하고 무엇은 안 말한다.
      ...
      onSelectMany: (ids) => {
        ...
        /* ★ 고른 것이 패널을 정한다 규칙은 이제 한 곳(`ui-state.ts`)에 산다 (KL-271 R6/C2).
           ...
           그때마다 화면이 튀면 하던 일이 끊긴다. 여럿, 선을 고른 것은 이제 이걸 할 거야라 따라간다. */
    ...
    /* ★ 다음에 만들 카드의 종류 드롭다운을 툴바에서 걷어냈다 (TASK-KL-271 P1).
       아무도 그걸 다음에 만들 종류로 안 읽는다. 고른 카드의 종류로 오해하고, 정작 종류를
    ...
    function spawnNodeAt(worldX: number, worldY: number, label: string): void {
      ...
      //   `?`(도움말)는 이름에 ?로 박히고, 크기 손잡이 첫 누름은 칸에서 빠져나오기로 먹혔다.
     ...
     * 지금까지 공용 글은 카드를 골라야만 읽혔다. 세계관 설정처럼 그림 옆에 늘 펼쳐 두고 싶은 글이
     ...
     * 전에는 툴바에 고르개가 따로 있어서 덜 보기가 툴바 찾기, 툴바 차수, 거르기 패널
    ...
    function spread(startIds: string[], degree: number): Set<string> {
      ...
      // 둘레 N다리도 지금 시점의 선을 타고 번진다 (KL-271 X2). 2부엔 없는 선으로 이웃이
    ...
    /**
     * 찾은 수를 찾기 칸 옆에 적는다. 흐려지는 것만으로는 없다와 아직 안 쳤다가
     * 구별되지 않는다. 아무것도 안 맞으면 판이 통째로 흐려지는데, 그게 찾는 중인지
     * 없다인지 화면이 말해 주지 않았다(실측 2026-08-12).
     */
    ...
    function syncFocus(): void {
      ...
      if (q0) {
        ...
        if (starts.length === 0) {
          // 아무것도 안 맞으면 전부 흐려서 없다를 눈으로 보여 준다.
        ...
        /* 고른 것만/1단계를 골랐는데 고른 카드가 없으면 **아무 일도 안 일어난다.**
    ...
    // 나눠 차례로 연다. 각 장은 어느 노드들을 또렷하게 둘지만 기억한다(Kumu 슬라이드).
    ...
    function saveStageForm(): void {
      ...
      if (stageFormMode === 'edit') {
        ...
        steps().splice(stepIndex + (steps().length ? 1 : 0), 0, {
          ...
          // 또렷하게 고른 것이 없으면 지금 보이는 자리를 **틀**로 굳힌다. 그러면 나중에 그 자리에
     ...
     *
     * 보기 전용 링크는 눌러서 열어 보세요까지다. 그런데 관계도를 남에게 보이는 가장 흔한 자리는
    ...
    function embedCodeOf(url: string): string {
      // 높이는 넉넉히, 테두리는 0. 블로그 글 안에서 끼워 넣은 창처럼 보이면 읽는 흐름이 끊긴다.
    ...
    function makeShareLink(readOnly: boolean, asEmbed = false): void {
      ...
      void encodeShare(live).then(async (code) => {
        ...
        if (url.length > SHARE_URL_LIMIT) {
          // 한계를 넘기는 것은 거의 언제나 사진이다. 파일로 보내세요로 끝내면 사진 한 장 때문에
    ...
    /**
     * 구조를 살리는 배치. 가지런히는 있던 자리를 존중하지만, 이미 엉킨 그림은 그것으로 안 풀린다.
    ...
    // 연표. 언제가 적힌 칸을 시간축으로 삼는다. 어느 칸인지는 **숫자가 가장 많이 든 칸**으로 고른다
    // (사람에게 날짜 칸을 먼저 정하라고 시키면 아무도 안 쓴다).
     ...
     * 이 작업이 없애려는 중복이다. 그래서 편집기들이 명령에 쓰는 자판을 따로 쓴다.
     * 이 위젯이 화면에 있을 때만 받는다.
     */
    ...
    function onKeyDown(ev: KeyboardEvent): void {
      ...
      if (!inField && !ev.ctrlKey && !ev.metaKey && !ev.altKey) {
        ...
        /* ★ 판에 있다 = **판 안에 진짜로 서 있다** (KL-271). 예전엔 손잡이가 아니면 판이라
        ...
        if (ev.key === 'Delete' || ev.key === 'Backspace') {
          ...
          /* ★ **선도 자판으로 지운다** (KL-271). 카드만 되던 동안, 선을 골라 패널까지 떠 있는데
             Delete 를 눌러도 아무 일이 없었다. 자판만 쓰는 사람은 고를 수는 있는데 못 지운다를
      ...
      if (key === 'z' && !ev.shiftKey) { ev.preventDefault(); restoreTo(histIndex - 1); }
        ...
        /* 전부 고르기. 나란히 놓기, 묶음 넣기, 한꺼번에 종류 바꾸기가 전부 여럿 고름에 있는데,
    ...
    /* ✍ **손그림 질감** (TASK-KL-238 / 18 excalidraw). 자로 잰 상자는 결정된 것처럼 보여
       사람이 고치자는 말을 못 꺼낸다. 삐뚤빼뚤한 상자는 아직 얘기 중으로 읽힌다.
       흔들림은 카드 id 로 정해져 있어 끌거나 확대해도 모양이 안 변한다(춤추지 않는다).
       배경 무늬와 같이 **맵마다** 기억한다(`_meta.sketchy`). 이건 그 그림의 말투이지 사람의 취향이 아니다. */
    ...
    /* 휠, 핀치, 전체 보기로 바뀐 배율도 따라 적어야 한다. 숫자가 거짓이면 없느니만 못하다.
    ...
    // ── 그림으로 내보내기 (격차 G) ──────────────────────────────────────────
    // 레퍼런스들은 UI 를 숨길 테니 직접 캡처하세요에서 멈춘다. 우리 캔버스는 SVG 라
     ...
     * 볼 수 없고, 폰에서는 주소창 이름이 함께 떠 사이트가 하는 말처럼 읽힌다. 여기서는
     * 판 위에 얹고, 읽는 동안 화면은 그대로 살아 있다. 토스트와는 다르다. **안 사라진다**
     * (안 됐다는 말이 3초 만에 사라지면 사람은 무슨 일이 일어났는지 영영 모른다).
     */
     ...
     * confirm 은 판을 얼려 **무엇을 지우려는지 뒤를 못 보게** 만든다. 그 판이 어느 판이더라를
     ...
     * 보이며, 고치라는 건지 복사하라는 건지가 안 읽혔다. 여기서는 **글자가 이미 골라진 채**로
     ...
     * 그림으로 저장과 SVG 로 저장이 같은 자리를 쓴다. 갈리면 한쪽만 틀을 입는다.
     */
    ...
    function exportImage(scale: number): void {
      ...
      // toBlob 이 통째로 막힌다(내보내기가 아무 일도 안 일어남이 된다).
     ...
     * (한 자리에 두 번 적어 두면 한쪽만 고쳐져 SVG 는 맞는데 영상은 딴 데를 비춘다가 된다.)
     */
    ...
    q<HTMLButtonElement>('canvas-out').onclick = () => {
      ...
      // JSON Canvas 도 그림을 옮기는 것이라 지금 시점을 따른다(자료 통째 백업은 JSON 내보내기다).
    ...
    q<HTMLButtonElement>('clear').onclick = async () => {
      ...
      store.clear();
      renderSide();
      snapshot(t('karmograph.act.clear'));   // 전체 삭제도 되돌릴 수 있어야 한다
    };
     ...
     * 남의 관계도를 보다가 여기서부터 내 걸로 이어 그리고 싶다가 이 도구의 가장 자연스러운 다음 걸음이다.
     */
    let readOnly = false;
    function enterReadOnly(): void {
      ...
      (badge.querySelector('[data-km="fork"]') as HTMLButtonElement).onclick = () => {
        ...
        badge.remove();
        // 주소에서 보기 전용 표시를 지운다. 새로고침해도 다시 잠기면 복제했는데 또 잠긴다가 된다.
    ...
    function followOtherTab(ev: StorageEvent): void {
      /* 목록이 바뀌었으면 **고르개도 따라간다**. 다른 탭에서 만든 판이 목록에 안 뜨면
         분명 만들었는데 없다가 된다(자료는 이제 안 사라진다. 106회차 합치기. 화면만 낡았다). */
      ...
      /* 내 용어도 따라간다. 다른 탭에서 만든 종류가 이 탭의 고르개에 없으면 분명 만들었는데
         목록에 없다가 된다(자료는 이제 안 사라진다. 107회차 합치기. 화면만 낡았다). */
      if (ev.key === 'karmograph.terms' && ev.newValue) {
        ...
        /* ★ **자리를 갈아 끼우지 말고 속을 채운다.** 패널은 이 객체를 그대로 쥐고 있어서,
           ...
           화면도 안 된다(실측 2026-08-14: A 탭의 새 종류가 통째로 사라졌다). */
    ...
    function openActiveMap(): void {
      touchedHere = false;   // 판을 새로 열면 내가 고친 적 없다로 돌아간다
    ...
    // 이름 옆 ✎ 와 서랍의 이름 바꾸기는 **같은 길**을 쓴다. 두 길이 생기면 언젠가 갈라진다.
    ...
    q<HTMLButtonElement>('map-del').onclick = async () => {
      ...
      // 지우는 말은 **판 이름을 넣어** 묻는다. 어느 판이더라를 되묻게 하지 않으려고.
    ...
    tabs: [
      {
        ...
        build: (container: HTMLElement): void => {
          /* ★ 스타일은 **마스코트 스크립트에 기대지 않는다** (TASK-KL-271, 원인 확정).
             ...
             먹는다로 보였고, 그 착시로 같은 자리를 세 번 고쳤다(실측: style 넷뿐, km 규칙 0).
