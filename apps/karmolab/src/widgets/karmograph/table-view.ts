 ...
 *
 * 판은 누가 누구와 어떻게 이어졌나를 잘 보여 주는데, **빠진 것 없이 훑기**에는 나쁘다 . 
 * 카드가 흩어져 있어서 눈이 순서를 못 잡는다. 인물 스물넷을 이름순으로 한 번 훑고 싶다
 * 출신이 안 적힌 사람만 모아 보고 싶다는 표가 하는 일이고, 그건 판이 못 하는 일이다.
 *
 * 판을 대신하려는 게 아니라 **같은 자료의 다른 렌즈**다. 그래서 고르면 그 카드가 판에서도 골라진다.
 *
 * 여기서는 **무엇을 어떤 순서로 늘어놓나**만 정한다(칸, 단추는 부르는 쪽).
 * 정렬은 눈으로 못 재는 셈이라 순수 함수로 둔다. 같은 판을 두 번 열면 같은 순서를 검사로 잠근다.
 */
...
export interface TableRow {
  ...
  label: string;
  kind: string;
  /** 칸 이름 → 값. 없으면 빈 문자열(표에서는 빈 칸도 한 줄의 정보다). */
 ...
 *   목적(적힌 것을 훑기)이 첫 화면에서 깨진다. 안 적은 것만 보고 싶다는 내림차순이 아니라
 *   거르기가 할 일이다.
