 ...
 *   카드 {shape=note group=Guide tags=ai,web} : 짧은 메모
 *                           ← 중괄호 안에 모양, 묶음, 꼬리표를 적을 수 있다.
 *
...
function parseProps(raw: string): Partial<Pick<TextNode, 'kind' | 'shape' | 'group' | 'groups' | 'tags' | 'note'>> {
  const out: Partial<Pick<TextNode, 'kind' | 'shape' | 'group' | 'groups' | 'tags' | 'note'>> = {};
  for (const part of raw.split(/[\s;,]+/)) {
    const token = part.trim();
    if (!token) continue;
    const eq = token.indexOf('=');
    const key = (eq >= 0 ? token.slice(0, eq) : token).trim().toLowerCase();
    const value = (eq >= 0 ? token.slice(eq + 1) : '').trim();
    if (!key) continue;
    if (key === 'kind' && value) out.kind = value;
    else if (key === 'shape' && value) out.shape = value as NodeShape;
    else if (key === 'group' && value) out.group = value;
    else if (key === 'groups' && value) out.groups = value.split('|').map((x) => x.trim()).filter(Boolean);
    else if (key === 'tags' && value) out.tags = value.split('|').map((x) => x.trim()).filter(Boolean);
...
export function parseOutline(text: string): OutlineDoc {
  ...
  for (const raw of text.split('\n')) {
    ...
    const [beforeColon, ...rest] = body.split(':');
    const colonNote = rest.join(':').trim();
    const propMatch = /^(.*?)(?:\s*\{([^}]*)\})?$/.exec(beforeColon.trim());
    const label = propMatch?.[1]?.trim() ?? '';
    const props = propMatch?.[2] ? parseProps(propMatch[2]) : {};
    if (!label) continue;
    const edgeLabel = colonNote || props.note || undefined;
 ...
 * (그래야 누가 누구 밑인지가 선을 따라가지 않아도 읽힌다).
 */
