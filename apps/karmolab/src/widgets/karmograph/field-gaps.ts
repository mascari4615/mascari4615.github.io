 ...
 * 「무엇을 더 채워야 하나」는 세계관을 짓는 사람이 가장 자주 하는 질문인데, 도구가 답을 안 했다.
 *
 * 관계망 칸은 이미 **이어질 법한데 안 이어진 사이**(구조적 공백)를 말해 준다. 이건 그 짝 —
 * **적힐 법한데 안 적힌 칸**이다.
 *
 * 여기서는 **무슨 말을 할지**만 정한다(말은 부르는 쪽이 번역해 끼운다). 세는 규칙이 눈에 안 보이는
 * 셈이라 순수 함수로 둔다 — 화면에 섞이면 「이 수가 맞나」를 검사로 못 묻는다.
 *
 * 규칙:
 *  - **빈 값 = 안 적은 것.** 칸 이름만 있고 값이 비었으면(공백만 있어도) 안 적은 것이다.
 *  - **종류별로 센다.** 「인물의 출신」과 「장소의 출신」은 다른 이야기다.
 *  - **한 장짜리 종류는 재촉하지 않는다.** 방금 만든 카드 하나를 두고 「1장이 안 적었어요」는 잔소리다.
 *  - **다 빈 칸이 먼저.** 「아무도 안 적었어요」는 「7장 중 2장」보다 놀랍고, 대개 그 칸이 쓸모없다는 신호다.
