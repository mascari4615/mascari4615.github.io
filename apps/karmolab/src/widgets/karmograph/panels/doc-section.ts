...
import type { DocHolder } from '../../../lib/karmograph/notes';
...
function sharedUsersHtml(ctx: PanelCtx, noteId: string, key: string): string {
  ...
  const spec = ctx.spec();
  const rows = [
    ...spec.nodes.filter((n) => n.docRef === noteId).map((n) => ({ id: n.id, kind: 'node', name: n.label || t('karmograph.unnamed') })),
...
export function docFieldHtml(ctx: PanelCtx, node: DocHolder, skin: DocFieldSkin = nodeDocSkin()): string {
  ...
  return `
    <div class="km-field">
      ...
      ${shared
        ? `<div class="km-hint" style="color:#fcd34d">🔗 ${t('karmograph.sharedDoc', { title: `<b>${esc(shared.title || t('karmograph.noteLabel'))}</b>`, n: users })}</div>
      ...
      ${others.length === 0 ? '' : `<select data-km="${skin.key}-embed">
        ...
        ${others.map((n) => {
          ...
          return `<option value="${esc(n.id)}">${esc(n.title || t('karmograph.noteLabel'))} ${esc(t('karmograph.docWhole'))}</option>`
            + blocks.map((b) => `<option value="${esc(n.id)}#${esc(b.id)}">${esc(n.title || t('karmograph.noteLabel'))} › ${esc(b.id)}</option>`).join('');
        }).join('')}
      </select>`}
