...
export function renderStoragePanel(ctx: PanelCtx): void {
  ...
  side.innerHTML = `
    ...
    <div class="km-field">
      ...
      ${rep.warn
        ? t('karmograph.rep.msg3')
        : ''}
    </div>
    <div class="km-field">
      <label>${esc(t('karmograph.rep.msg4'))}</label>
    ...
    <div class="km-hint">${t('karmograph.hint12', { em: `<b>${esc(t('karmograph.rep.msg6'))}</b>` })}</div>
    <button class="btn btn-ghost" data-km="st-close">${esc(t('karmograph.stClose.label'))}</button>`;
