 ...
 *
 * 그림으로 저장이 판을 있는 그대로 오려 내던 시절, 그 그림을 받은 사람은 제목도 범례도 없이
 ...
 * 위에 제목줄, 아래에 범례줄을 두른다. 그림 자체는 안 건드린다. 건드리면 화면과 다른 그림이 된다.
 *
...
export interface PosterOptions {
  ...
  stamp?: string;
  legend: LegendItem[];
  /** 범례에서 접힌 가짓수. 0 보다 크면 그 밖 N가지를 끝에 붙인다. */
  more?: number;
  skin: PosterSkin;
