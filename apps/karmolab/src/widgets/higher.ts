 ...
 *
 * 자리: 커뮤니티와 같다. 앱 안의 한 화면(`/#higher`). 도구 상세 페이지는 만들지 않는다.
    ...
    tabs: [
      {
        ...
        build: function (container: HTMLElement): void {
           ...
           * 놀이터가 코스를 세려면 오늘 한 판 끝냈다가 있어야 한다. 그 한 줄을 여기서 남긴다. */
           ...
           * 열 판을 이어 가도 계속 키였고 다시를 눌러도 그대로였다. 같은 질문을
          ...
          function answer(el: HTMLElement, chosen: Item, other: Item): void {
             ...
             * 끝나는 순간이야말로 아, 저쪽이었구나가 눈에 들어와야 하는 자리다. */
            if (!win) {
            ...
            if (win) {
              ...
              // 첫 판에 지면 0연승에서 끝났습니다가 된다. 아깝지도 않은데 아깝다고 말한다.
           ...
           * 여기서 겨룰 수 있는 칸은 **숫자 칸**뿐이다. 분류로는 어느 쪽이 더 큰지 물을 수 없다.
          ...
          /** 내 표가 밀어 준 판이 있으면 그걸로, 없으면 있던 대로. */
          ...
          const keys = (e: KeyboardEvent): void => {
            /* 지금 보이는 화면인가는 **페이지 칸**에게 물어야 한다. 위로 훑기만 하면
