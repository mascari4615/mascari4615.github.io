 ...
 * 예전엔 이 셋이 보기 3개로 뭉쳐 있어서, 크게 보려면 살결까지 따라 바뀌었다.
 */
...
/* 납작은 뺐다. 덱 배치에서 기존과 눈에 띄는 차이가 없었다(둘 다 카드 얼굴).
...
/**
 * **자리는 덱 기준으로 적는다.** 덱은 빈 칸이 자리로 보이는 배치라 3번 칸이 비었다가
...
export const DECK_CSS = `
    ...
    /* 탭 줄은 껍데기에 **파 놓은 홈**이다. 안쪽 그림자로 한 단 내려앉고, 그 안에서
       탭이 솟아 있다. 양끝을 흐리게 지워 두면 잘린 탭이 보여서 옆으로 더 있다가
    ...
    /* ── 라이트 테마의 스트림덱. **기기도 밝다** ──────────────────
       처음엔 실물이 검으니 라이트에서도 검게로 뒀는데, 밝은 화면 한가운데 검은 판이
    ...
    [data-theme="light"] .skin-glass .fav-deck-tab.on {
        color:var(--accent-ink); background:#e7e3f7; border-color:color-mix(in srgb, var(--accent) 45%, transparent);
    ...
    /* ── 살결 ③간판. 밤의 편의점 미니 간판 자석 (TASK-KL-346).
       스트림덱이 기기라면 간판은 물건이다: 어두운 프레임 안에 유백 확산판이
    ...
    /* 손이 닿으면 **한 단 더 솟는다**. 두께가 늘어나면서 옆면이 넓어진다.
       위치를 옮기는 대신 두께를 바꾸는 쪽이 물건으로 읽힌다. */
    ...
    /* 빈 칸 = **불 꺼진 간판**. 프레임만 보이고, 손이 닿으면 형광등처럼 예열된다.
       이게 살결이 데코가 아닌 이유다. 빛이 담겼나라는 사실을 나른다. */
    ...
    @media (prefers-reduced-motion: reduce) {
        ...
        /* 움직임을 줄인 사람에게 깜빡임은 그냥 켜짐이다. 정보는 표시등이 나른다. */
...
export function registerDeckProps(): void {
    ...
    const props = [
        ...
        /* 유리 높이, 우물 깊이도 **전이 가능한 값**으로 등록한다. 등록 안 하면 손이 닿는
           ...
           옮겨 앉는다. 그게 빛이 계속 튄다의 정체였다 (2026-08-19 제보). */
 ...
 * 빛이 튄다로 보인다 (2026-08-19 제보). 마지막 자세를 들고 있다가 새 판에 그대로
 * 얹으면 다시 그려도 이어진다.
 */
...
export function renderDeckHtml(
    ...
    const pages = groups
        .map((g, i) => {
             ...
             * (그게 정렬되지 않게의 뜻이다, 사용자 결정 2026-08-19). */
...
export function wireDeck(container: HTMLElement, onSlotsChanged: () => void = () => {}): () => void {
    ...
    const onMove = (e: PointerEvent): void => {
        ...
        /* **반사는 키마다 따로 친다.** 덱 하나에 값 하나를 두면 열두 칸의 빛이 한 몸으로
           움직여서, 눈에는 안 따라온다 / 가끔 튄다로 보인다. 각 키의 제 상자 기준으로
    ...
    const onDragStart = (e: DragEvent): void => {
        ...
        if (e.dataTransfer) {
            /* 링크(<a href>)를 그냥 끌면 브라우저가 **주소**를 끌어 다른 창에 떨군다.
               우리 뜻은 칸을 옮긴다이므로 자리 열쇠만 싣고, 끌리는 그림도 키 얼굴로 준다. */
     ...
     * 두 벌로 적으면 한쪽만 고쳐져 마우스로는 되는데 자판으로는 딴 자리로 간다가 된다. */
...
/**
 * 키 배선. **배치와 무관**하다. 살결이 기존이 아니면 목록 배치에서도 키를 쓰므로,
