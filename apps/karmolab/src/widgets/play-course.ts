 ...
 * 그 자리에서 하나 남았다를 말해 주지 않으면 그냥 창을 닫는다.
 *
 * 그러려면 놀이마다 오늘 뭘 했나를 각자 세게 되는데, 세 곳에 적으면 그날부터 갈라진다.
...
export function doneToday(id: string): boolean {
  try {
    ...
    /* 월드컵은 오늘 한 판을 따로 안 적는다. 지난 우승 목록에 오늘 날짜가 있으면 한 것이다
...
/**
 * 코스가 **셀 줄 아는** 놀이. 오늘 했나를 읽는 법이 위 doneToday 에 적힌 것들.
 *
...
/**
 * 한 판을 끝낸 자리에 붙이는 줄. 오늘 남은 놀이와 거기로 가는 단추.
...
export function mountCourseNext(slot: HTMLElement, meId: string): void {
  courseGames()
    .then((games) => {
      ...
      /* 한 판이 끝난 자리 = 오늘 뭘 했나가 방금 바뀐 자리다. 계정에 옮겨 적는 것도 여기서
