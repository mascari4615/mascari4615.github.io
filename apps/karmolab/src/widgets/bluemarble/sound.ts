 ...
 *
 * (TASK-KL-241) 여기 있던 「라디오를 틀면 그건 남의 방송이지 이 지구가 아니다」는 이제 반만 맞다 —
 * 지구본은 소리를 둘 낸다. **이 별의 소리**(이 파일, 합성)와 **사람의 소리**(`radio.ts`, 실제 방송).
 * 둘의 관계는 배타가 아니라 층이다: 방송이 울리면 이쪽은 `duck()` 으로 밑에 깔리고,
 * 방송이 끊기면 다시 올라온다. 그래서 **침묵이 생기지 않는다** — 우주가 조용한 것과 화면이
 * 고장 난 것은 사람 귀에 똑같이 들리기 때문이다.
 *
 * 넷을 쌓는다:
...
export class EarthSound {
  ...
  duck(on: boolean): void {
    ...
    // 2.2초에 걸쳐 — 급히 줄이면 그게 더 눈에 띈다(귀에 띈다)
