 ...
 * 나가고 있는 방송**. 합성음이 이 별이 어떤 곳인가라면, 이쪽은 거기 누가 살고 있나다.
 *
...
export function slim(r: RawStation): Station | null {
  ...
  // http 스트림은 https 페이지에서 소리가 안 난다. 여기서 버리지 않으면 나중에 왜 조용하지가 된다.
 ...
 * (사람이 안 나오네를 알아채기 전에 다음이 나오는 게 목표다.)
 */
export class RadioPlayer {
   ...
   * 브라우저가 제스처 밖의 소리로 보고 막는다. 켜자마자 한 곳이 울려야 하는 이 기능에서는
   ...
   * 다음 방송이 막 시작된 뒤에 와서, 멀쩡한 방송을 죽었다고 넘겨 버린다(2026-08-12 실측:
  ...
  private bind(el: HTMLAudioElement): void {
    el.addEventListener('playing', () => {
      ...
      this.clearTimer();
      this.tried = 0; // 소리가 났으면 몇 번 실패했나는 없던 일이다
