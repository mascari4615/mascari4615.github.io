...
/* 쟁반 폭. 주사위 한 변이 1 이라 11 이면 주사위가 폭의 9%. 레퍼런스 bloob 은 그릇 지름의
   12.5%(그릇 400px, 주사위 50px). 사용자 지적도 같다(주사위가 작은 게 아니라 판이 크다) */
const TRAY_W = 8;
const ROLL_Z0 = -1.9; /* 구르는 자리 (먼 쪽) */
const ROLL_Z1 = 4.0; /* 구르는 자리 (앞). 5.4 는 빈 초록이 너무 넓었다(2026-08-31 사용자) */
