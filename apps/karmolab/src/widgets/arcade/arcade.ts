...
(function (): void {
  ...
  function injectStyles(): void {
    ...
    el.textContent = [
      ...
      /* 상황판. 자리 카드 대신. 종이 톤, 왼쪽 위 버튼 아래 */
      '#acPlay.ac-bare:has(.ac-t3bar) #acSeats{display:none}',
      '.ac-ychud{position:absolute;left:14px;top:58px;z-index:3;width:236px;padding:10px 12px 10px;border-radius:var(--radius-sm);background:linear-gradient(172deg,rgba(247,239,220,.96),rgba(230,216,186,.96));box-shadow:0 12px 30px rgba(0,0,0,.45),inset 0 0 0 1px rgba(120,80,40,.25);color:#3a2a18;font-family:"Noto Serif KR","Nanum Myeongjo","Yu Mincho",Georgia,serif;font-size:var(--font-size-sm);transform:rotate(-.4deg)}',
      '.ac-ychudhead{display:flex;justify-content:space-between;align-items:baseline;padding-bottom:6px;margin-bottom:6px;border-bottom:1px solid rgba(80,55,30,.35);font-size:var(--font-size-xs);letter-spacing:.06em;color:#5c4630}',
      '.ac-ychudrolls{font-variant-numeric:tabular-nums;color:#8a5a1a;font-weight:600}',
      '.ac-ychudseats{list-style:none;margin:0;padding:0;display:flex;flex-direction:column;gap:2px}',
      '.ac-ychudseats li{display:flex;align-items:center;gap:8px;padding:4px 6px;border-radius:var(--radius-sm);opacity:.72;transition:opacity .2s,background .2s}',
      '.ac-ychudseats li.ac-cur{opacity:1;background:rgba(184,120,26,.16)}',
      '.ac-ychudmark{width:0;height:0;border:5px solid transparent;border-left:8px solid transparent;flex:0 0 auto;margin-left:-2px}',
      'li.ac-cur .ac-ychudmark{border-left-color:#b8781a}',
      '.ac-ychudname{flex:1;min-width:0;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;text-align:left}',
      '.ac-ychudname small{font-size:var(--font-size-4xs);color:#8a7050;letter-spacing:.08em;margin-left:2px}',
      '.ac-ychudscore{font-variant-numeric:tabular-nums;font-size:var(--font-size-md)}',
      'li.ac-me .ac-ychudname{font-weight:700}',
      '.ac-ychudbest{margin-top:8px;padding-top:6px;border-top:1px dashed rgba(80,55,30,.35);color:#8a5a1a;font-size:var(--font-size-xs);letter-spacing:.04em}',
