...
(function (): void {
  ...
  function injectStyles(): void {
    ...
    el.textContent = [
      ...
      '.ac-tlbranch{font-size:var(--font-size-xs);color:var(--accent-ink)}',
      ...
      '.ac-sol-slot.ac-hole{border-style:dashed;cursor:default;width:100%;height:100%}',
      '.ac-sol-slot.ac-dead{opacity:.4;cursor:default}',
      '.ac-sol-slot small{position:absolute;right:3px;bottom:2px;font-size:var(--font-size-4xs);opacity:.6}',
      '.ac-sol-slot.ac-can{border-color:#7fd6a8;border-style:solid;box-shadow:0 0 0 2px rgba(127,214,168,.4)}',
      ...
      '.ac-sol-cell[disabled]{cursor:default}',
      '.ac-sol-cell.ac-held .ac-sol-card{box-shadow:0 0 0 2px #e0b95f,0 10px 18px rgba(0,0,0,.45)}',
      '.ac-sol-card{position:absolute;inset:0;display:block;border-radius:8px;background:var(--ac-card-face);box-shadow:var(--ac-card-sh);color:var(--ac-black);will-change:transform}',
      /* 들린 카드는 살짝 뜨고 기운다. 손에 든 것이 눈에 보여야 한다 */
      '.ac-sol-cell.ac-held .ac-sol-card{transform:translate(7px,-5px) rotate(1.4deg)}',
      '@keyframes ac-solpop{0%{transform:scale(1.06)}60%{transform:scale(.985)}100%{transform:scale(1)}}',
      '.ac-sol-card.ac-lands{animation:ac-solpop .26s ease-out}',
      ...
      '.ac-sol-card i{position:absolute;left:6px;top:calc(var(--sw) * 0.32);font-style:normal;font-size:calc(var(--sw) * 0.2)}',
      '.ac-sol-card em{position:absolute;right:6px;bottom:5px;font-style:normal;font-size:calc(var(--sw) * 0.42);opacity:.85}',
      '.ac-sol-bar{display:flex;align-items:center;justify-content:space-between;gap:12px;min-height:32px;font-size:var(--font-size-sm);color:#e8f2ec}',
      ...
      /* 족보 연출. 큰 글자가 판 위에 떠오르고 종이 조각이 내린다 */
      '.ac-yccombo{position:absolute;left:50%;top:38%;transform:translate(-50%,-50%);z-index:5;pointer-events:none;display:none;width:60%;height:60%;align-items:center;justify-content:center}',
      '.ac-yccombo.ac-show{display:flex}',
      '.ac-yccombo b{font-family:"Noto Serif KR","Nanum Myeongjo","Yu Mincho",Georgia,serif;font-weight:700;letter-spacing:.12em;color:#f7e0a8;text-shadow:0 2px 0 #7a4a12,0 8px 30px rgba(0,0,0,.7),0 0 40px rgba(230,189,122,.5);animation:acYcPop 1.9s cubic-bezier(.2,1.4,.4,1) forwards;white-space:nowrap}',
      '.ac-yccombo.ac-lv1 b{font-size:clamp(28px,4vw,48px)}',
      '.ac-yccombo.ac-lv2 b{font-size:clamp(36px,5vw,64px)}',
      '.ac-yccombo.ac-lv3 b{font-size:clamp(48px,7vw,96px);animation-duration:2.6s}',
      '@keyframes acYcPop{0%{opacity:0;transform:scale(.4) rotate(-4deg)}12%{opacity:1;transform:scale(1.08) rotate(1deg)}22%{transform:scale(1) rotate(0)}80%{opacity:1;transform:scale(1) translateY(0)}100%{opacity:0;transform:scale(1.05) translateY(-30px)}}',
      '.ac-yccombo i{position:absolute;left:var(--x);top:-10%;width:10px;height:16px;background:#f2d16b;border-radius:2px;opacity:0;animation:acYcFall 2.2s linear var(--d) forwards;transform:rotate(var(--r))}',
      '.ac-yccombo i[style*="--h:1"]{background:#e8d8bd}',
      '.ac-yccombo i[style*="--h:2"]{background:#c9863d}',
      '@keyframes acYcFall{0%{opacity:0;transform:translateY(0) rotate(var(--r))}10%{opacity:1}100%{opacity:0;transform:translateY(120%) rotate(calc(var(--r) + 540deg))}}',
      /* 내 차례 카드의 굴리기 버튼 */
      '.ac-ycroll{display:block;margin-top:8px;padding:6px 12px;border-radius:var(--radius-pill);border:1px solid #f0c98a;background:linear-gradient(180deg,#c9863d,#8f4f1c);color:#fff6e4;font:inherit;font-size:var(--font-size-sm);font-weight:600;letter-spacing:.06em;cursor:pointer;box-shadow:0 4px 12px rgba(120,60,10,.4)}',
      '.ac-ycroll:hover{background:linear-gradient(180deg,#d8954a,#9d5a22)}',
      '.ac-ycroll:disabled{opacity:.55;cursor:default}',
      '.ac-ycroll-off{background:none;border:1px dashed rgba(120,80,40,.4);color:#8a5a1a;box-shadow:none;cursor:default}',
      /* 폰. 세로로 접는다. 점수표가 위(최대 42vh, 스크롤), 무대가 아래(레퍼런스 bloob 폰: 표 위, 쟁반 가운데, 버튼 아래) */
      '@media (max-width:560px){',
      '  .ac-ycpaper.ac-pin{inset:36px 8px auto 8px;width:auto;max-height:42vh;padding:10px 10px 8px;transform:none;overflow:auto}',
      '  .ac-ycpaper.ac-pin .ac-ycpaperin{font-size:var(--font-size-sm)}',
      '  .ac-ycpaper.ac-pin .ac-yctable tr{height:30px}',
      '  .ac-ycpaper.ac-pin .ac-yctable th,.ac-ycpaper.ac-pin .ac-yctable td{height:30px;padding:0 4px;font-size:var(--font-size-sm)}',
      '  .ac-ycpaper.ac-pin .ac-yctable tbody th{width:38%;font-size:var(--font-size-xs)}',
      '  .ac-ycpaper.ac-pin .ac-yccell{height:24px;line-height:22px;font-size:var(--font-size-md)}',
      '  .ac-ycpaper.ac-pin .ac-ychead .ac-ycdice .ac-die{width:26px;height:26px;padding:3px}',
      '  .ac-ycpaper.ac-pin .ac-ychead .ac-ycleft{height:auto;font-size:var(--font-size-xs)}',
      '  .ac-ycpin{left:8px;top:6px;height:28px;padding:0 10px;font-size:var(--font-size-4xs)}',
      '  .ac-ycsplit .ac-yctoast{left:50%}',
      /* 카드 줄은 화면 아래 전폭. transform 을 지워야 안에서 전폭 버튼이 산다 */
      /* 셸 아래 계정 띠가 30px 을 덮는다(실측: 굴리기 버튼이 안 눌렸다). 그 위로 */
      '  .ac-ychud,.ac-ycsplit .ac-ychud{left:8px;right:8px;bottom:42px;transform:none;gap:6px;max-width:none;align-items:stretch}',
      '  .ac-ychudcard{flex:1;min-width:0;align-items:center;text-align:center}',
      '  .ac-ychudrank{right:4px;top:-8px;padding:1px 6px}',
      /* 3D 는 아래 56%. 위는 종이가 덮으므로 쟁반이 종이 밑에 숨지 않게 */
      '  #acPlay.ac-roomfill .ac-ycsplit .ac-t3.ac-t3bar{top:44vh;height:56vh}',
      '  .ac-ychudround{display:none}',
      '  .ac-ychudcard{min-width:84px;padding:4px 7px}',
      '  .ac-ychudcard.ac-cast{padding-left:40px;min-width:112px}',
      '  .ac-ychudface,.ac-ychudface svg{width:24px;height:24px}',
      '  .ac-ychudface{left:6px;top:16px;transform:none}',
      '  .ac-ychudcard.ac-cast{padding-left:34px;min-width:96px}',
      '  .ac-ychudname{font-size:var(--font-size-xs)}',
      '  .ac-ychudname small{display:none}',
      '  .ac-ychudscore{font-size:var(--font-size-md)}',
      '  .ac-ychudsub{font-size:var(--font-size-4xs);white-space:normal}',
      '  .ac-ychudbubble{display:none}',
      /* 굴리기는 카드 밖 전폭 버튼(레퍼런스 bloob 폰: 아래 전폭). 카드는 이름과 점수만 */
      /* 굴리기는 내 카드 안 전폭(레퍼런스 bloob 폰: 아래 전폭). fixed 는 부모 transform 때문에 카드 폭으로 줄었다(실측 60px) */
      '  .ac-ychudcard .ac-ycroll{width:100%;margin-top:5px;padding:7px 4px;font-size:var(--font-size-sm);text-align:center}',
      '  .ac-ychudsub{display:none}',
      '  .ac-yctoast{font-size:var(--font-size-md);padding:8px 14px;max-width:calc(100% - 24px);white-space:normal;text-align:center}',
      ...
      '.ac-root .ac-spfoe .ac-pc{margin-left:-14px}',
      '.ac-root .ac-spfoe .ac-pc:first-child{margin-left:0}',
      '.ac-root .ac-spfoe b{margin-left:12px;font-size:var(--font-size-sm);font-weight:600;opacity:.9}',
      ...
      '.ac-sol3d{position:absolute;left:0;right:0;bottom:14px;display:flex;align-items:center;justify-content:center;gap:14px;z-index:6;color:#eaf4ee;font-size:var(--font-size-sm);text-shadow:0 1px 3px rgba(0,0,0,.6)}',
      ...
      '.ac-root .ac-spsay{min-height:20px;margin:6px 0;font-size:var(--font-size-sm);color:#ffd66b}',
      ...
      '.ac-kind i{font-style:normal;font-size:var(--font-size-2xs);font-weight:700;color:var(--accent-ink);background:var(--accent-dim);border-radius:var(--radius-pill);padding:2px 8px}',
      ...
      '.ac-wclast{font-weight:700;color:var(--accent-ink)}',
      ...
      '.ac-suc{aspect-ratio:1;border:1px solid var(--border);background:var(--bg-primary);color:var(--accent-ink);font-size:min(6vw,22px);font-weight:600;padding:0;cursor:pointer}',
      ...
      '.ac-aulot b{font-size:clamp(44px,14vw,76px);line-height:1;color:var(--accent-ink)}',
      ...
      '.ac-auwho i{font-style:normal;color:var(--accent-ink)}',
      ...
      '.ac-seat.ac-watch{border-color:var(--accent);color:var(--accent-ink)}',
      ...
      '.ac-best{font-size:var(--font-size-xs);color:var(--accent-ink);align-self:flex-start}',
      ...
      '.ac-len.ac-short{color:var(--accent-ink);border-color:var(--accent)}',
      '.ac-none{color:var(--text-secondary);font-size:var(--font-size-sm);margin:var(--space-lg) 0}',
      '.ac-streak{margin-left:8px;font-size:var(--font-size-xs);color:var(--accent-ink)}',
      ...
      '.ac-introcount{font-size:clamp(48px,16vw,84px);font-weight:800;color:var(--accent-ink);line-height:1}',
      /**
      ...
      '.ac-flone.ac-me .ac-flname{color:var(--accent-ink)}',
