...
export function buildCyclicEcosystem(container: HTMLElement): void {
  if (!document.getElementById('ce-style')) {
    ...
    style.textContent = `.ce-wrap{position:relative;width:100%;height:clamp(420px,78svh,900px);overflow:hidden;border-radius:var(--radius-md,12px);background:#07090a}.ce-canvas{display:block;width:100%;height:100%;image-rendering:pixelated}.ce-head{position:absolute;inset:0 0 auto;display:flex;gap:9px;align-items:baseline;padding:13px 15px 30px;pointer-events:none;background:linear-gradient(#07090ae8,transparent)}.ce-name{color:#f2fff6;font-size:var(--font-size-2xs)}.ce-code,.ce-step{color:#c6ddd080;font:11px var(--font-mono,monospace)}.ce-step{margin-left:auto}.ce-actions{position:absolute;right:12px;top:45px;display:flex;gap:6px}.ce-btn{border:1px solid #ffffff28;background:#101912bd;color:#dcf6e3;padding:6px 9px;border-radius:var(--radius-pill);cursor:pointer;font:11px var(--font-mono,monospace)}.ce-btn[aria-pressed="true"]{border-color:#75d98d88;background:#1b4525bd}.ce-log{position:absolute;inset:auto 0 0;padding:35px 16px 16px;color:#eaffef;font-size:var(--font-size-xs);line-height:1.45;pointer-events:none;background:linear-gradient(transparent,#07090ae8)}.ce-hint{display:block;margin-top:3px;color:#c6ddd080;font:11px var(--font-mono,monospace)}`;
  ...
  const actions = document.createElement('div'); actions.className = 'ce-actions'; const reseed = document.createElement('button'); reseed.type = 'button'; reseed.className = 'ce-btn'; const pause = document.createElement('button'); pause.type = 'button'; pause.className = 'ce-btn'; const controls = createObservationControls(); actions.append(controls.element, reseed, pause);
  ...
  const day = new Date().toISOString().slice(0, 10); let seedNo = 0, paused = false; let loop: GardenLoop | undefined; let restartTimer: number | undefined; let sim = new CyclicEcosystem(1, 1); let pixels = fc.createImageData(1, 1); const watcher = new CyclicWatcher();
  ...
  function frame(): void { if (paused || restartTimer !== undefined) return; let stats = null as unknown as CyclicStats; controls.run(2, () => { stats = sim.step(2); }); const event = watcher.observe(stats); if (event) line.textContent = t(`garden.ce.event.${event}`, { n: stats.gen, species: stats.leader + 1 }); draw(stats); if (stats.gen >= 5000 || event === 'extinct') restartTimer = window.setTimeout(plant, 2400); }
  ...
  reseed.textContent = t('garden.reseed'); pause.textContent = t('garden.pause'); name.textContent = t('garden.ce.name'); code.textContent = t('garden.ce.code'); hint.textContent = t('garden.ce.hint'); build(); line.textContent = t('garden.ce.today'); loop = gloop(frame, wrap);
  ...
  Toolbox.onDispose?.(() => { loop?.stop(); if (restartTimer !== undefined) window.clearTimeout(restartTimer); ro.disconnect(); });
}
