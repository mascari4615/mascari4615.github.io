...
export function buildPhysarum(container: HTMLElement): void {
  ...
  const day = new Date().toISOString().slice(0, 10); let seedNo = 0; let paused = false; let loop: GardenLoop | undefined; let sim = new Physarum(1, 1, 1); let pixels = fc.createImageData(1, 1); let random = rng(1); const watcher = new PhysarumWatcher();
