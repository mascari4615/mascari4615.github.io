...
멍(`/t/meong/`)에 걸린 관찰물 열 개다. 2026-09-05 에 정원 위젯이 여기로 들어왔다. 결과를 맞히는 게임보다, 같은 규칙이 반복될 때 어떤 질서와 붕괴가 나타나는지 관찰하는 실험실이다. 손잡이 패널의 `0.5× / 1× / 2× / 4×` 로 한 장면을 오래 보거나 전이를 빠르게 훑고, dock 의 재생 단추로 사건 직전에 멈추고, 다시 심기로 같은 규칙의 다른 초기조건을 비교한다.

껍데기는 안 짓는다. `sim-host.ts` 의 `SimHost` 를 받아 판에 그리고 사건만 말한다. 한 번에 하나만 돈다.
