 ...
 * = 도형으로 지은 도형. 무한히 반복한 도형 S 는 어떤 칸 f 안의 S 를 k 배 확대한 것과 같다.
 ...
 * 계속 잘아지므로, 잘아질수록 퍼진다로 두면 저절로 분열로 보인다. 파고드는 쪽을 고르면
 * 반대로 커질수록 퍼진다가 분열이다. 그래서 방향에 따라 자를 뒤집는다.
 *
 * ── 비용 ──────────────────────────────────────────────────────────────
 ...
 * 단색 검정 위에 낟알만 있으면 스크린세이버가 아니라 디버그 화면처럼 보인다.
 ...
 * 지루하고, 무엇보다 **자로 잰 듯 대칭**이라 자란 것으로 안 보였다. 이제 실루엣은
...
/**
 * 낟알 사이 틈은 **0 이다.** 조금이라도 벌려 두면 다 자란 것 = 통짜 네모가 깨져서, 낟알
 ...
 * 복잡한 덩어리 → 통짜 네모로 뚝 바뀐다(그게 눈에 걸리던 그 점프다). 그렇다고 처음부터
 ...
 * 네모가 자란다로 보인다. 성기게 두고 판을 키워야 울퉁불퉁한 모양이 나온다.
 */
...
export const droste: Piece = {
  ...
  frame(s: Stage): void {
    ...
    /* **세대 번호**. 층마다 다른 덩어리를 쓰되, 시간이 흘러 한 주기가 넘어가도 *같은 노드는
       ...
       나오는데(같은 그림으로 안 돌아온다) 이음매는 안 생긴다. 매번 다른 모양과 한 주기 뒤
       픽셀 동일은 같이 못 가진다. 뒤엣것은 모든 층이 한 모양일 때만 성립한다. */
     ...
     * 계속 갈라진다가 된다. (한때 반대로 뒤집어 뒀다가 축소가 합체로 보였다. 그건 틀렸다.)
     *
    ...
    const draw = (x: number, y: number, size: number, depth: number, alpha: number): void => {
      ...
      for (let i = 0; i < n; i++) {
        ...
        /* **미끄러지기만 한다.** 낟알 크기는 이 애니메이션이 절대 안 건드린다. 크기를
           ...
           그게 확대, 축소로 보여서 지저분했다. 칸은 자기를 틔운 이웃 칸에서 **같은 크기
    ...
    /* 가장자리를 한 번 더 눌러 준다. 낟알이 화면 끝에서 잘리는 게 덜 보이고, 눈이
       한가운데로 모인다. 바탕과 같은 색이라 덧칠이 아니라 어두워짐으로 읽힌다. */
