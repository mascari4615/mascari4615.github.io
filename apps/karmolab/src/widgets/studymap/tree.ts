 ...
 *
 * 왜 그래프인가: 목록은 무엇이 있나는 알려 주지만 무엇 다음에 무엇인가를 못 알려 준다.
...
export interface TreeNode {
   ...
   * 왜 필요한가: 한 갈래 안에서는 단계가 곧 층이다. 사슬로 깊이를 재면 칸이 한 줄로 늘어서서
...
export function layoutTree(nodes: TreeNode[], gapX = 168, gapY = 118): TreeLayout {
  ...
  for (let d = 0; d <= maxDepth; d++) {
    ...
    row.sort((a, b) => {
      /* 적어 둔 차례가 있으면 그것이 먼저다. 부모 x 평균은 누가 옆인가를 아무렇게나 정한다. */
   ...
   * 가장 깊은 부모 = 바로 앞에 배운 것이므로, 그 한 줄이 곧 방금 어디서 왔나가 된다.
   */
 ...
 * 무엇과 무엇이 가까운가를 그린다. 선(선수관계)은 그대로 긋는다. 자리는 뜻, 선은 순서.
 *
...
export function layoutMeaning(nodes: TreeNode[], span = 1180, minGap = 132): TreeLayout {
  ...
  /* 선을 **안 긋는다**. 갈래 사이에는 사람이 적어 둔 순서가 없다. 지금 그을 수 있는 선 100개는
     전부 칸의 선수관계에서 우연히 파생된 것이라, 그으면 자리는 뜻과 선은 순서 두 논리가
 ...
 * `dim` 이 참이면 아직 손대지 않은 자리를 흐리게. 어디부터가 눈에 띄게.
 */
