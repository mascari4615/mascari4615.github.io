 ...
 *
 * downdetector 는 남들도 안 된다를 보여 주는 곳이라 알맹이가 *제보 창고*다. 그건 못 짓는다.
 ...
 * 그래서 이 도구의 된다는 *서버가 대답한다*는 뜻이고, 화면에도 그렇게 적는다. 대신 그걸로도
 * 연결이 아예 안 된다와 대답은 한다는 확실히 갈린다. 그 둘이 사람의 다음 행동을 가른다.
 */
...
(function (): void {
  ...
  /** 대조군 = 여러 나라, 여러 회사. 한 회사만 쓰면 그 회사가 흔들릴 때 내 인터넷이 죽었다로 잘못 말한다. */
  ...
  /** 한 곳을 재 본다. 오래 걸리면 끊는다. 안 끊으면 안 되는 것과 느린 것이 안 갈린다. */
