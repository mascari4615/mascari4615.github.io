 ...
 */
import { 타건수 } from '../../core/hangultype';
...
(function (): void {
  ...
  /* ── 한글 타수 (자소 단위) ────────────────────────────────────────
   * 세는 법은 `core/hangultype.ts` 하나뿐이다. 여기 같은 셈을 한 벌 더 두면 언젠가 한쪽만
   * 고쳐지고, 그러면 **같은 글인데 화면마다 타수가 다르게** 나온다(오늘의 타자 ↔ 유령 대결).
   * 그 어긋남은 아무도 오류로 안 보고 「내 기록이 이상하다」로만 느낀다. */
