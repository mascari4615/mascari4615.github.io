    ...
    tabs: [
      {
        ...
        build: function (container: HTMLElement): void {
          void loadNamespace('urlparse').then(function () {
          ...
          container.innerHTML = `
            ...
            <div style="display:flex; gap:6px; margin-bottom:var(--space-lg); flex-wrap:wrap;">
              ...
              <button class="btn btn-ghost" id="upCopy">${esc(t('urlparse.btn.copy'))}</button>
            </div>

            <div class="tool-list" id="upParts"></div>
            <div class="tool-status" id="upStatus">${esc(t('urlparse.status.idle'))}</div>
          `;
          ...
          function analyze(): void {
            ...
            if (!raw) {
              parts.innerHTML = '';
              status.textContent = t('urlparse.status.idle');
            ...
            try {
              ...
              parts.innerHTML = '';
              status.textContent = t('urlparse.warn.notUrl');
              status.className = 'tool-status';
              return;
            }
            const rows = [
              row(t('urlparse.row.protocol'), u.protocol.replace(':', '')),
              row(t('urlparse.row.host'), u.hostname),
              u.port ? row(t('urlparse.row.port'), u.port) : '',
              row(t('urlparse.row.path'), decodeURIComponent(u.pathname)),
              u.hash ? row(t('urlparse.row.hash'), decodeURIComponent(u.hash)) : ''
            ];
            const qs = [...u.searchParams.entries()];
            qs.forEach(([k, v]) => {
              const track = TRACKING.test(k);
              rows.push(row(`? ${k}${track ? t('urlparse.tag.tracking') : ''}`, v || t('urlparse.value.empty'), track));
            });
          ...
          $<HTMLButtonElement>('#upDecode').onclick = () => {
            try {
              ...
              status.textContent = t('urlparse.err.decode');
          ...
          $<HTMLButtonElement>('#upStrip').onclick = () => {
            try {
              ...
              analyze();
              status.textContent = drop.length ? t('urlparse.say.dropped', { n: drop.length, list: drop.join(', ') }) : t('urlparse.say.nothingToDrop');
              status.className = 'tool-status ok';
              Toolbox.trackUse?.('strip');
            } catch {
              status.textContent = t('urlparse.err.cannotStrip');
              status.className = 'tool-status error';
            }
          };
          $<HTMLButtonElement>('#upCopy').onclick = async () => {
            if (!input.value) return;
            await Toolbox.copyText?.(input.value, { message: t('urlparse.copy.done') });
          };
                  });
        }
      }
