 ...
 * ```
 *
 * 「글자수」는 **트위터 글자수·이력서 자수 제한** 때문에 보는 것이라, 사람 눈과 다르면 쓸모가 없다.
 ...
 * 낱말 수.
 *
 * 공백으로만 가른다 — **한국어에는 완벽하지 않다**(「먹었습니다」가 한 낱말). 그래도 이렇게 두는
 * 이유는, 사람이 「낱말 수」로 기대하는 것이 대개 **띄어쓰기 덩이**이기 때문이다.
 * 형태소로 가르면 숫자가 훨씬 커져서 「내가 쓴 만큼」과 안 맞는다.
 */
