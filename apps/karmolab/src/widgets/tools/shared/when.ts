...
export interface WhenParsed {
  ...
  how: string;
  /** 시각까지 말했나 (안 말했으면 그 날 0시로 둔다. 하루를 가리킨 것이다) */
  hasTime: boolean;
}
 ...
 * 내일이 무엇인지는 오늘이 언제냐에 달렸는데, 그걸 안에서 몰래 읽으면 검사가 날마다 달라진다.
 */
...
/** ISO 주차. 몇 주차는 나라마다 세는 법이 달라 **표준(ISO 8601)**으로 못 박는다. */
...
/** 며칠 남았나. **날짜 단위**로 센다(시각까지 빼면 내일이 0일 남음으로 나온다). */
 ...
 * 한 칸이라도 자는 때면 뺀다. 하나도 없으면 그럭저럭까지 받아 준다(그래도 없으면 빈손).
 */
...
export function bestHours(rows: HourRow[]): BestHours {
   ...
   * 그때 빈손으로 두면 화면이 불가능이라고 말하는 셈인데, 실제로는 **덜 나쁜 때**가 답이다 . 
   * 자는 사람이 가장 적은 칸을 짚어 준다. 없다가 아니라 이만큼은 감수해야 한다가 진짜 답. */
