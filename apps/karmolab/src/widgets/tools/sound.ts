 ...
 * 재료라, 파형이 없으면 어디가 말하는 데고 어디가 빈 데인지를 **들어 봐야만** 안다.
...
(function (): void {
  ...
  async function drawWaveform(file: File, box: HTMLElement, alive: () => boolean): Promise<string> {
     ...
     * 그것만 두면 여기서부터 들어 보기가 막힌다. 화살표 5초, Home/End, 스페이스로 듣고 멈추기.
...
.sd-wave-wrap{position:relative;border:1px solid var(--border);border-radius:var(--radius-xl);
