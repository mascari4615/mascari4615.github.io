 ...
 * 이미지다움뿐이다: 무엇을 받나, 왼쪽에 무엇을 그리나, 할 일 목록.
 *
 * 왼쪽에 그리는 것 = **사진 그 자체와 치수**. 이미지 작업의 판단 기준은 늘 지금 몇 픽셀인가,
 * 얼마나 무거운가다. 크기를 줄일지, 형식을 바꿀지가 거기서 갈린다(Squoosh 도 화면 아래에
...
(function (): void {
   ...
   *
   * Squoosh 의 값어치는 압축 자체가 아니라 **이만큼 줄였는데 눈에 보이나를 그 자리에서 판단**하게
   * 해 주는 데 있다. 가운데 손잡이를 끌어 원본과 결과를 겹쳐 본다. 숫자(용량)도 같이 붙인다 . 
   * 30% 줄었다와 눈에 안 보인다가 **같은 화면에** 있어야 결정이 선다.
   *
  ...
  function compare(box: HTMLElement, before: { url: string; size: number }, after: { blob: Blob; size: number }): void {
     ...
     * 끌어야 한다를 모르는 사람이 절반이라, 지나가다 우연히 알게 되는 편이 낫다. */
     ...
     * 있어야 하는 조작이다. 그것만 두면 얼마나 달라졌나를 볼 길이 통째로 막힌다.
  ...
  /** 눌러서 크게. 작게만 보면 이 사진이 맞나를 확인할 수가 없다. */
...
/* 잘리는 쪽 그림은 **바깥 칸의 너비**를 그대로 써야 두 장이 겹친다 . 
   50% 칸에 넣고 width:100% 를 주면 그림이 반으로 줄어 전/후가 아니라 크고 작고가 된다. */
