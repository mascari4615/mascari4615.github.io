...
(function (): void {
  ...
  Toolbox.register({
    id: 'lorem',
    title: t('widgets.lorem.title', undefined, "더미 텍스트 생성"),
    category: 'tool',
    desc: t('widgets-desc.lorem.desc', undefined, "화면 시안용 임시 글을 만듭니다. 한글 더미와 로렘 입숨, 문단·문장·단어 단위"),
    layout: 'form',
    icon: '<path d="M4 6h16M4 10h16M4 14h12M4 18h8" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/>',
    tabs: [
      {
        id: 'app',
        label: t('lorem.tab', undefined, "더미 텍스트"),
        build: function (container: HTMLElement): void {
          void loadNamespace('lorem').then(function () {

          container.innerHTML = `
            <div class="field-group">
              <div class="tool-chips" id="loLang">
                <button type="button" class="tool-chip active" data-lang="ko">${esc(t('lorem.kind.ko'))}</button>
                <button type="button" class="tool-chip" data-lang="en">${esc(t('lorem.kind.lorem'))}</button>
              </div>
            </div>
            <div class="field-group">
              <div class="tool-grid-2">
                <div>
                  <div class="tool-sublabel">${esc(t('lorem.label.unit'))}</div>
                  <select id="loUnit" aria-label="${esc(t('lorem.label.unit'))}">
                    <option value="para">${esc(t('lorem.opt.para'))}</option>
                    <option value="sent">${esc(t('lorem.opt.sent'))}</option>
                    <option value="word">${esc(t('lorem.opt.word'))}</option>
                  </select>
                </div>
                <div>
                  <div class="tool-sublabel">${esc(t('lorem.label.count'))} <span id="loCountVal" class="range-value">${esc(t('lorem.value.count'))}</span></div>
                  <input type="range" id="loCount" aria-label="${esc(t('lorem.label.count'))}" min="1" max="20" value="3">
                </div>
              </div>
            </div>
            <div style="display:flex; gap:6px; margin-bottom:var(--space-lg); flex-wrap:wrap;">
              <button class="btn btn-primary" id="loGen">${esc(t('lorem.btn.loGen'))}</button>
              <button class="btn btn-ghost" id="loCopy">${esc(t('lorem.btn.loCopy'))}</button>
            </div>
            <textarea id="loOut" aria-label="${esc(t('lorem.aria.out'))}" rows="10" spellcheck="false" readonly></textarea>
            <div class="tool-status" id="loStatus">${esc(t('lorem.note.ko'))}</div>
          `;
          ...
          function run(): void {
            ...
            status.textContent = t('lorem.status.count', {
              n: out.value.length.toLocaleString(locale()),
              note: korean ? t('lorem.note.ko') : t('lorem.note.latin')
            });
          ...
          $<HTMLButtonElement>('#loCopy').onclick = () => {
            if (out.value) void Toolbox.copyText?.(out.value, { message: t('lorem.copy.done') });
          };
          run();
                  });
        }
