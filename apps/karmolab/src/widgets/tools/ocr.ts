...
(function (): void {
  ...
  function draw(container: HTMLElement): void {
    ...
    container.innerHTML = `
      ...
      <div id="ocRoute" style="display:none; padding:10px; border-radius:var(--radius-xl); background:var(--bg-tertiary); margin-bottom:10px;"></div>
      <div style="display:flex; gap:16px; flex-wrap:wrap;">
        <div>
          <div class="tool-sublabel">${esc(t('ocr.label.prepared'))}</div>
          <canvas id="ocCanvas" style="border-radius:var(--radius-lg); border:1px solid var(--border); max-width:100%;"></canvas>
        </div>
        <div style="flex:1; min-width:240px;">
          ...
          <button class="btn btn-ghost" id="ocCopy" style="margin-top:var(--space-sm);">${esc(t('ocr.btn.copy'))}</button>
        </div>
      </div>
      <div class="tool-status" id="ocStatus">${esc(t('ocr.status.idle'))}</div>
      <p class="tool-hint">${esc(t('ocr.note.limits'))}</p>
    `;
