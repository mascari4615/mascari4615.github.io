/**
 * 작업물 보기. 소개 장 안의 한 절 (2026-09-08 사용자 결정: 나에 대한 것은 /about/ 한 장).
 * 예전엔 `/works/` 위젯 하나. 이제 `about.ts` 가 `#aboutWorks` 자리에 `mountWorks` 로 부착
 *
 * 보기 둘을 **같은 자료 한 벌**로 그린다 (사용자 확정 2026-08-28: 내가 보기엔 지도, 남이
 * 보기엔 진열):
 *   ① 진열. 지금 붙들고 있는 것이 큰 칸, 나머지는 연도별 그림 벽. 처음 온 사람이 읽는 순서.
 *   ② 지도. 가로가 시간, 가로줄이 소속. 몇 해씩 이어 온 것과 한 번 스친 것의 구분
 *
 * 원료는 `data/works.json` (배포 산출, `gen-post-pages.mjs`). 메타 정본은 **각 글의
 * frontmatter `work:`**, 글이 없는 바깥 링크만 `apps/blog/_data/works.yml` 소관
 * 그 외 참여 18건은 소품 목록 글에서 읽어 온 것. 예전에는 카드 한 장 뒤에 숨어 있던 것
 *
...
Mdd.injectCSS(
    ...
    .wk-seg { display:flex; border:1px solid var(--border); border-radius:var(--radius-pill); overflow:hidden; }
    .wk-seg button { background:none; border:0; color:var(--text-secondary); font-size:var(--font-size-2xs);
    ...
    .wk-bar input { flex:1; min-width:150px; background:var(--bg-secondary); border:1px solid var(--border);
        border-radius:var(--radius-sm, 8px); color:var(--text-primary); padding:7px 12px; font-size:var(--font-size-2xs); }
    ...
    .wk-now a { position:relative; display:block; aspect-ratio:16/10; border-radius:var(--radius-xl); overflow:hidden;
    ...
    .wk-now b { display:block; font-size:var(--font-size-sm); color:var(--text-primary); }
    .wk-now span { font-size:var(--font-size-2xs); color:var(--text-secondary); font-family:var(--font-mono, ui-monospace, monospace); }
    .wk-now em { position:absolute; top:10px; left:10px; font-style:normal; font-size:var(--font-size-3xs); color:var(--text-primary);
        background:var(--bg-secondary); border:1px solid var(--border); border-radius:var(--radius-pill); padding:3px 9px; }
    ...
    .wk-tile { position:relative; aspect-ratio:1; border-radius:var(--radius-xl); overflow:hidden; background:var(--bg-tertiary);
    ...
    .wk-tile .cap { position:absolute; inset:auto 0 0 0; padding:18px 8px 7px; font-size:var(--font-size-3xs); line-height:1.35;
        background:linear-gradient(transparent, var(--bg-primary)); opacity:0; transition:opacity .18s; color:var(--text-primary); }
    .wk-tile:hover .cap { opacity:1; }
    .wk-tile .cap i { display:block; font-style:normal; color:var(--text-secondary); font-size:var(--font-size-4xs); }
    .wk-tile .pin { position:absolute; top:6px; right:6px; width:7px; height:7px; border-radius:50%; }

    /* ── 지도 ── */
    .wk-map { border:1px solid var(--border); border-radius:var(--radius-xl); padding:14px 16px 8px; background:var(--bg-primary); }
    ...
    .wk-lane .who { font-size:var(--font-size-2xs); color:var(--text-tertiary); padding-right:10px; }
    .wk-lane .who b { display:block; color:var(--text-primary); font-size:var(--font-size-2xs); }
    .wk-track { position:relative; height:46px; }
    .wk-bar2 { position:absolute; top:11px; height:24px; border-radius:var(--radius-xl); display:flex; align-items:center;
        gap:7px; padding:0 10px; font-size:var(--font-size-2xs); white-space:nowrap; color:var(--text-primary);
        border:1px solid currentColor; cursor:pointer; overflow:hidden; }
    .wk-bar2 span { overflow:hidden; text-overflow:ellipsis; }
    .wk-bar2 img { width:16px; height:16px; border-radius:var(--radius-sm); object-fit:cover; }
    ...
    .wk-pick { display:grid; grid-template-columns:150px 1fr; gap:14px; margin-top:12px; border:1px solid var(--border);
        border-radius:var(--radius-xl); overflow:hidden; background:var(--bg-secondary); min-height:104px; }
    ...
    .wk-pick h3 { margin:0 0 4px; font-size:var(--font-size-sm); }
    .wk-pick .sub { font:12px var(--font-mono, ui-monospace, monospace); color:var(--text-tertiary); }
    .wk-pick .none { padding:16px; color:var(--text-tertiary); font-size:var(--font-size-2xs); grid-column:1 / -1; }

    /* ── 그 외 참여 ── */
    .wk-tail { border-top:1px solid var(--border); padding-top:14px; }
    .wk-tail h3 { font-size:var(--font-size-xs); margin:0 0 8px; color:var(--text-secondary); font-weight:600; }
    ...
    .wk-tail li { font-size:var(--font-size-2xs); color:var(--text-secondary); }
    .wk-tail li b { color:var(--text-primary); font-weight:500; }
    .wk-tail li i { font-style:normal; color:var(--text-tertiary);
        font-family:var(--font-mono, ui-monospace, monospace); font-size:var(--font-size-3xs); margin-right:6px; }
...
function renderMap(host: HTMLElement, works: WorkRow[]): void {
    ...
    const to = Math.max(...yearsAll, today.getFullYear()) + 1;
    ...
    host.innerHTML =
        `<div class="wk-map-scroll" tabindex="0" role="region" aria-label="${esc(t('works.viewMap', undefined, '연표'))}"><div class="wk-map"><div class="wk-ruler"><div></div><div class="yy" id="wkYears" ` +
        `style="grid-template-columns:repeat(${span},1fr)"></div></div><div id="wkLanes"></div></div></div>` +
    ...
    for (const name of lanesOf) {
        ...
        for (const w of mine.slice().sort((a, b) => a.at.localeCompare(b.at))) {
            if (!w.at) continue;
            const left = at(w.at);
            const end = w.period?.includes('~')
                ? at(w.period.split('~')[1].trim() || currentMonth)
                : null;
            const el = document.createElement('button');
            el.type = 'button';
            el.title = `${w.title}, ${w.period ?? w.at}`;
            el.setAttribute('aria-label', el.title);
            const width = Math.min(Math.max(end === null ? 0 : end - left, 6), 100 - left);
            let row = rowEnds.findIndex((edge) => left >= edge + 1);
            if (row < 0) row = rowEnds.length;
            rowEnds[row] = left + width;
            el.style.top = `${11 + row * 32}px`;
            el.style.left = `${left}%`;
            ...
            el.addEventListener('focus', () => pick(w));
            el.addEventListener('click', () => pick(w));
            track.appendChild(el);
        }
        /* 줄이 늘면 **레인 자체가 커져야 한다**. 안 그러면 넘친 막대가 아래 레인을 덮는다. */
        const rows = Math.max(rowEnds.length, 1);
        if (rows > 1) {
            track.style.height = `${16 + rows * 32}px`;
    ...
    const pick = (w: WorkRow): void => {
        (host.querySelector('#wkPick') as HTMLElement).innerHTML =
            (w.image ? `<img src="${esc(w.image)}" alt="">` : '<div></div>') +
            `<div class="txt"><h3><a href="${esc(w.url)}"${w.slug ? '' : ' target="_blank" rel="noopener"'}>${esc(w.title)} ↗</a></h3>` +
...
function build(container: HTMLElement, data: Payload): void {
    ...
    wrap.innerHTML =
        `<div class="wk-bar">
            <div class="wk-seg">
                <button type="button" data-view="shelf" class="on" aria-pressed="true">${esc(t('works.viewShelf', undefined, '진열'))}</button>
                <button type="button" data-view="map" aria-pressed="false">${esc(t('works.viewMap', undefined, '연표'))}</button>
            </div>
    ...
    for (const button of wrap.querySelectorAll<HTMLButtonElement>('.wk-seg button')) {
        button.addEventListener('click', () => {
            ...
            wrap.querySelectorAll('.wk-seg button').forEach((el) => el.setAttribute('aria-pressed', String(el === button)));
...
/** 소개 장의 자리에 작업물 보기를 붙인다. */
export function mountWorks(container: HTMLElement): void {
