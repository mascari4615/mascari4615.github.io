...
export function shortcutsHtml(esc: (value: unknown) => string): string {
  const groups = SHORTCUTS.map((group) => `<section><h5>${esc(group.title)}</h5>${group.items.map((item) => `<p><span class="ks-keys">${item.keys.map((key) => `<kbd>${esc(key)}</kbd>`).join('')}</span><span>${esc(item.what)}</span></p>`).join('')}</section>`).join('');
  return `<div class="ks-help" role="dialog" aria-modal="true" aria-label="단축키"><div class="ks-help-head"><strong>단축키</strong><button class="ks-btn" data-help-act="close">닫기</button></div><div class="ks-help-body">${groups}</div></div>`;
}
