 ...
 *
 * 이름은 거푸집의 본이다. 손으로 한 장씩 그리는 도구가 아니라 **숫자를 돌려 찍어내는** 틀이다.
 *
 * 먹과 무엇이 다른가: 먹은 픽셀 판(`Surface`)을 들고, 확대하면 깨진다. 여기는 **도형 노드**를
 ...
 * 화면이 채우기, 테두리를 만질 때 이걸로 먼저 걸러야 무리에 없는 자리를 안 건드린다.
 */
...
/** 레이어 자리 옮기기. 배열 끝이 위에 그려지므로 `+1` 이 앞으로다. */
...
/** 도형 개수. 무리 안까지 센다(검사에서 정말 들어갔나를 볼 때 쓴다). */
