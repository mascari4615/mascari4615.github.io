...
| 파일 | 무엇 |
| --- | --- |
| `karmomap.ts` | 위젯 본체 — 툴바·오른쪽 패널 8종(노드 / 선 / 묶음 / 용어 / 거르기 / 관계망 / 저장 / 도움말)·발표 모드 |
...
| 열쇠 | 무엇 |
| --- | --- |
| `karmomap.index` | 맵 목록 + 지금 열린 맵 |
| `karmomap.map.<id>` | 맵 한 장 (`GraphSpec`) |
| `karmomap.prev.<id>` | 그 맵의 **직전 판** — 사고 났을 때 되살리는 한 판 |
| `karmomap.terms` | 내 용어 (사람 단위, 모든 맵 공용) |

## 검사

```bash
cd apps/karmolab && npm run dev          # 먼저 띄우고
npm run smoke:karmomap                   # 34항목 화면 검사
```
