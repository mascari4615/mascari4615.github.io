...
export function renderStoragePanel(ctx: PanelCtx): void {
  ...
  side.innerHTML = `
    <h4>${esc(t('karmomap.t261'))}</h4>
    <div class="km-hint">${esc(t('karmomap.t262'))}</div>
    <div class="km-field">
    ...
    <div class="km-field">
      <label>${esc(t('karmomap.t263'))}</label>
    ...
    <button class="btn btn-ghost" data-km="st-prev">${esc(t('karmomap.t264'))}</button>
    <div class="km-hint">${t('karmomap.hint11', { em: `<b>${esc(t('karmomap.t266'))}</b>` })}</div>
    <button class="btn btn-primary" data-km="st-backup">${esc(t('karmomap.t268'))}</button>
    <button class="btn btn-ghost" data-km="st-restore">${esc(t('karmomap.t269'))}</button>
    <div class="km-hint">${t('karmomap.hint12', { em: `<b>${esc(t('karmomap.t271'))}</b>` })}</div>
    <button class="btn btn-ghost" data-km="st-close">${esc(t('karmomap.t273'))}</button>`;
