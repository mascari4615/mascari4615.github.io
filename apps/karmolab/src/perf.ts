 ...
 *
 * 왜 있나: 요즘 느린 것 같다를 고치려면 **무엇이** 느린지가 먼저 있어야 하는데, KarmoLab 에는
 ...
 * 받는 시간을, 어떤 것은 그리는 시간을 로딩이라 부르게 된다. 셸(`toolbox.ts`)이 이미
...
(function (): void {
   ...
   *
   * 긴 작업(longtask)은 50ms 넘게 뭔가 했다까지만 말한다. **누가** 했는지는 거의 안 준다.
   * LoAF 는 프레임 단위로 보고 **어느 파일의 어느 함수**가 몇 ms 를 썼는지, 그중 입력을 막은
   * 시간이 얼마인지, 스타일, 레이아웃에 얼마가 갔는지를 준다. 긴 작업 3건과
   * `chat.js` 의 `tick` 이 프레임마다 40ms는 고칠 수 있느냐 없느냐의 차이다.
   *
   * 크로미움 전용(123+)이다. 없는 브라우저에서는 지금까지대로 긴 작업만 보인다 . 
   * 그 자리는 0건이 아니라 못 잼으로 적는다.
   */
   ...
   * 한 번의 조작 (TASK-KL-201 ②).
   *
   * 굼뜨다는 **셋 중 하나**다. 이 셋을 안 가르면 고칠 곳을 못 찾는다:
  ...
  /** 긴 작업 관찰자가 아예 없는 브라우저(사파리)와 하나도 없었다를 가른다. */
  let longTaskSupported = false;
  let lcpMs: number | null = null;
  /** 큰 그림은 부팅 지표다. 첫 손가락 또는 첫 화면 전환에서 끝낸다. */
  let lcpFrozen = false;
  let pageSwitches = 0;
  /**
   * 화면이 밀린 사건 하나 (TASK-KL-201 ⑥. Layout Instability).
   *
   * 눌렀는데 딴 게 눌렸다의 정체다. 총점(CLS)만 있으면 고칠 수가 없다. **무엇이** 밀었는지가
  ...
  function mark(name: string): void {
    ...
    /* 화면을 갈아 끼우면 제일 큰 그림이 그 시점으로 밀린다. 부팅 지표가 아니라
    ...
    if (name.indexOf('page:') === 0) {
      /* **첫** page: 는 부팅의 일부다. 셸이 첫 화면을 세우면서 부른다. 그걸로 얼려 버리면
         큰 그림이 영영 안 정해진다(실측: 통째로 못 잼이 됐다). 두 번째 전환부터 끝이다. */
      pageSwitches += 1;
  ...
  try {
    ...
    /* 사파리엔 없다. 0건이 아니라 못 잼이다 (아래 snapshot 이 그렇게 적는다). */
  }
    ...
    const shiftObserver = new PerformanceObserver((list) => {
      for (const item of list.getEntries()) {
        ...
        shifts.push({
          ...
          /* 노드는 곧 사라진다. 나중에 읽으면 전부 모름이 된다. 지금 굳힌다. */
      ...
      for (const item of list.getEntries()) {
        ...
        slowFrames.push({
          ...
          /* renderStart 가 0 이면 **그리기를 아예 안 한 프레임**이다. 그때의 길이 - 0 은
             프레임 전체가 되어 그리기가 전부였다로 읽힌다. 그건 거짓이므로 0 으로 둔다. */
  ...
  try {
    const eventObserver = new PerformanceObserver((list) => {
      for (const item of list.getEntries()) {
        ...
        /* `interactionId` 가 0 인 것은 **사용자 조작이 아니다**(스크롤 등). 그것까지 세면
           제일 굼뜬 조작이 사람이 만진 적 없는 것으로 채워진다. */
    ...
    /* 안 되면 0건이 아니라 못 잼. snapshot 이 그렇게 적는다. */
  }
   ...
   *
   * 그래서 표시 40ms = 느리다는 판정은 틀렸다. 같은 환경의 하한과 비교해야 한다.
   * 하한은 **처리 시간이 거의 0 인 조작들의 중앙값**으로 잡는다. 그건 아무 일도 안 한
   * 클릭이고, 그 값이 곧 이 환경의 바닥이다. 자료가 적으면 `null` (섣불리 정하지 않는다).
   */
  ...
  interface ResourceRow {
    ...
    /* 받는 데 200ms가 회선이 느려서인지, 서버가 늦게 답해서인지, 이름 찾기에서 샌 건지
      . 총량만으로는 못 가른다. 단계로 쪼개야 고칠 곳이 정해진다 (TASK-KL-201 ⑤).
       크로스오리진은 `Timing-Allow-Origin` 이 없으면 **전부 0** 으로 온다 → 그건 캐시가
       아니라 안 알려 줌이다. null 로 둔다. */
    dnsMs: number | null;
  ...
  function resources(): ResourceRow[] {
    ...
    return list.map((item) => {
      ...
      /* 단계값이 통째로 0 이면 빨랐다가 아니라 안 알려 줬다다 (크로스오리진 + TAO 없음).
   ...
   *
   * DevTools 의 Coverage에 해당하는 자리다. 브라우저는 이 줄이 실행됐나를 페이지 쪽에
   * 안 알려 준다(그건 개발자 도구 전용 통로다). 그래서 우리가 아는 것으로 근사한다:
   * **받은 위젯 묶음** 중 `runBuild` 를 한 번도 안 지난 것 = 이번에 화면에 안 나온 코드다.
   *
   * 낭비라고 단정하지 않는다. 곧 누를 위젯일 수도 있다. 다만 **부팅에 딸려 온 것**이
  ...
  function unusedWidgetCode(): { bytes: number | null; rows: Array<{ id: string; bytes: number | null; atBoot: boolean }> } {
    ...
    /* **받은 것에서 출발한다.** 위젯 목록에서 출발하면 부팅에 딸려온 것을 통째로 놓친다 . 
       그건 지연 로더를 안 거쳐서 우리 목록에 아예 없다(실측: 그래서 늘 전부 썼다가 나왔다).
  ...
  /** 도메인별 합계 (TASK-KL-201 ⑤. Lighthouse 의 서드파티 요약에 해당). */
   ...
   * 첫 그림이 없다로 남는다 (실제로 재기 전에 재면 그렇게 된다).
   */
  let bootLogged = false;
  /**
   * 부팅 낭비를 **그 시점 값으로 굳힌다** (TASK-KL-201 ⑬).
   *
   * 받았는데 안 쓴 코드는 시간이 갈수록 줄어든다. 사람이 그 위젯을 누르면 쓰인 것이 되니까.
   * 그래서 지금 재면 언제 재느냐에 따라 답이 달라진다. **부팅 낭비는 부팅 직후의 값**이다.
   * 예산도 이 굳은 값으로 판정해야 누가 많이 눌러 본 판이 저절로 합격하는 일이 없다.
   */
  ...
  function snapshot(): Record<string, unknown> {
    ...
    return {
      ...
      /* 굼뜬 순. 못 재는 브라우저(사파리 등)는 `null`. 조작이 다 빨랐다와 다르다. */
   ...
   *
   * 한 번 재고 이번 판이 빨라졌다는 말은 못 한다. 같은 코드도 기계가 바쁘면 두 배가 난다.
   * 그래서 판마다 중앙값(p50)과 나쁜 쪽(p75)을 **샘플 수와 함께** 낸다. 표본이 2~3개면
   * 숫자를 보여 주되 아직 못 믿는다는 뜻으로 수를 같이 읽게 한다.
   *
   ...
   * 예산은 지금보다 조금 나빠지는 것이 아니라 사람이 답답해지는 선에 둔다).
   */
  ...
  /**
   * 예산 대조 결과. **못 잰 것은 통과로 세지 않는다**. 합격과 검사 못 함을 같은 칸에
