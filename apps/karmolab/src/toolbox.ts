 ...
 */
// @ts-expect-error — redeclared as const IIFE; var Toolbox declared in global.d.ts
const Toolbox = (() => {
    ...
    function closeAllHeaderNav() {
        ...
        document.querySelectorAll('.header-nav-group.is-open').forEach((wrap) => {
            ...
            const p = wrap.querySelector('.header-nav-panel') as HTMLElement | null;
            if (p) p.hidden = true;
        });
    }
    ...
    function closeAllHeaderNavExcept(except: any) {
        document.querySelectorAll('.header-nav-group.is-open').forEach((w) => {
            ...
            if (tr) tr.setAttribute('aria-expanded', 'false');
            const p = w.querySelector('.header-nav-panel') as HTMLElement | null;
            if (p) p.hidden = true;
        });
    }
    ...
    function setNotifyInvokeDebugPayload(payload: any) {
        ...
        try {
            ...
            const det = document.querySelector('.karmolab-notify-debug') as HTMLDetailsElement | null;
    ...
    function mirrorToastToDesktop(msg: any, type: any, detailText: any) {
        ...
        const payload: { title: string; body: string; sound?: string } = { title: title.slice(0, 120), body: body || 'KarmoLab' };
    ...
    function init() {
        ...
        if (headerNav) {
            ...
            document.addEventListener('click', (e) => {
                if (!(e.target as Element | null)?.closest('.header-nav')) closeAllHeaderNav();
            });
    ...
    function switchPage(pageId: any, opts: { pushHistory?: boolean } = {}) {
        closeAllHeaderNav();
    ...
    function showToast(msg: string, type = 'success', detail?: unknown) {
        ...
        if (hasDetail && detailText) {
            ...
            t.onclick = null;
            const copyBtn = t.querySelector('.status-toast-copy') as HTMLElement | null;
            if (copyBtn) {
                copyBtn.onclick = function (ev: any) {
                    ev.stopPropagation();
    ...
    function showLightbox(imageUrl: any) {
        let overlay = document.querySelector('.tb-lightbox-overlay') as HTMLElement | null;
        if (!overlay) {
    ...
    function field(container: any, { tag = 'textarea', id, label, placeholder, type, topRight, mono }: { tag?: string; id: any; label: any; placeholder?: any; type?: any; topRight?: any; mono?: any }) {
    ...
    function button(container: any, { text, onclick, style }: { text: any; onclick: any; style?: any }) {
        const btn = document.createElement('button');
    ...
    function select(container: any, { id, label, options, onChange }: { id: any; label: any; options: any; onChange?: any }) {
        const g = document.createElement('div');
        g.className = 'field-group';
        const lbl = document.createElement('label');
        lbl.className = 'field-label'; lbl.htmlFor = id; lbl.textContent = label;
    ...
    function setPrismTheme(themeId: string, silent = false) {
        ...
        const oldLink = document.getElementById('prism-css') as HTMLLinkElement | null;
    ...
    function getUsageStats() {
        try { return JSON.parse(localStorage.getItem(USAGE_KEY) ?? 'null') || {}; }
        catch (_) { return {}; }
    }
    ...
    function completeAchievement(id: any, meta: { title?: string } & Record<string, any> = {}) {
        const data = getUserData();
        if (!data.achievements) data.achievements = [];
        if (data.achievements.includes(id)) return false;
        data.achievements.push(id);
        saveUserData(data);
        const title = meta.title || (ACHIEVEMENT_REGISTRY as any)[id]?.title || id;
        showToast('도전과제 달성: ' + title, 'success');
        return true;
    }

    function unlockBadge(id: any, meta: { title?: string } & Record<string, any> = {}) {
        const data = getUserData();
        if (!data.badges) data.badges = [];
        if (data.badges.includes(id)) return false;
        data.badges.push(id);
        saveUserData(data);
        const title = meta.title || (BADGE_REGISTRY as any)[id]?.title || id;
