 ...
 * 있나를 안 알려 주지만 시간대는 알려 준다(Asia/Seoul → 한국). 위치 권한을 안 물어도 되고,
 ...
 * 지금 열린 도구만 살짝 고치면 안 바뀐 자리가 남아 반만 바뀐 화면이 된다.
 */
