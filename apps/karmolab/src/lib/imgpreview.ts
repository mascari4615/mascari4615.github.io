 ...
 * 바꾼 뒤에 「너무 뭉갰네」를 알면 처음부터 다시 해야 한다.
 *
 ...
 * 빈 자리를 검게 채우고, 사람은 그 검은 띠를 「압축이 망가뜨린 것」으로 읽는다.
 */
...
/** 얼마나 줄었나. 0 바이트 원본은 나눌 수 없으니 「같다」로 둔다(모르는 것을 지어내지 않는다). */
 ...
 * 다만 이건 어림이라 화면이 「어림」이라고 말해야 한다(정확한 수는 바꿔 봐야 안다).
 */
