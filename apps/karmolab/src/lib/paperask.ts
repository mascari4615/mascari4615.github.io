 ...
 *   말을 만들어 얹으면 「논문이 그렇게 말했다」로 읽히는데, 그건 우리가 책임질 수 없는 말이다.
 *   (AI 로 다듬는 자리는 화면 쪽에 따로 둔다 — 그때도 원문 문장을 같이 보여 준다.)
 *
 * 고르는 셈법: 물음의 낱말이 몇 개 겹치나 + 그 문장이 결론 자리인가(뒤쪽·「we show/결론」류) +
...
/** 결론에 자주 붙는 말 — 이 문장이 「그래서 뭔데」에 가깝다는 신호다. */
const CONCLUSIVE = [
 ...
 * 「관련 논문 8편」이라는 숫자가 부풀고, 사람은 그 숫자를 믿는다.
 */
