...
export interface Match {
  win: string;
  lose: string;
  /** 몇 강에서 붙었나 (16 = 16강). 내가 고른 길이 이걸로 선다. */
  round: number;
}
 ...
 * 한 판의 진행 상태.
 *
 * 왜 클래스가 아니라 값인가: 화면은 지금 누구와 누구냐만 물으면 되고, 상태를 두 곳에서
...
export class Bracket {
  ...
  /** 이번 라운드에서 몇 번째 대결인가 (화면이 3 / 8로 쓴다). */
 ...
 *
 * 같은 갈림길만 센다: 같은 라운드에서 **같은 둘**이 붙은 대결. 다른 대진에서 나온 판을
