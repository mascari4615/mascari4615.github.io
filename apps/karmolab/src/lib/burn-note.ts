...
/** 잠근 것이 파일 봉투인지 그냥 글인지 가르는 표. 안쪽에만 있다. */
const FILE_TAG = '\0karmolab-file\0';

export function packFile(name: string, type: string, bytes: Uint8Array): string {
  return FILE_TAG + JSON.stringify({ name, type, data: bytesToBase64Url(bytes) } satisfies FilePack);
}

/** 푼 것이 파일 봉투면 그 안을, 아니면 null. */
export function unpackFile(text: string): { name: string; type: string; bytes: Uint8Array<ArrayBuffer> } | null {
  if (!text.startsWith(FILE_TAG)) return null;
  try {
    const p = JSON.parse(text.slice(FILE_TAG.length)) as FilePack;
    if (!p || typeof p.data !== 'string') return null;
    return { name: String(p.name || 'file'), type: String(p.type || ''), bytes: base64UrlToBytes(p.data) };
  } catch {
    return null;
  }
}
