 ...
 * 어느 날 조용히 다른 코드가 실려 오고, 그게 로컬 AI 에서는 모델이 안 올라간다로 나타난다.
 *
 * 이 파일에는 **모델도 추론도 없다.** 데려오기만 담는다. 그래야 이 자리에서 틀릴 수 있는
 ...
 * 무엇을 권할지를 스스로 판단하지 않게 하려는 것이다. 판단은 한 곳에만 있어야 한다.
 *
 * `load`, `nav` 를 인자로 받는 이유는 시험 때문만이 아니다. Node 에서 `globalThis.navigator`
 * 는 **바꿀 수 없는 값**이라(getter 전용), 밖에서 못 넣으면 GPU 없는 자리를 아예 못 재 본다.
