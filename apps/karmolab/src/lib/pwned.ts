 ...
 * 그 순간 이 도구는 비밀번호를 남에게 물어보는 도구가 된다.
 *
...
/** 접두사 길이. 이 값이 곧 서버가 얼마나 모르는가다. 짧을수록 더 모른다. */
 ...
 *
 * 한 줄이 `접미35글자:횟수` 다. 못 찾으면 0. 목록에 없다는 뜻이지 안전하다가 아니다.
 */
...
export interface PwnedResult {
  ...
  count: number;
  /** 서버가 이번에 돌려준 해시 개수. 서버는 이 중 어느 것인지 모른다의 증거 */
  amongst: number;
 ...
 * 화면이 무엇을 보냈는지를 사람에게 그대로 보일 수 있다.
 */
