 ...
 * 전부 「이 컴퓨터의 어디에 소스가 있나」를 알아야 도는 일이다.
 *
 * 왜 한 곳으로 모았나: 여태 이 값은 **서버 모니터 위젯 안에만** 살았다. 그래서 설치
 * 위젯에서 「설치」를 누르면 「저장소 루트를 먼저 설정하세요」가 떴다 — 무엇을 어디서
 ...
 *
 * 「눌러 보고 알게 되는 실패」를 없애는 자리다 — 틀린 경로를 넣으면 그 자리에서 말한다.
 */
export async function setWorkFolder(path: string): Promise<{ ok: true; path: string } | { ok: false; why: string }> {
    ...
    /* 「폴더가 있다」와 「그 소스가 맞다」는 다르다. 표가 되는 파일을 실제로 읽어 본다 —
