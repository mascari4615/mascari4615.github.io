 ...
 *
 * 화면이 없다. 「어디에 묻고 · 무엇을 들고 있고 · 사람에게 어떻게 말할지」만 담는다.
 *
...
/** 고를 수 있는 수명. 「분」으로 말한다 — 사람이 세는 단위다. */
export const TTL_CHOICES = [10, 30, 60];

/** 뒷단이 편지를 받을 준비가 됐나. 못 물으면 `null` — 「안 켜졌다」와 「못 물었다」는 다르다. */
...
export function recall(now = Date.now()): Mailbox | null {
  try {
    ...
    /* 이미 지난 주소를 되살리면 「편지가 안 온다」로 보인다 — 지난 건 없는 것과 같다. */
...
/**
 * 남은 시간을 사람 말로. **0 초는 「곧」이 아니라 「끝」**이라고 말해야 한다 —
 * 「곧 사라집니다」가 몇 분째 떠 있으면 그건 시계가 아니라 장식이다.
 */
