...
export interface AiContext {
  ...
  modelCached: boolean;
  /** 사용자가 키가 있어도 로컬로를 골랐나. 기본은 원격 선호. */
  preferLocal?: boolean;
...
export interface AiDecision {
  route: AiRoute;
  /** 사람에게 보여 줄 한 줄. 왜 이 길인가가 안 보이면 사용자는 고칠 수가 없다. */
  why: string;
}
 ...
 *   것이라 입력을 줄이는 것과 무관하다. 그런데 흔한 대응이 작게 해서 다시라, 몇 번을
 *   더 실패하고 나서야 원인을 본다.
 * - **총 VRAM 이 넉넉하다는 근거가 안 된다.** 지금 *비어 있는* 양이 문제다.
...
export function explainFailure(stage: AiFailStage, detail = ''): AiFailure {
  ...
  switch (stage) {
    ...
    case 'load':
      return {
        stage,
        /* 여기서 작게 해서 다시를 권하지 않는다. 적재 실패는 입력 크기와 무관하다. */
        say:
...
/** 받기 전에 **얼마나 걸리는지** 먼저 말한다 (12 § 2). 숫자 없이 잠시만은 안 된다. */
