 ...
 *
 * 왜 있나: 도구 129장이 전부 열어서 손으로 넣어야만 쓸 수 있었다. 계산해 놓고 남에게
 * 보내려면 결과를 복사해 붙이는 수밖에 없었고, 이 값으로 다시 열어 봐가 불가능했다.
...
function coerce(name: string, type: FieldType, rawValue: string): ArgValue {
  switch (baseType(type)) {
    ...
    case 'boolean':
      // 주소에 `&urlSafe` 만 적은 경우(값 없음)도 켬으로 본다. 손으로 링크를 쓸 때 자연스럽다.
...
/**
 * 반대로. 지금 상태를 링크로 만든다.
 * `spec` 에 없는 칸을 넣으면 던진다. 조용히 빠뜨리면 받는 쪽에서 값이 빠졌다로 보여 원인을 못 찾는다.
 */
