 ...
 *
 * 그때 없으면 그냥 한 번 그리고 끝으로 두면 화면이 **영영 그 모습으로 남는다**:
 * 남이 만든 도구는 목록을 요청조차 안 한 채 비어 있었고(실측: `kl/tools/user` 요청 0건),
 * 흐름도 같은 꼴이었다. 둘 다 로그인한 사람에게 로그인하라고 말하는 화면이 된다.
 *
 ...
 *
 * 약속:
 *  , 상태가 정해졌을 때만 부른다 (`loading` 중에는 안 부른다)
