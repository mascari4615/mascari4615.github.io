/**
 * lib/karmograph/canvas-decor.ts. **어떻게 보일지 정하는 순서** (TASK-KL-202 방향① 해체 16조각).
 *
 * 색, 크기를 정하는 근거가 넷이나 된다(규칙 목록, 칸 색, 꼬리표 색, 종류 색). 순서가 흐려지면
 * 분명 빨갛게 해 뒀는데 파랗다가 생긴다. 그래서 **이기는 순서**를 이 파일 하나에 못 박는다:
 *
 *   ① 꾸미기 **규칙**(사람이 이런 것은 이렇게라고 직접 적은 것)
 *   ② 칸 값으로 물들이기(방금 고른 기준)
 *   ③ 꼬리표 색
 *   ④ 종류 색(기본)
 *
 * 크기도 같은 결: 규칙의 배율은 연결 수만큼 크게와 **곱해진다**(둘 다 사람이 켠 것이라 하나만 이기면 놀란다).
 */
