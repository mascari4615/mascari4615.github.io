/**
 * lib/karmograph/canvas-dom.ts. 캔버스 **뼈대 만들기** (TASK-KL-202 방향① 해체 13조각).
 *
 * 여기서 정해지는 것은 무엇을 그리나가 아니라 **어디에 그리나**다: svg 한 장, 그 안의 층 셋,
...
export function buildCanvasDom(
  ...
  theme: { minimapBg: string; minimapBorder: string; edgeDefaultColor: string },
    ...
    // touch-action:none. 안 주면 브라우저가 손가락 끌기를 페이지 스크롤로 먼저 먹어서
