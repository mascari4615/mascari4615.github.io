/**
 * lib/karmograph/canvas-card.ts. **보통 카드의 속**: 이름 + 한마디 + 칸 줄 (TASK-KL-202 방향① 해체 12조각).
 *
 * 자리 규칙 두 개가 여기 산다:
 *  - 얼굴이 있으면 글은 그 오른쪽에서 시작한다(동그라미 카드는 가운데 정렬).
 *    ★ 얼굴이 있나는 `canvas-avatar.ts` 만 안다. 얼굴을 안 정한 노드에도 이름 첫 글자 원이
...
export function buildCardText(
  ...
  // 카드 이름은 **1:1 에서 읽히는 크기**여야 한다. 전체 보기가 100% 를 안 넘게 된 뒤로
