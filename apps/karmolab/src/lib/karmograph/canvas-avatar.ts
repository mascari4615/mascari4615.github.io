/**
 * lib/karmograph/canvas-avatar.ts. 노드의 **얼굴** (TASK-KL-202 방향① 해체 4조각).
 *
 * 관계도에서 누구인지는 글자보다 얼굴로 읽힌다. 얼굴은 셋 중 하나다: 이모지, 색 원, 사진.
 ...
 * 글 쪽은 `node.avatar` 가 있을 때만 얼굴이 있다고 알고 있었다. 그런데 여기는 얼굴을
 ...
 * 첫 글자는 누구는 말해도 무엇(인물, 장소, 사건)은 말하지 않는다. 종류가 색으로만
