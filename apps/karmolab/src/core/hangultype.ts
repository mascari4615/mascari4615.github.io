 ...
 *
 * 한국어 타자 속도를 **영어처럼 세면 틀린다.** 영어는 흔히 낱말 수 × 5로 재는데,
 * 한글은 글자 하나가 자소 둘~넷을 눌러야 나온다. 값도 왔도 네 번이다.
 ...
 * 두 벌이 되면 연습에서는 400타인데 데일리에서는 250타가 되고, 둘 중 무엇도 못 믿는다.
 */
  ...
  ops: {
    count: {
      desc:
        ...
        ' / 한글 타수(자소 기준)를 센다. 낱말 수 × 5로 세면 절반쯤 낮게 나온다.',
      in: { text: 'string' },
      out: 'string'
    },
    speed: {
 ...
 * (MCP 로는 낸다. 이 문장 타수가 몇이야는 에이전트가 자주 틀리는 계산이다.)
 */
 ...
 * 정확도는 **글자 단위**로 센다.
 *
 * 자소로 세면 받침 하나 틀림이 글자 하나 틀림보다 가벼워지는데, 읽는 사람에게는 똑같이
