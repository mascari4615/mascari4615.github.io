 ...
 *
 * 「우리 동네엔 GS25 밖에 없다」는 체감을 지도로 만든다. 방법은 하나뿐이다 —
...
export interface Brand {
   ...
   * 긴 것을 먼저 적는다 — 「메가엠지씨커피」가 「메가커피」보다 먼저 걸려야 한다.
   */
  match: string[];
}
 ...
 * 업종별 브랜드표.
 *
 * 「상위 몇 개」만 담는다. 꼬리(1인 카페 수만 곳)까지 색을 주면 지도가 모래알이 되고,
 * 애초에 물음이 「어느 대기업 땅인가」다. 표에 없는 가게는 영토 계산에서 **빠진다** —
 ...
 * 격자 색인.
 *
 * 왜 필요한가: 화면 한 장을 칠하려면 수만~수십만 번 「가장 가까운 가게」를 묻는다.
...
export interface Grid {
   ...
   * 고리를 스무 겹 넓히며 수천 칸을 훑고서야 「없다」고 답한다. 화면의 절반이 바다인
...
export function nearest(g: Grid, lat: number, lng: number, maxKm = 8): Store | null {
  ...
  /* 격자 밖이거나, 미리 재 둔 「가장 가까운 가게 칸」이 이미 상한 밖이면 훑지 않고 끝낸다.
  ...
  for (let ring = 0; ring <= maxRing; ring++) {
    /* 더 볼 필요가 없으면 멈춘다. 고리 ring 의 가장 가까운 지점은 (ring-1) 칸 거리 이상 떨어져 있으므로,
       ...
       2026-08-20 에 실제로 브라우저가 멎었다. 「못 찾으면 일찍 포기한다」가 이 줄이다. */
    if (ring > 1) {
