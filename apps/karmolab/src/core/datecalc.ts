 ...
 *  ③ D-Day. D-100이 오늘을 세느냐 마느냐로 하루가 갈린다. 관례는 **시작일을 1일째로** 센다.
 *  ④ 두 날짜 사이. 끝날을 포함하느냐로 또 하루가 갈린다.
 * 그 갈림을 값과 말로 **분명히 적어** 낸다. 며칠만 던지면 맞는지 확인할 방법이 없다.
 *
 * 오늘은 인자로 받는다. 알맹이가 시계를 직접 보면 같은 입력에 답이 매번 달라진다.
...
export const run: ToolRunner = (op, args) => {
  ...
  if (op === 'between') {
    ...
    return [
      `${label(from)} ~ ${label(to)}`,
      `${b.days}일 (끝날 안 셈) ,  ${b.inclusive}일 (끝날까지 셈)  ← 며칠간은 대개 이쪽`,
