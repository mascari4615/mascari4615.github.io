 ...
 *
 * 그날 이후 얼마나 지났나를 **숫자가 계속 올라가는 모양**으로 보여 주는 것. 기념일, 금연, 
 ...
 * ① 몇 개월은 달마다 길이가 달라 뺄셈으로 안 나온다. 1월 31일의 한 달 뒤는 2월 28일이다.
 * ② 미래 날짜면 -3일이 아니라 **남은 시간**이라고 말해야 한다.
 ...
 * 달력대로 센다.
 *
 * 몇 개월을 30일로 나누면 3월생과 2월생의 나이가 달라진다. 사람이 세는 방식은
...
export function elapsed(from: Date, now: Date = new Date()): Elapsed {
  ...
  if (days < 0) {
    /* 전 달의 날 수를 빌린다. 이 한 줄이 30일로 나누기와 갈리는 자리다. */
...
/** 사람이 읽는 한 줄. 0인 앞자리는 빼고 말한다. 0년 0개월 3일은 읽기 나쁘다. */
 ...
 * 그래서 이 함수는 값만 주고, 어림이라는 말은 부르는 쪽이 반드시 붙인다(아래 `run` 참고).
 */
