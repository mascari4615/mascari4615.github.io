 ...
 * (`candidates`) 한국어, 일본어, 한자로 보이는 정도로 점수를 매겨 가장 그럴듯한 것을 앞에 둔다.
 * 사람에게 무슨 일이 있었나를 같이 말해 주는 게 이 도구의 값어치다. 다음엔 안 겪게.
 *
 * 못 되살리는 경우도 **분명히 말한다**: 물음표(`?`)나 대체 문자(U+FFFD)로 이미 바뀐 글자는
 * 바이트가 사라진 것이라 되돌릴 수 없다. 해 봤지만 안 된다와 원래 안 된다는 다르다.
 */
...
export function candidates(text: string): Candidate[] {
  ...
  // ② UTF-8 을 한국어 완성형(cp949)으로 읽어 버린 경우. 뷁류
  ...
  // ⑤ 주소, HTML 로 감싸인 경우. 깨짐은 아니지만 글자가 이상하다로 같이 온다
...
export function explain(text: string): string {
  ...
  if (/\?{2,}/.test(text) || loss.question >= 3) {
    rows.push('⚠ 물음표가 ' + loss.question + '개. 저장할 때 이 표에 없는 글자를 ? 로 바꿔 버린 흔적일 수 있다. 그건 못 되살린다.');
  }
