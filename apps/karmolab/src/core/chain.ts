 ...
 * 이 글을 Base64 로 바꾸고 그 결과의 SHA-256 을 내라는 두 번 왕복해야 하고, 그 사이에
 * 중간값이 에이전트의 말(토큰)을 거친다. **거기서 한 글자만 틀려도 뒤가 통째로 틀린다.**
 * 해시, 체크섬처럼 지어내면 안 되는 값을 다루는 서버에서 그 왕복은 특히 나쁘다.
 *
...
export function runChain(steps: Step[], call: CallTool): StepResult[] {
  ...
  for (let i = 0; i < steps.length; i++) {
    ...
    try {
      ...
      /* 어디서 멈췄는지 말해 준다. 실패했습니다만 오면 8단계 중 어디인지 알 수 없다. */
...
export const run: ToolRunner = (op, args, deps) => {
  ...
  /*
   * 마지막 값만 주지 않는다. 중간을 감추면 왜 이 값이 나왔나를 되짚을 수 없고,
