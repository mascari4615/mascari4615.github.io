 ...
 * 오늘을 사용자 기기 시계로 잡으면 하와이와 서울이 서로 다른 문제를 풀게 되고, 그 순간
 * 전원이 같은 문제라는 전제가 조용히 깨진다. 공유 격자를 서로 견줄 수 없어진다.
 ...
 *
 * 다른 알맹이는 전부 짝이 되는 화면과 `/t/<id>/` 주소를 갖는다. 그걸 강제하는
...
/** 3시간 12분처럼. 초는 안 보여 준다. 매초 다시 그릴 이유가 없다. */
 ...
 *
 * 그래서 여기서는 애초에 정답을 **받지 않는다.** 받아 놓고 안 쓴다로 두면 언젠가 누가
