 ...
 *
 * 검색, 정렬을 만들다 보면 강을 ㄱ/ㅏ/ㅇ 으로 쪼개야 하는 순간이 온다(초성 검색).
 * 반대로 자모만 남은 문자열을 글자로 되돌려야 할 때도 있다. 맥에서 만든 파일 이름이
 * 윈도우에서 ㄱ ㅏ ㅁ처럼 풀려 보이는 게 대표적이다(자모가 따로 저장된 표기, NFD).
 *
 * MCP 로 내놓는 이유(B등급): 유니코드 한글은 **계산으로** 쪼갠다
 * (코드 − 0xAC00 → 초성 ÷588, 중성 ÷28, 종성 나머지). LLM 은 이걸 외워서 답하다 자주 어긋나고,
 * 특히 겹받침(ㄳ, ㄺ, ㅄ)과 다음 자모가 종성이냐 다음 글자의 초성이냐에서 틀린다.
 ...
 * 자모 나열을 다시 글자로.
 *
 * 어려운 자리 하나: ㄱㅏㅁㅏ에서 ㅁ 은 **종성이 아니라 다음 글자의 초성**이다(가마).
 * ㄱㅏㅁ이면 종성이다(감). 그래서 뒤를 한 칸 더 본다. 다음이 모음이면 종성으로 안 붙인다.
 */
