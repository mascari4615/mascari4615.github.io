 ...
 * 그 숫자는 1년에 두 번 바뀐다. 서울과 뉴욕은 14시간은 겨울엔 맞고 여름엔 13시간이다.
 * 회의 시간을 그렇게 잡으면 한 시간 어긋난 채로 잡힌다. 여기선 **그 순간의 실제 오프셋**을
 * `Intl` 에서 읽는다(브라우저, Node 둘 다 시간대 표를 갖고 있다).
 *
 * 지금은 인자로 받는다. 알맹이가 시계를 직접 보면 같은 입력에 답이 매번 달라진다.
 */
...
export function wallOf(at: Date, zone: string): string {
  ...
  /* 이 한 줄이 없어서 배포가 며칠 막혔다. 서식기 설정은 부탁이고, 값 확인은 보장이다. */
