 ...
 * 점수가 쌓이고, 다섯 판째에 단추가 tournament 끝이 되는가.
 *
...
/* 한 판을 기다리는 참을성. 주석에 4분, 2.5분이 적혀 있었지만 코드는 60초였다 . 
...
for (let i = 1; i <= ROUNDS; i++) {
  ...
  /* ★ **시계를 세워 놨으면 기다리기가 아무 일도 안 한다** (2026-08-17, 재현해서 알아냈다).
     이 검사는 창을 열기 전에 `clock.install()` 로 시계를 세운다. 그래서 판은 **감을 때만** 움직인다.
     그런데 판이 끝나는 길은 두 걸음이다: ① 놀이가 이겼다를 내면 커널이 `roundOverAt = 지금 + 쉬는참`
  ...
  /* ★ **감는 총량은 손으로 안 적는다** (2026-08-17). 놀이마다 제 시간 제한이 있고 가장 긴 것이
     ...
     시험이 못 찾았다로 빨개졌다(실측). 값을 **소스에서 끌어와** 관계가 저절로 지켜지게 한다. */
    ...
    const stopped = await p
      .evaluate(() => ({
        ...
        /* ★ **시계가 갔는지부터 봐야 한다** (2026-08-17). 이 검사는 6분을 감아 어떤 놀이든 제
           시간 제한에 닿는다고 가정한다. 그런데 놀이마다 제한이 25초~300초로 다르고, 판이
           스스로 끝나는 것은 그 놀이의 `tick` 이 **불려야** 일어난다. 안 끝났을 때 그 둘을 모르면
           누가 범인인가에서 또 추측이 시작된다. 감긴 시각과 끝날 시각을 같이 들고 나간다. */
        now: Math.round(performance.now()),
        onEnd: window.__arcade?.endsAt ?? null,
        /* ★ **대회가 아직 서 있나** (2026-08-17). CI 가 남긴 줄은 화면: 1 / 1 판이었는데
  ...
  if (!st.status.startsWith(`${i}/${ROUNDS}판`)) fail.push(`${i}판 점수판이 ${i}/${ROUNDS}판으로 안 적힌다: ${st.status}`);
  const sum = [...st.status.matchAll(/(\d+)(?:\s|$)/g)].reduce((a, m) => a + Number(m[1]), 0);
  if (sum <= prevSum) fail.push(`${i}판에서 점수가 안 늘었다 (${prevSum} → ${sum})`);
  prevSum = sum;
  if (i < ROUNDS && st.again.includes('끝')) fail.push(`${i}판인데 단추가 벌써 tournament 끝이다`);
