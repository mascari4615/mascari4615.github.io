 ...
 *
 * 두 덩어리를 나란히 놓고 「이쪽만 쓰는 말」을 보여준다. 그런데 그 말이 저쪽에서도
 ...
 * 몇 배 이상 차이 나야 「A 만 쓰는 말」이라 부를 수 있다.
 */
...
const TIMES = 2;        // 상대보다 이만큼은 더 자주 나와야 「이쪽만 쓰는 말」이다
...
if (weak > checked * 0.25) {
  console.log('[compare] **「이쪽만 쓰는 말」이 저쪽에서도 나온다** — 견주는 게 아니라 늘어놓는 것이다');
