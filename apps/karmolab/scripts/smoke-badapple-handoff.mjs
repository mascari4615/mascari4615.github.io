 ...
 *
 * 그래서 여기서 보는 것은 두 가지다:
 *   ① 굽는 화면이 홈이 이어받는다고 말하는가 (담기 실패를 성공으로 말하지 않는가)
...
const baked = await studio.evaluate(async () => {
  ...
  await new Promise((done) => {
    const draw = () => {
      ...
      // 색이 안 나온다가 항상 참이 되어, 고장이 아닌데 빨개진다(한 번 그렇게 나왔다).
...
const picked = await home.evaluate(async () => {
  ...
  // 덮는 층이 **색으로** 칠해졌나. 회색만 나오면 세 채널이 거의 같다. 그걸 색이 없다로 본다.
  ...
  /* ★ **빈 층과 회색 층을 가른다** (2026-08-21). 예전에는 `colored` 하나만 세어서,
     ...
     색 평면이 안 실린 것(format/스튜디오). 2026-08-19 에 증상 없는 고장으로 한 번
     겪고도 문구가 회색으로만 칠해진다라 엉뚱한 데를 보게 만들었다. */
  let opaque = 0;
