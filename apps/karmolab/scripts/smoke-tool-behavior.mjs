 ...
 * 기대가 `CHANGES` 면 값을 바꾸면 화면이 달라진다만 본다. 계산기처럼 답이 숫자라
 * 미리 적어 둘 수 없는 도구용이다. 이때 넣는 값은 **기본값과 달라야** 한다.
 * (BMI 에 기본값과 같은 값을 넣고 결과가 안 바뀐다고 오해한 적이 있다.)
 */
...
const CASES = [
  ...
  ['linebreak', '한 줄\n두 줄', null, /한 줄/],
  /* 이 도구는 **고친 쪽이 곧 방향**이라 누를 단추가 없다 (KL-133 에서 없앴다).
     검사가 옛 단추를 찾느라 오래 빨갰다. 제품은 멀쩡했는데 게이트가 거짓말을 했다. */
  ['csvjson', 'a,b\n1,2', null, /"a"|\[/],
  ...
  /* 계산기는 화면이 바뀌었나가 아니라 **답이 맞는가**로 본다 (TASK-KL-089).
  ...
  /* 대출: 1억, 연 4.5%, 360개월 원리금균등 = 월 506,685원 (손으로 따로 셈한 값).
     이 숫자가 흔들리면 이 도구는 믿을 수 없다. */
  [
    'loan',
    [
      ['대출 금액', '100000000'],
  ...
  /* 텍스트 비교는 줄이 바뀌었다가 아니라 **줄 안에서 어디가 바뀌었나**가 값어치다.
  ...
  [
    ...
    [
      ['루트 글자 크기', '16'],
      // 칸 이름이 바꿀 숫자에서 숫자로 바뀌었다. 검사만 옛 이름을 들고 있어서
   ...
   * 바꾸기: 바나나 사과에서 바나나를 포도로 → 포도 사과 (넣은 값에는 없는 짝이라 답으로만 나온다) */
  ['grade', '3 A+\n3 B0', null, /3\.75/],
  [
    'replace',
    [
...
/* 작업대로 합친 옛 도구는 낱개 장이 없다. 그 주소를 열면 안내 한 장이라 넣을 칸이 없고
   넣어도 답이 안 나온다로 읽힌다(2026-08-13: `replace` 가 그렇게 빨갰다).
...
async function runCase([id, input, press, expect]) {
  ...
  try {
    ...
    // 달라지는가로 보는 도구는 넣기 전 화면을 먼저 담아 둔다.
    ...
    if (expect === CHANGES) {
       ...
       * 안 된다**. 1을 지우면 답 1,000,000,000 도 함께 뭉개져 답이 없다가 된다
