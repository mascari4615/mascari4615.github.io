/**
 * 오늘의 판이 **매일 실제로 갈리는가** (TASK-KL-194 · memo `projects/karmolab/놀이.md` §5).
 *
 * 왜 따로 있나: 기존 `test:daily`(smoke-daily)는 「답이 화면에 새지 않는가」를 본다.
 ...
 *   - 같은 날 + 같은 게임이면 **누가 열어도 같은 수** (그게 「전원 같은 문제」의 뜻)
 *   - 게임이 다르면 같은 날이어도 다르다
...
ok(
  ...
  '같은 날 같은 게임인데 씨앗이 흔들린다 — 「전원 같은 문제」가 깨진다',
);
...
/* 30일을 훑어 「연달아 같은 씨앗」이 없는지 본다 (하루 차이가 안 퍼지면 문제가 비슷해진다) */
{
  const seeds = [];
  for (let i = 0; i < 30; i++) {
