 ...
 *  ④ 화면이 「약 N차원 · 2차원에 담으면 찢김은 피할 수 없다」를 적는다
 */
...
if (!D) {
  ...
  console.log('[idim] **고유차원이 안 실려 있다** (idim) — 「2차원에 담기나」를 아직 안 물었다');
  process.exit(1);
}
...
/* ★ **과소추정 편향이 실제로 있어야** 한다 — 없으면 「우리 18 은 하한」이라는 말이 거짓이다. */
const hi = D.calibration.find((c) => c.truth === 20);
if (!hi) bad.push('눈금에 20차원 판이 없다 — 과소추정 편향을 보여 줄 수 없다');
else if (!(hi.mle < hi.truth)) bad.push(`20차원을 ${hi.mle} 로 본다 — 과소추정 편향이 안 보이면 「하한」이라 말할 수 없다`);
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysN) bad.push('화면이 「약 N차원」을 안 적는다');
