 ...
 * 것은 **없는 것**이 된다. 나중에 볼게가 성립하지 않으면 그건 받은함이 아니라 종소리다.
...
if (!problems.length) {
  ...
  await page.waitForFunction(() => document.querySelectorAll('.kl-bell-item').length > 30, undefined, { timeout: 5000 })
    .catch(() => problems.push('더 보기를 눌러도 안 늘었다'));
  ...
  // 다 보여 준 갈래에는 더 보기가 없어야 한다. 눌러도 아무 일 없는 단추를 두지 않는다
  if ((await page.locator('#klBellMore').count()) !== 0) problems.push('다섯 개뿐인데 더 보기가 떠 있다');
}
