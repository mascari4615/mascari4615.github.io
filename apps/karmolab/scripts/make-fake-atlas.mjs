 ...
 * 열아홉이 지도가 없다며 건너뛰었다. 재료가 없으면 자는 아무것도 안 잰다.
 *
...
const PER = 80;            /* 무리마다 여든 글. **성기면 자가 못 잰다**. 156글로 만들었더니
                              당기면 이름이 사라지나 자가 글 10개 넘는 자리를 못 찾았다로
...
for (let g = 0; g < GROUPS; g += 1) {
  /* 무리마다 자기 낱말 둘을 갖는다. 이름 짓기, 견주기 자가 이 무리만 쓰는 말을 찾을 수 있게. */
  const own = [`${HEAD[g]} ${TAIL[g]}`, `${HEAD[(g + 3) % HEAD.length]}터`];
  /* 무리를 고리로 놓되 반지름을 줄여 **가운데도 채운다**. 한가운데가 비면
     Enter 로 가운데 글 고르기가 아무것도 못 잡는다(자가 그걸 잡아냈다). */
  ...
  for (let k = 0; k < PER; k += 1) {
    ...
    /* 본문을 넉넉히. 400자 밑이면 얇은 글로 걸러져 혼자 있는 글 자가 빨개진다. */
