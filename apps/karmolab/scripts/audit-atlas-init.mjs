 ...
 *
 * Kobak & Linderman(Nature Biotechnology 2021): 「UMAP 이 t-SNE 보다 전역 구조를 잘 지킨다」는
 * 알고리즘 차이가 아니라 **초기화 차이 하나**였다. **전역 배치는 최적화가 만드는 게 아니라
 * 초기값이 물려주고**, 끌림-밀침은 그 위에서 지역만 다듬는다. 우리가 잰 「자리의 절반은
 * 난수가 정한다」는 그 증상이고, 처방이 여기 있다 — 그래서 사다리를 굽고 재서 골랐다.
 *
...
if (!L.winner || !L.control) bad.push('본선 결과가 없다 — 씨앗 여러 판으로 다시 재야 판정이 선다');
  ...
  /* ★ **판정이 수와 맞나** — 이 자의 핵심. 졌는데 「바꿨다」고 적으면 빨강. */
  ...
  /* ④ 떨림은 보조 — 다만 **실려는 있어야** 한다. 안 실으면 「무엇을 샀나」를 못 적는다. */
