#!/usr/bin/env node
/**
 * audit-atlas-lonely — **「어디에도 안 붙는 글」이 새 렌즈인가** (TASK-KAR-233).
 *
 ...
 * 재료가 없으니 당연히 혼자 떨어진다. 그건 「본문이 없다」를 다시 찾아낸 것이다.
 *
 * 그래서 셋을 본다:
 *  ① 뽑힌 글에 **본문 얇은 글이 끼지 않았나** (그러면 뜻이 아니라 길이를 잰 것이다)
 *  ② 「묻힌 글」과 **너무 겹치지 않나** (겹치면 새 렌즈가 아니다)
 *  ③ 실린 값이 다시 재도 맞나
 */
...
console.log(`[lonely] 「묻힌 글」과 겹치는 것 ${both}/${lonely.length} (${(share * 100).toFixed(0)}%)`);
const TOO_SAME = 1 / 3;
if (share > TOO_SAME) {
