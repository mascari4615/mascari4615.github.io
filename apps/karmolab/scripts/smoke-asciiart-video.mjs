...
// 브라우저가 없는 자리(배포 앞단)에서는 빨강 대신 못 돈다를 남기고 비킨다. `lib/browser.mjs` 머리말.
...
const report = await page.evaluate(async () => {
  ...
  /** 그 자리에서 만드는 영상. 도는 막대가 있어야 그림이 바뀌는가를 잴 수 있다. */
  ...
  // 밝은 칸 수로 재면 안 된다. 빛번짐이 더하는 빛은 옅어서 문턱을 못 넘고, 세어 보면
  ...
  // 정지 이미지에서도 같은 효과가 걸리나. 여기가 갈리면 화면과 저장물이 같다가 깨진다.
  effect.value = 'none';
  effect.dispatchEvent(new Event('input', { bubbles: true }));
  await sleep(200);
  $('aaSample').click();
