 ...
 *
 * 왜 필요한가: 값을 넣으면 답이 나오는지를 보는 검사는 도구마다 기대값을 손으로 적어야 해서
 * 125개 중 32개만 보고 있었다. 도구는 계속 느는데 그 사이가 벌어진다. 새 도구가 통째로
 * 망가져도 화면이 보인다만 통과하면 아무도 모른다.
 *
 ...
 *   이미 골라진 탭을 누르니 화면이 그대로고, 검사는 고장이라 했다. 도구는 멀쩡했다.
...
/* 작업대로 합친 옛 도구는 이제 **작업대로 보내는 안내 한 장**이다. 그 주소를 열면 안내가
   뜨고 입력칸이 없다. 도구가 아닌 것을 반응이 없다로 세면 이 검사가 늘 빨갛다
...
async function checkOne(page, id) {
   ...
   *   느린 판에서는 그 사이에 재게 되고, 그러면 글자를 넣을 칸이 없다로 제품을 탓하게 된다.
  ...
  const target = await page.evaluate((toolId) => {
     ...
     * 예전에는 글자, 숫자 칸만 찾아서, 날짜만 받는 도구는 넣을 칸이 없다로 지나쳤다 . 
  ...
  if (!target) {
    /* 안 그려졌을 수 있다로만 적으면 제품 고장인지 이 판이 막힌 것인지 못 가른다 . 
  ...
  /* ★ 아무 글자나 넣으면 안 되는 도구가 있다 (2026-08-12). 색 변환기는 테스트 abc 123을
   ...
   * 아무 일도 안 일어나서, 검사에는 고장으로 보였다(도구는 멀쩡했다). */
 ...
 * 예전에는 전체 - 여기서 본 것 - 32로 숫자만 냈다. 그 32 는 답을 맞춰 보는 다른 검사가
...
/* 볼 수 없는 데는 이유가 있는 도구. 사유를 적어 두고 안 보는 목록에서 뺀다.
