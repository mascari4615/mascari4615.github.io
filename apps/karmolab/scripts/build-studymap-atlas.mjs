...
/**
 * 주성분 두 축은 **가장 큰 방향**을 잡을 뿐, 「누가 누구 옆인가」를 지키지는 않는다
...
function mds(rows, init, rounds = 300) {
  ...
  /* 목표 거리 = 뜻의 각도. 코사인이 1 이면 0, -1 이면 2 — 사람이 보는 「멀다」와 순서가 같다. */
  const D = [];
  for (let i = 0; i < n; i += 1) {
...
/**
 * 이웃 묶음 — 자리만 있으면 「왜 여기 있나」가 안 보인다. 가까운 갈래를 묶고 이름을 붙인다.
 *
 ...
 * 한가운데 갈래 이름을 쓰면 거짓말이 된다 — 웹·Git·보안이 든 묶음이 「DevOps 둘레」가 됐다(실측).
 * (memo 지형도는 AI 가 이름을 붙이지만 거기엔 「진짜 내 글에서 나온 말인가」를 재는 자가 있다.
...
function cluster(rows, k) {
  ...
  /* 씨앗을 안 쓴다 — 첫 축으로 줄 세워 **고르게** 집는다. 같은 입력이면 같은 묶음이고,
     「가장 먼 점」으로 집으면 외톨이가 중심이 되어 한 묶음에 절반이 몰린다(실측: 41 중 20). */
 ...
 *  ① 한가운데 갈래 이름: 웹·Git·보안이 든 묶음이 「DevOps 둘레」가 됐다. 거짓말이다.
 *  ② 제목에서 유난히 잦은 말(tf-idf): 「것을 · 라이선스」 「내보낸다 · 무엇으로」가 나왔다. 헛말이다.
 * 검증할 수 없는 이름은 안 붙이는 편이 낫다 — 묶음은 **색으로만** 「이 근처는 한 동네」를 말하고,
 * 무엇인지는 그 안의 갈래 이름들이 직접 말한다(이미 화면에 다 적혀 있다).
 * 이름이 필요해지면 사람이 붙여라 — 그때 이 자리에 표를 하나 두면 된다.
 */

/* 41갈래에 6묶음 — 한 묶음이 대여섯이라야 「이 근처는 이런 것」이 눈에 들어온다. */
