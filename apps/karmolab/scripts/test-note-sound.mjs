/**
 * 판 도는 중에 무슨 일이 나르고, 소리가 **한 번씩만** 우는가 (arcade-next 놀이마다의 소리)
 *
 * 재는 것 셋:
 *  ① 커널이 **판이 안 끝나도** 말을 나른다 (전에는 끝날 때만 받아서 화살이 든 순간이 안 남았다)
 *  ② 말에 소리 이름이 붙는다. 게임은 무슨 일만 말하고 어떤 소리인지는 껍데기가 고른다
 *  ③ **한 사건에 한 번**만 운다. 껍데기는 말이 바뀌었나로 견주는데, 같은 사람이 연달아
...
/**
 * 놀이마다 일어난 일을 어디에 적어 두는지가 다르다. 함대는 `last`, 투호는 `shots` 끝.
...
const CASES = [
  ...
  /* 협동 놀이. 불이 꺼졌다는 모두의 손해라 소리가 특히 값어치 있다. 일어난 일은
  ...
  /* 하이로우. 이어 맞히면 배로, 틀리면 0. 한 장만 더가 심장이라 소리가 곧 놀이다.
  ...
  /* 지뢰찾기. 여럿이 **동시에** 파는 놀이라 매 칸마다 울리면 시끄럽다. 밟은 순간만 운다.
     그래서 사건 세기도 밟은 사람 수다. 한 사람은 한 번만 밟는다(밟으면 그 판은 끝). */
  {
    id: 'minesweeper',
...
for (const one of CASES) {
  ...
  /**
   * **놓친 사건이 없는가**. 이게 알맹이다. 처음엔 운 횟수가 적당한가만 봤는데 놀이에서
  ...
  /* **마지막 하나는 빠질 수 있다**. 그 사건으로 판이 끝나면 결과 말이 그 자리를 덮는다.
     ...
     투호에서 실제로 그렇게 잡혔다: 16발 중 15번. 마지막 발이 이겼다로 바뀐 것이었다. */
