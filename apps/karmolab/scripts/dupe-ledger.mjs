 ...
 * 늘어나는데 표는 안 늘어난다. 그러면 어디를 손대면 몇 줄이 줄어드나를 **추측**으로 고르게 된다.
 * 이 저장소가 그걸 싫어한다. 그래서 세는 일을 코드로 옮긴다.
 *
 * 세는 것 둘:
 *   ① 되풀이 조각. 같은 서너 줄이 몇 파일에서 다시 쓰이나. 공용으로 빼면 몇 줄 주나까지.
...
const SNIPPETS = [
  ...
  ['화면 통짜 (innerHTML)', /container\.innerHTML/, 30, 0], // ⚠ 이 수는 뽑을 수 있는 줄이 아니다. 2026-08-16 실측: 같은 뼈대로 보이는 59곳 중 **바이트가 같은 꼴은 2곳뿐**이었다(같은 클래스를 쓰지만 안에 든 것이 제각각). 통짜 줄이기는 조각 뽑기로 거의 안 되고 선언형 조작 자리(KL-256)로만 된다.
];
...
/* ★ **이미 합쳐 둔 계열이 있다**. 그게 본보기다 (2026-08-16 발견).
   ...
   나머지 계열은 일 하나가 파일 하나다. 그래서 합치면 얼마나 주나는 어림이 아니라 **견줌**으로 낸다:
...
/* ★ **옮길 수 있는지부터 잰다** (2026-08-16). 남은 28개는 세어 봐야 하는 수가 아니다 . 
...
function movability(f) {
  ...
  /* 화면을 **여러 번** 새로 그리면 그 도구는 결과칸 하나로 안 접힌다 (2026-08-16 실측:
     jsonfmt 이 나무 보기를 그리는데 표, 그림 표식이 없어 옮길 수 있다로 셌다).
     첫 innerHTML 은 판 짜기라 안 센다. 그 뒤부터가 따로 그리는 것이다. */
  const draws = (f.text.match(/innerHTML\s*=/g) || []).length;
  if (draws > 1) rich += 1;
  /* 단추가 여럿이면 그중에 **일하는 단추**가 있다. 작업대 조작에는 복사 말고 손이 하나뿐이다
     (2026-08-16 실측: curlkit 은 보내기로 진짜 요청을 보낸다. 그리기 표식만 봐서는 안 잡혔다).
