...
/* 주석은 화면이 아니다 (2026-08-13). 설명글에 적힌 `<input accept>` 같은 예시가 진짜 칸으로
   잡혀 이름 없는 칸 1개가 됐다. 없는 결함을 쫓게 만드는 빨강이라 먼저 걷어낸다. */
...
for (const { name, src: raw } of files) {
  ...
  for (const m of src.matchAll(NEEDS_NAME)) {
    ...
    // placeholder 는 브라우저가 이름이 없을 때 대신 읽어 준다. 완벽하진 않지만 편집란보다는 낫다.
...
if (offenders.length) {
  console.error(`[check-input-names] 이름 없는 입력칸 ${offenders.length}개. 화면낭독기는 편집란으로만 읽습니다`);
