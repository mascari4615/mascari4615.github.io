 ...
 *
 * 두 문턱을 같이 낸다. 하나만 내면 지켰다/못 지켰다가 문턱 고르기 싸움이 된다:
...
for (const url of URLS) {
  ...
  const m = await p.evaluate(() => {
    ...
    /* ★ **문장 안 링크는 기준이 스스로 빼 준다** (2026-08-17). WCAG 2.2 AA 2.5.8 에는
       ...
       **문단 속 링크 넷**이 21px 로 잡혀 기준 위반처럼 보였다(고치면 줄 모양이 깨진다).
  ...
  /* ★ 24px 미만을 먼저 보여 준다. 그게 바깥 기준(WCAG 2.2 AA)이 실제로 요구하는 선이다.
     44px 미만은 더 크면 좋다쪽이라 목록이 길어도 사고가 아니다. */
...
if (openPages === 0) {
  console.error('[tap-size] CANNOT-RUN: 한 장도 못 열었다. 이건 다 크다가 아니라 안 잰 것이다.');
  process.exit(2);
}
