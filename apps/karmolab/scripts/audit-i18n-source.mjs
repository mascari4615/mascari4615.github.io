 ...
 * 멈춘다. 지금 0으로 만들어라가 아니라 여기서부터는 늘지 마라. 그래야 사람이 검사를
 * 안 끄고, 숫자가 실제로 내려간다.
 *
 ...
 * 막은 것이 훨씬 컸다.
 *
 * 세우고 싶을 때는 `--strict`. 옮기는 일이 더 진행돼 다 옮긴 파일이 뚜렷해지면 그때 기본으로.
 */
 ...
 * 주석을 걷어 낸다.
 *
 * 정확한 파서를 쓰지 않는 이유: 우리가 알아야 하는 건 한국어 글자열이 몇 개인가라는 **추세**지
...
function countKoreanLiterals(code) {
  ...
  for (let i = 0; i < code.length; i++) {
    ...
    /* ★ 정규식 리터럴을 건너뛴다 (2026-08-09 실측으로 고침).
       ...
       문자열로 잡혔다. 태그, 코드가 섞인 조각이 한국어 글자열로 세어졌다.
...
if (REBASE || TIGHTEN || BLESS.length) {
   ...
   * (실제로 `karmograph` 이 86→113 으로 늘어 있었다). 잠금을 조이려던 행동이 그 파일에
   * 대해서는 정확히 반대로 작동한다.
 ...
 *
 * → 갈라 본다: **이미 옮긴 파일이 되돌아가는 것**만 잘못이고, 새 파일은 아직 안 옮긴 것이라
 ...
 * 기계가 맞는 말인가를 못 보므로, **의심 자리를 좁혀 사람에게 보여 준다**. 세우지는 않는다
 ...
 * 파일은 있는데 git 은 없앨 참인 자리를 여기서 본다. */
{
  let staged = '';
  try {
