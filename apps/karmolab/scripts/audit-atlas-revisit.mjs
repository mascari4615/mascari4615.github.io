 ...
 * **정교한 분류 체계는 번번이 버려졌다.**
 *
 * 우리 지도가 정확히 묵힌 것을 위한 정교한 분류 체계다. 그래서 **git 을 정답으로** 재 봤다 . 
 * **다시 손댄 글**이 곧 사용자가 실제로 돌아온 글이다.
 *
 * ★ **결과도 냉정했다.**
 * , 2019~2025년 글의 다시 손댄 비율이 **전부 0%**. 2026년만 54%. **묵힌 것은 정말 안 본다**
 * , 곧 다시 손댈 글을 **앞 시기 정보만으로** 짚으면 상위 10편 중 **0%** (바탕 11.5%)
 * , 우리가 만들어 둔 **묻힌 글로 짚어도 0%**. 우리 기능이 이 일에는 안 듣는다
 ...
 *  ② **나이별 재방문율**을 낸다. 묵힌 것은 안 본다가 우리에게도 맞나
 *  ③ 예측은 **앞 시기 정보만**. 같은 시기 신호는 예측이 아니다(갈라서 둘 다 적는다)
 *  ④ 못 맞히면 **그렇게 적는다**. 일깨움에도 안 쓰인다도 결과다
 */
...
if (!R) {
  ...
  console.log('[revisit] **쓰이는지를 재 본 표가 없다** (revisit). 이 지도가 쓰이나를 안 물었다');
  process.exit(1);
}
...
/* ② 나이별이 있어야 묵힌 것은 안 본다를 말할 수 있다. */
if (!(R.ages?.length >= 3)) bad.push('나이별 재방문율이 없다. 묵힌 것은 안 본다를 확인할 수 없다');
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysBuried) bad.push('화면이 **우리가 만든 묻힌 글의 성적**을 안 적는다. 안 듣는 기능을 숨기면 안 된다');
  await browser.close();
}
