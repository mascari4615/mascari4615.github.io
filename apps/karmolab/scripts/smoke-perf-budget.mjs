...
await page.waitForFunction(() => !!window.KLPerf && typeof Toolbox !== 'undefined', null, { timeout: 20000 });
/* 첫 그림·큰 그림이 정해질 틈. 너무 일찍 재면 「아직 안 나온 것」을 「없다」로 적는다. */
await page.waitForTimeout(2500);

/* 조작 지연은 **일으켜야** 잡힌다 — 기다리면 영영 「못 잼」이다.
   첫 화면의 진짜 단추를 누른다(빈 곳을 누르면 핸들러가 없어 「우리 코드가 느린가」를 못 본다). */
const clickable = await page.$$('.landing-cta, .tool-card, .hp-open, header button, nav button');
...
const verdict = await page.evaluate(() => window.KLPerf.snapshot().verdict);
const trust = await page.evaluate(() => window.KLPerf.snapshot().trust);
...
const rows = REGRESS
  ? verdict.map((v) => (v.value == null ? v : { ...v, limit: v.value / 2, state: 'fail' }))
  : verdict;
...
if (!trust.ok) console.log(`[perf-budget] ⚠ 이 판은 비교에 못 쓴다 — ${trust.why}`);
if (pageErrors.length) console.log(`[perf-budget] ⚠ 화면 오류 ${pageErrors.length}건 — ${pageErrors[0].slice(0, 120)}`);
...
if (REGRESS) {
  ...
  console.log(`[perf-budget] --regress: 예산을 반으로 조여 ${caught}건 잡음 (0 이면 게이트가 죽은 것)`);
