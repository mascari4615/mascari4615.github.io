 ...
 * 오래 있었고, 그때마다 flaky로 적고 넘겼다. 그런데 2026-08-13 에 재현율을 재 보니
 * **제품 결함**이었다(카드를 고르면 판이 튀어 손잡이가 도망갔다). 흔들린다는 진단이 아니다.
 *
...
/* ★ **한 번 빨간 것은 아직 진단이 아니다** (2026-08-14 실측). 이 작업공간은 세션 여럿이 같은
   ...
   빨간 것들은 한 판 더 돌려 되풀이되는지 본다. 되풀이되면 제품, 아니면 그 판의 부하다. */
if (reds.size > 0 && [...reds.values()].every((n) => n === 1)) {
  /* ★ **어느 항목이 흔들렸는지 이름을 남긴다** (2026-08-17). 여태 부하로 본다로 넘기며
...
console.log('RESULT: SHAKY. 재현율을 재고 원인을 찾아라 (flaky는 진단이 아니다)');
process.exit(1);
