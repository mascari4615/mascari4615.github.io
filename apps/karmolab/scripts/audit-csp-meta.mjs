 ...
 * 문구는 `lib/head-security.mjs` 한 벌이고, 이 검사는 빠진 화면이 있나만 본다.
 *
...
for (const f of files) {
  ...
  /* ★ **글자로 훑으면 말한 것과 단 것이 안 갈린다** (2026-08-17, 옆 검사에서 같은 구멍을 봤다).
     이 저장소에는 CSP 를 **설명하는 도구 장**이 있다(`/t/csp/`). 거기 본문이나 주석에 같은 글자가
     들어가면 실제 `<meta>` 가 없어도 통과한다. 안전 한 줄을 있다고 세는 검사가 그러면 안 된다.
  ...
  const sweet = [...src.matchAll(/<meta\s+[^>]*http-equiv=["']Content-Security-Policy["'][^>]*>/gi)]
    /* 값 안에 홑따옴표가 있다(`'none'`). **연 따옴표와 같은 것으로 닫히는 데까지** 읽어야 한다.
       `[^"']*` 로 읽으면 `object-src ` 에서 잘려 멀쩡한 장이 빠졌다로 잡힌다(방금 6장). */
