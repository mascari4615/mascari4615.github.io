 ...
 *   (이름이 나오면 부르든 import 하든 연결돼 있다로 본다. 여기서 더 따지면 오탐이 는다.)
 *
...
/* 파일을 하나도 못 찾으면 전부 연결돼 있다가 아니라 못 쟀다. 자리가 옮겨진 것이다. */
...
/* 별 **제 이름을 제가 적은 것은 부른다가 아니다** (2026-08-17 실측). 검사 파일은 머리주석에
   사용: node scripts/test-meok-ops.mjs처럼 제 이름을 적어 둔다. 그 글자까지 한 건초에 넣으면
...
/* 건초가 package.json 하나뿐이면 읽기가 깨진 것이다. 그 상태의 0건은 통과가 아니다. */
...
/* 별 **왜 안 묶었는지를 기준선이 스스로 적게 한다** (2026-08-17). 이름만 늘어놓은 목록은
   반년 뒤 아무도 못 읽고, 언젠가 묶자로 굳는다. 손으로 적은 사유는 다시 쓸 때도 지킨다
    ...
    `${JSON.stringify(
      {
        ...
        list: neverCalled,
        reason: Object.fromEntries(neverCalled.map((f) => [f, previousReason[f] ?? '아직 사유를 안 적었다. 재 보고 여기 적어라'])),
