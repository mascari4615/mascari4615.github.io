...
if (!it) {
  ...
  console.log('  「나눔이 좋은가」만 재고 「읽히나」는 안 재고 있다. build-memo-atlas.mjs 의 wordIntrusion 을 봐라.');
  process.exit(1);
}
...
if (typeof it.dfMp !== 'number' || typeof it.dfHiMp !== 'number') {
  bad.push('빈도로 푸는 판정자를 안 재 봤다 — 「드문 것 고르기」로 풀리는 시험일 수 있다');
} else {
  if (it.dfMp > it.chance) bad.push(`「드문 것 고르기」로 ${it.dfMp} 를 맞힌다 — 시험이 순환한다`);
  if (it.dfHiMp > it.chance) bad.push(`「흔한 것 고르기」로 ${it.dfHiMp} 를 맞힌다 — 시험이 순환한다`);
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysChance) bad.push('화면이 **찍기**를 안 적는다 — 찍기 없이 적은 비율은 「잘 맞힌다」로 읽힌다');
}
