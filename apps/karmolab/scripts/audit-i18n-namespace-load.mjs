...
if (missingNs.length) {
  ...
  console.error('  되받을 글이 있어도 안 구해진다. 묶음을 받아오는 단계에서 죽고, 화면에는 도구 로드 실패로만 뜬다.');
...
for (const [g, uses] of usedBy) {
  ...
  for (const [ns, file] of uses) {
    ...
    /* 그 묶음 파일이 아예 없으면 다른 검사 몫이다. 여기서는 받아 오나만 본다 */
...
/* **톱니(ratchet)**. 지금 걸린 자리들은 기준선에 적어 두고 통과시킨다. 이 검사는 그 자리가
   실제로 깨졌다까지는 못 본다(다른 길로 묶음이 들어와 있을 수 있다). 확실한 것은 **새로 생기는
