...
const { SHORTCUTS, shortcutKeys, shortcutsHtml } = load('src/widgets/karmo-studio/shortcuts.ts');
const widget = fs.readFileSync(path.resolve('src/widgets/karmo-studio/karmo-studio.ts'), 'utf8');
...
console.log(`[test-karmo-studio-shortcuts] ✓ ${keys.length}개 단축키가 목록·코드·화면에서 일치`);
