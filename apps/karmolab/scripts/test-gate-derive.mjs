 ...
 * 제일 비싼 고장이다. 그래서 잠그는 것은 빨라졌나가 아니라 **빠지면 안 될 것이 빠지나**다.
 *
...
eq(trapHit.length, 0, `알아낸 발판에 전부 걸리는 자리가 섞였다: ${trapHit.slice(0, 3).join(', ')}`);
...
/* 역따옴표로 감싼 주석 속 경로도 집는다. 그건 이 검사가 무엇을 딛나를 사람이 적어 둔
...
/* 모든 소스는 발판이 아니다. 넣어도 늘 걸리므로 알아낸 게 없는 것과 같다. */
