...
/* 회색 한 겹은 **색이 아니라 마스크**다. 이걸 색으로 읽으면 검은 데가 「검은 물체」가 되어
...
/* 자료가 모자라면 **조용히 0 으로 채우지 않는다** — 반쯤 채운 그림은 「됐다」로 보인다. */
let threw = false;
try {
...
/* ★ 상한이 없으면 3분짜리를 넣은 사람이 브라우저를 잃는다. 「느리다」가 아니라 「망가졌다」다. */
...
/* ★ 0장을 주면 부르는 쪽이 빈 결과를 「됐다」로 보고 저장까지 한다. */
