 ...
 * 계보가 동시에 있어 2차원에 다 못 담는다」가 그대로 적혀 있다. 그래서 **쓰기 전에 잰다** —
 ...
 *  ④ 화면이 「굽은 2차원이 도움이 될 자료인가」를 적고, **아니면 아니라고 말한다**
 */
...
/* 판정이 수와 맞나 — 잡음보다 크면 「나무 아님」이어야 한다. */
if (D.matched != null) {
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysVerdict) bad.push('화면이 「굽은 2차원이 도움이 되나」의 답을 안 적는다 — 다음 사람이 같은 것을 또 해 본다');
  await browser.close();
}
