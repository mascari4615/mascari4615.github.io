 ...
 *
 * 이미 있는 검사들이 왜 못 잡았나:
 *  , `audit:i18n-load` 는 그 파일이 `loadNamespace` 를 부르나만 본다. 부르긴 **부른다**.
 *     이르고 늦음은 글자로 안 보인다.
 ...
 *
 * 어디까지를 일찍으로 보나: 위젯 파일은 통째로 IIFE 로 감싸여 있다. 그 IIFE 몸통은
...
/* ★ **빚 목록**. 오늘 이전부터 있던 자리들. 여기 적힌 것은 **안전하다고 증명된 게 아니라**
   오늘 새로 생긴 것이 아니다는 뜻이다. 이 중 실제로 죽어 있는 화면은 `test:play-i18n`
