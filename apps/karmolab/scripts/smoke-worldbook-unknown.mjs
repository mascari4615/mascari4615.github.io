/**
 * 「데이터가 UI 를 정한다」를 **실제로** 재 본다 (wm-hub.md §0 마지막 줄).
 *
 * WM 은 개발 중이라 필드·종류가 주 단위로 바뀐다. 그래서 도감은 「이 필드들을 보여라」가 아니라
 * 「있는 것을 보여라」여야 한다 — 안 그러면 WM 이 필드를 하나 더할 때마다 웹 코드를 고쳐야 하고,
 ...
 *
 * 「그려졌나」로는 못 잡는 것이라 화면 글자로 잰다. 묶음이 없으면 **못 돌린다**고 말하고 비킨다
 * (제품 고장과 「안 지음」을 안 섞는다).
 *
