#!/usr/bin/env node
/**
 * audit-atlas-h0 — **나누지 않고 본 「조각 몇 개」가 맞나** (TASK-KAR-233).
 *
 * 실루엣(거리)·DBCV(밀도)·HDBSCAN(안정성)은 다 「이 **나눔**이 좋은가」를 잰다.
 ...
 * 정본을 그대로 쓰면 안 됐다 — 「대각선에서 띠보다 멀면 신호」로 세니 **갈린 셋도 아무 구름도
...
function h0(points) {
  ...
  /* **조각 수 = 긴 막대 수 + 1** — 막대는 「합쳐지는 사건」이지 조각이 아니다.
...
{
  /* 눈금 — 띠를 「아는 값」으로 주고 판정만 본다(재표본은 굽는 쪽에서 이미 돌린다). */
...
if (!st) {
  ...
  if (!Array.isArray(st.bars) || st.bars.length < 5) bad.push('막대가 안 실려 있다 — 「왜 그 답이냐」를 다시 볼 수 없다');
  else {
    /* ③ 같은 규칙을 실린 막대에 다시 건다. */
    let cutAt = 0; let drop = 1;
...
if (!chromium || !fs.existsSync(BUNDLE) || !st) {
  ...
  if (!said) bad.push('화면이 「나누지 않고 본 답」을 안 적는다');
}
