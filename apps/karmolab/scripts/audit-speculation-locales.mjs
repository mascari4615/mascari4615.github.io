...
function covers(bundle, urlPattern) {
  ...
  const verdictOf = (where) => {
    ...
    return false;   // not 안쪽은 빼는 것이라 안 센다
  };
...
for (const l of LOCALES) {
  ...
  /* ★ **글자로 찾으면 빼는 규칙까지 있는 것으로 센다** (2026-08-17 실측, 내 검사의 구멍).
     규칙은 허브를 `not` 안에도 적는다. 도구 장은 미리 그리되 허브 자체는 빼라는 뜻이다.
     그런데 글자만 훑으면 그 `not` 이 허브 규칙이 있다로 읽혔다. 실제로 허브 규칙이 통째로
