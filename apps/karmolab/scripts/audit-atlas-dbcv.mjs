...
/* 굽는 쪽 함수를 빌려 오면 「자기가 자기를 확인」이 된다 — 여기서 따로 구현한다.
...
if (isFake(ATLAS) || !fs.existsSync(CACHE)) {
  ...
  if (use.length < 200) {
    ...
    /* ③ **라벨을 마구 섞으면 떨어져야 한다.** 안 떨어지면 이 자는 아무것도 안 재는 것이다.
       (우리 층은 이미 음수라 「떨어질 자리가 없다」가 될 수 있으니 **차이**로 본다.) */
    const li0 = 0;
