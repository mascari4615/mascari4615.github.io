 ...
 *
 * ★ 왜 찍어 내서 커밋하나: 사용자 기기에서 표를 받아 오게 하면 인터넷 없이 돈다가 깨진다.
 *   그래서 표를 코드로 박아 함께 배포한다. 대신 **손으로 고치지 않는다**. 여기서 다시 찍는다.
 *
 * ★ 왜 한 글자씩만인가: 간체→번체는 한 글자가 여럿으로 갈리는 일이 있다(发 → 發/髮).
...
/**
 * 글자 짝을 한 줄짜리 글로 굳힌다. `원본글자 바뀐글자` 를 붙여 쓴 것.
...
/** 여럿으로 갈리는 글자. 화면이 이건 뜻을 봐야 한다고 말해야 하는 자리. */
const ambiguous = (map) =>
  [...map]
