 ...
 *
 * ── 첫 판(KL-159)이 틀렸던 자리 ────────────────────────────────────────────────
 * 창(최근 N시간) 안의 실패, 취소를 그냥 세서 하나라도 있으면 아프다고 했다. 그런데 그 창에는
 ...
 *
 * 실패는 **사유별로 묶는다.** 실패 13은 아무 정보가 아니지만 타입 검사 1, 브라우저 없음 12는
 * 무엇을 고칠지 말해 준다.
 *
...
/* ★ **수명만으로는 대기 밀림과 짓다가 죽음을 못 가른다** (2026-08-14 실측).
   이 배포는 `cancel-in-progress: false` 라 돌던 것은 안 끊는다. 그런데도 짓다가 죽음이
   쌓여 경보가 울렸다. 까닭: 앞 배포가 13분을 도는 동안 **줄 서서 기다리던** 것들이 새 push 에
...
const started = (run) => {
  try {
    ...
    /* 대기만 하다 밀린 판은 작업이 아예 안 돌았거나(시작=끝) 몇 초짜리다. */
    return jobs.some((j) => j.s && j.c && (Date.parse(j.c) - Date.parse(j.s)) / 1000 >= 60);
  } catch {
...
/* 마지막으로 **끝까지 간** 배포. 이 시각이 고쳐졌다의 경계선이다. */
 ...
 *
 * 이게 없어서 한 번 크게 속았다: 마지막 성공 1분 전이라 초록이었는데, 그 성공은 100건
...
/* 실패 사유. 실패 13은 아무 정보가 아니다. 무엇을 고칠지 말해 주는 것은 사유별 개수다.
...
/* 사이트에 서 있는 판. 성공했나보다 이쪽이 사람이 겪는 진실이다. */
const lag = liveLag();
...
/* ── 판정 ──────────────────────────────────────────────────────────────────────
 * 몇 건 있었나로는 판정하지 않는다. 대기 밀림은 많아도 정상이고, 고쳐진 실패는 시체다.
...
if (lastSuccessAt === null) {
  ...
  /* 성공했다와 사람에게 닿았다는 다른 말이다. 올린 것이 열 건 넘게 쌓여 있으면
