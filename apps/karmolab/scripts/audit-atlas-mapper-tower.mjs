...
if (!fs.existsSync(ATLAS)) {
  ...
  if (!tw) {
    ...
    if (!Array.isArray(tw.bars) || !tw.bars.length) bad.push('막대가 없다. 어느 눈금에서 사나를 다시 볼 수 없다');
    ...
    if (!chromium || !fs.existsSync(BUNDLE)) {
      ...
      console.log(`  ② 화면이 눈금 ${lo}~${hi} 사다리, 전 구간 ${tw.full}개를 적나 . `
