 ...
 * 걸렸고, 매번 「빌드는 초록인데 위젯만 안 뜬다」로 시간을 썼다 (2026-08-28 보드 위젯이 마지막).
 *
 * 두 가지를 본다:
 ...
 * 빠져도 여기서 걸린다. 「없는 것」은 검사할 대상이 없어서 조용히 지나가는 종류의 사고다.
 *
...
if (!fs.existsSync(OUT)) {
  ...
  for (const [ns] of asked) {
    ...
    for (const l of ALL_LOCALES) {
      /* 번역이 아직 없는 언어는 폴더 자체가 없다 — 그건 「빠진 번역」이지 이 검사의 대상이 아니다. */
