...
import { resolveLocalPackageDependencies } from './lib/local-package-dependencies.mjs';
...
const materializeCommit = (ref, destination) => {
  try {
    git(['clone', '--shared', '--no-checkout', repoRoot, destination]);
    git(['sparse-checkout', 'init', '--cone'], destination);
    git(['sparse-checkout', 'set', 'apps/karmolab', 'packages', '.github', 'scripts'], destination);
    git(['checkout', '--detach', ref], destination);
    return true;
  } catch {
    return false;
  }
...
/* 의존이 **비어 있는 순간**이 실제로 있다. 옆 세션이 `npm ci` 를 도는 중이면 그렇다.
   그 상태로 재면 tsc 가 없다가 나오고, 이 게이트는 push 를 막으므로 **멀쩡한 커밋이
...
/* 앱이 `file:` 로 무는 꾸러미는 **지어야 형태가 생긴다**. dist 가 없으면 모듈을 못 찾겠다가
...
const buildableLocalPackages = resolveLocalPackageDependencies(appManifest, appRoot)
  .filter((pkg) => {
    const manifestPath = join(pkg.path, 'package.json');
    if (!existsSync(manifestPath)) cannotRun(`${pkg.spec} 의 package.json 이 없다`);
    return Boolean(JSON.parse(readFileSync(manifestPath, 'utf8')).scripts?.build);
  });
for (const pkg of buildableLocalPackages) {
  if (!existsSync(join(pkg.path, 'dist'))) {
    cannotRun(`${pkg.spec} (${pkg.name})의 dist 가 없다. 그 꾸러미를 아직 안 지었다 (npm ci && npm run build)`);
  }
}
...
try {
  /* 독립 clone 의 sparse 설정은 원본 저장소 설정을 건드리지 않는다. `git archive | tar` 는
     Windows bsdtar 가 한글 파일명을 풀지 못하므로 Git checkout 으로 커밋을 재현한다. */
  if (!materializeCommit(sha, work)) cannotRun('그 커밋을 독립 checkout 하지 못했다');
  ...
  /* `node` 는 `.cmd` 가 없다. `npx`, `npm` 만 그렇다. 붙였더니 node.cmd 를 찾을 수 없다로 죽었다. */
  ...
  if (failed.length) {
    ...
    try {
      ...
      if (materializeCommit('origin/main', baseDir)) {
 ...
 * 빨강 1개만 남고 **왜인지 한 줄도 안 나왔다**. 사유 없는 빨강은 게이트가 아니라 벽이다.
