...
/* 두 우연 수준이 서로 크게 다르면 셈이 이상하다. 둘 다 이웃을 안 본 답이다. */
...
/* ③ 좌표만으로도 새는가. 이 값이 없으면 목록을 빼면 된다는 착각을 못 막는다. */
if (L.xyRate == null) bad.push('좌표만 줬을 때가 안 실려 있다. 이웃 목록만 빼면 안전하다를 못 반증한다');
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysXy) bad.push('화면이 **좌표만 줬을 때**를 안 적는다. 목록만 빼면 된다는 착각이 남는다');
