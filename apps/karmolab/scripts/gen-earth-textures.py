...
원본은 대부분 NASA 자료(저작권 없음)이고, three.js 예제 저장소가 오래 두고 쓰는 판을 그대로
받는다. **화성만 Solar System Scope 판(CC BY 4.0)이라 출처 표기 의무가 있다** — 이 저장소는
공개라 파일 자체가 같이 배포되므로, 조건은 `data/earth/CREDITS.md` 에 파일별로 적어 둔다.
새 그림을 더하면 그 표도 같이 늘린다.
