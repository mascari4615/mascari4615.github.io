...
/* 굽는 쪽 함수를 빌려 오면 자기가 자기를 확인이다. 여기서 따로 구현한다. 정의는 같게. */
const WIN = 10;
const EPS = 1e-12;
...
// ── ① 눈금 (지어낸 자료. 지도가 없어도 돈다) ─────────────────────────
{
  ...
  /* 무리마다 제 말주머니를 준다. 제 이름은 그 주머니 말, 엉뚱한 이름은 남의 주머니 말. */
  const BAGS = [
    ['빌드', '배포', '파이프라인', '캐시', '아티팩트'],
    ['고양이', '사료', '털', '병원', '산책'],
    ['환율', '금리', '채권', '증시', '배당'],
...
/* **제목만으로는 못 잰다.** 처음엔 지도에 실린 제목으로 다시 재려 했는데, 창 10 낱말
   안에 이름 말과 대표어가 같이 들 일이 거의 없어 전부 -1 이 나왔다. 다시 재기가
...
if (!withFit.length) {
  ...
  const { collectAll } = await import(new URL('./build-memo-atlas.mjs', import.meta.url).href);
  ...
  levels.forEach((lv, li) => {
    ...
    /* ★ **비긴 것을 어긋남으로 세지 않는다** (2026-08-23). 제 무리와 남의 무리 값이 거의 같으면
       ...
       실제로 층 30 에서 값 차이 0.005 짜리 하나가 뒤집혀 자료를 안 따른다로 빨개졌다.
     ...
     * 11/14(78.6%)가 12/14(85.7%)로 바뀌는 것이 근거가 선다/안 선다를 갈랐고,
     ...
     * (5/6 의 최선이 p 0.109). 그건 찍기를 이긴다가 아니라 한 번도 틀리지 마라다.
  ...
  /* **층 하나로는 표본이 모자란다**. 성긴 층(6개)에서는 걸어 볼 수 있는 무리가 둘뿐이었다.
     층 전부를 돈다. 몇 번 걸어 봤나도 같이 적는다(적으면 그것 자체가 실패다). */
