/**
 * **또는으로 이어 붙인 통과 조건**을 센다 (TASK-KL-304)
 *
...
if (process.argv.includes('--update')) {
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        note: '통과 조건에 또는이 든 검사. 줄기만 한다. 느슨한 쪽이 늘 이긴다 (audit-loose-checks.mjs)',
...
if (added.length) {
  console.log(`[audit-loose-checks] 통과 조건에 또는이 든 새 검사 ${added.length}곳`);
...
console.log(`[audit-loose-checks] 또는이 든 자리 ${found.length}곳 (기준선과 같음. 늘지 않았다)`);
