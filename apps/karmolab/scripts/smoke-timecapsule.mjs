 ...
 *
 * 이 도구의 값어치는 그날 전에는 못 연다한 줄에 전부 걸려 있다. 화면이 뜨는지 보는 검사도,
...
try {
  ...
  /* 단추가 보인다고 손이 달린 것은 아니다. 미리 그린 그림과 진짜 화면 사이 틈 (TASK-KL-135).
     이 검사가 그 틈에서 두 번 헛돌아 공개 시계에 못 닿았다로 끝났다(도구는 멀쩡했다). */
  ...
  await earlier.addInitScript(`{
    ...
    Date.now = () => 진짜() + 앞으로;
    const OD = Date;
    // new Date() 도 같이 앞당긴다. 화면이 지났다고 믿게 만드는 것이 목적이다
  ...
  /* 시계를 돌리면 공개 시계에서 아직 없는 회차를 달라고 하게 되어, 못 연다는 말 대신
