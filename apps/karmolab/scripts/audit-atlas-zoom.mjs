 ...
 *
 * 뜻 있는 당기기(semantic zoom)의 요점은 그냥 커진다가 아니라 **뭉친 것이 갈라진다**
 ...
 *  ③ 층이 바뀌어도 **점은 안 움직인다**
 *  ④ 화면이 지금 층 N, 당기면 갈라진다를 적는다
 *
 * ★ **문턱의 잣대를 한 번 바꿔 박았다.** 처음엔 ZMLT 의 이름 겹침 0을 그대로 문턱으로
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!Array.isArray(sw) || sw.length !== (atlas.levels?.length || 0)) {
    ...
    /* 문턱이 **잰 값**인지: 바로 아래는 예산을 넘고, 바로 위는 예산 이하여야 한다.
       ...
       탐침은 이웃 문턱까지 거리의 절반까지 줄인다. 바로 아래, 바로 위라는 질문 그대로다. */
    for (let i = 1; i < sw.length; i += 1) {
  ...
  if (Array.isArray(sw) && sw.length > 1) {
    ...
    /* ★ **그냥 얼마나 움직였나를 재면 안 된다**. 배율이 1% 달라졌으니 화면 끝 점은
  ...
  if (!saysNow) bad.push('화면이 지금 층 N을 안 적는다. 저 혼자 바뀌는 것처럼 보인다');
