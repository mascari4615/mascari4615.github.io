 ...
 *
 * 왜 has: 높은 쪽 고르기가 실서비스에서 **죽어 있었다**. 열면 장비 꺼내는 중이에요...
 ...
 *
 * 왜 기존 검사가 못 잡았나:
 *  , `audit:i18n-load` 는 그 파일이 `loadNamespace` 를 부르나만 본다. higher 는 **부른다** . 
 ...
 *   ② 그 놀이 판에 아직 꺼내는 중만 있으면 빨강 (= 지어지다 말았다)
 *
...
/* ★ **놀이만이 아니었다** (2026-08-14, 같은 날 다섯 건 더). 같은 병으로 반려동물, 활동, 광장, 
   내 정보, 상태가 실서비스에서 죽어 있었다. 전부 **도구 장이 없는 앱 안 화면**이라
...
/* ★ **데스크톱 전용 화면도 본다** (2026-08-14). 브라우저에서는 셸이 그것들을 감춰서
   `switchPage` 가 아무 일도 안 한다. 그래서 앱 안 화면이 아님으로 넘어갔고,
...
/* ★ **한 판씩 여느라 3분을 썼다** (2026-08-19 실측: 이 검사 하나가 게이트 158개 합계
   707초 중 192초 = 27%). 안을 열어 보니 값이 두 군데였다:
     ① 화면마다 **고정 1.2초 잠자기**. 59개면 71초. 이 저장소가 이미 이름 붙여 막는
        ...
        → 지어졌나를 **조건으로** 기다린다. 못 기다리면 예전 그 자리(1.5초)에서 멈춘다.
...
async function onePage(id) {
  ...
  try {
    ...
    /* **다 지어졌나를 묻는다. 초를 세지 않는다.** 판정이 보는 것은 둘뿐이다:
       판에 글이 찼나, 셸이 못 열었어요로 바꿨나. 둘 중 하나가 되면 더 기다릴 이유가
    ...
    const locale = await page.evaluate((x) => {
      const el = document.getElementById('page-' + x);
      /* ★ **건너뛰는 까닭을 갈라 적는다** (2026-08-14). 판이 안 뜬다를 한 통에 담으면
    ...
    /* ⚠ 만드는 쪽은 `content`, 읽는 쪽은 `text` 였다 (2026-08-21 고침). 없는 필드라 늘 참 . 
       59개 중 48개가 화면이 비어 있다로 빨갰는데 실제로는 멀쩡히 떠 있었다(글 212자 확인).
       한 커밋(`6b1113723`) 안에서 `글` 을 한쪽은 `content`, 다른 쪽은 `text` 로 바꾼 탓이다. */
    } else if (locale.didNotLoad || !locale.content) {
      /* 불러오는 중이 **화면의 전부**일 때만 멎은 것으로 본다. 다 지어진 화면 안에도
         그런 글자가 한 조각 있을 수 있다. 서버 모니터가 그랬다(브라우저에서는 제 서버에
         못 닿아 한 칸이 불러오는 중이다. 그건 고장이 아니라 그 화면의 정상이다). */
      red.push(`${id}: 판이 안 지어졌다. ${locale.didNotLoad ? "셸이 못 열었어요로 바꿨다" : "화면이 비어 있다"}`);
    } else {
      process.stdout.write('.');
    }
  } catch (e) {
