...
try {
  ...
  /* ★ **안 구운 자리에서는 못 돌림이다** (2026-08-17 실측). 이 검사를 미는 자리에 붙였더니
     갓 꺼낸 커밋에는 `sw.js` 같은 **구운 것**이 없어 생성기가 못 돌고 배포가 멈춘다로 빨개졌다.
 ...
 * 사람이 열어 보기 전에는 아무도 모른다. 그래서 여기서 실제로 **읽혀지는지** 본다. */
{
  ...
  const check = (file) => {
    ...
    for (const m of html.matchAll(/<script(?![^>]*\bsrc=)[^>]*>([\s\S]*?)<\/script>/g)) {
       ...
       * 받아 둬라를 알려 주는 미리읽기 규칙(speculationrules). 둘 다 내용은 JSON 이다.
       * 이걸 자바스크립트로 읽으려 하면 첫 중괄호를 코드 묶음으로 보고 곧바로 실패한다 . 
       * 실제로 그래서 멀쩡한 페이지 여섯 장이 스크립트가 안 읽힌다로 잡혔다. */
...
/** 조용히 반쪽만 찍히는 경우를 막는다. 돌긴 돌았다는 통과 조건이 아니다. */
