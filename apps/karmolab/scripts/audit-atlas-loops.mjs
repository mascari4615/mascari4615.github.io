...
if (!fs.existsSync(ATLAS)) {
  ...
  if (!h1) {
    ...
    if (!h1.rand || typeof h1.rand.rank !== 'number') {
      bad.push('마구 섞은 점의 고리 수가 안 실려 있다 — 그것 없이 「고리 N개」는 아무 뜻이 없다');
    } else {
    ...
    if (!chromium || !fs.existsSync(BUNDLE)) {
      ...
      if (!saysN) bad.push('화면이 「고리 N개 · 가장 짧은 것은 마디 몇 개」를 안 적는다');
      if (!saysRand) bad.push('화면이 **마구 섞은 점의 고리 수**를 안 적는다 — 「고리가 있다」가 발견처럼 읽힌다');
