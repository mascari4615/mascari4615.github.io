 ...
 * 화면이 그걸 「덩어리」라 부르면 없는 경계를 있다고 말하는 셈이라, **말만** 바꿨다.
 *
 * ⚠ 실루엣은 볼록한 무리를 전제하고 고차원에서 약해진다(문헌 명시). 그러니 「0.045 니까
 * 나쁘다」로 단정하지 않는다 — **상대 신호**로만 쓴다: 곡선끼리 견주고, 말을 고른다.
 *
 * 이 자가 보는 것 셋:
 ...
 *
 * ③ 이 관계인 게 핵심이다. 「구획이라 적혀 있나」로 걸면 그 낱말을 박아 둔 화면도 통과한다.
 */
...
const SIL_REAL = 0.15;   // 이보다 낮으면 「덩어리」가 아니라 **구획**이라 부른다
const bad = [];

// ── ① 실린 실루엣이 다시 재도 맞나 ────────────────────────────────────
/* 굽는 쪽 함수를 빌려 쓰면 「자기가 자기를 확인」하는 꼴이라 여기서 따로 구현한다.
...
function silhouette(points, assign, offset, pickMask) {
  ...
  /* **식구는 표본이 아니라 전부다.** 처음엔 뽑은 점들만 서로의 식구로 셌는데, 굽는 쪽은
     ...
     (층 6 에서 0.034 vs 0.023) 그걸 「실린 값이 틀렸다」로 읽을 뻔했다. 정의를 맞춘다 —
     셈은 여전히 여기서 따로 한다. */
...
if (isFake(ATLAS) || !fs.existsSync(CACHE)) {
  ...
  /* **가장 많이 맞물리는 층을 고르면 안 된다** — 갈아 치운 옛 모델도 같은 1516개를 갖고
     있어서 그쪽을 집어 들었고, 그러곤 「실린 값이 안 맞는다」고 했다. 지도가 **누가
  ...
  if (use.length < 200) {
    ...
    /* 굽는 쪽과 **같은 손**: 모두가 공유하는 쏠림을 빼고 다시 정규화한 벡터로 잰다.
       이걸 안 빼면 값이 0.24 로 보인다 — 그건 「모두가 공유하던 방향」이 만든 착시고,
       지도를 그린 벡터가 아니다. */
     ...
     * 표본이 다르면 값도 조금 다르다. 그래서 「0.01 안이면 같다」는 박아 둔 상수였고,
    ...
    levels.forEach((lv, li) => {
       ...
       *
       * 「같은 1200개」인 줄 알았는데 실은 서로 다른 1200개였다 — 굽기는 자기 차례대로,
...
/* **관계로 건다.** 같은 화면에 숫자만 올렸다 내렸다 하며 낱말이 따라오는지 본다.
   「구획이라 적혀 있나」로 걸면 그 낱말을 박아 둔 화면도 통과한다. */
let chromium;
...
/* 「안 갈린다」를 한 수로만 말하면 고칠 수가 없다. 갈려 보이느냐는 **여러 요인의 결과**다 —
...
}
{
  ...
  /* ★ **「요인이 서로 안 섞여야 한다」는 내 가정이 틀렸다.** 표준화 겹침 = 중심 거리 ÷ 퍼짐이라
