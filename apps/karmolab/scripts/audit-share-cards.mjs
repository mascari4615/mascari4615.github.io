...
/* 작업대로 합친 옛 도구는 낱개 장이 없다. 그 주소는 작업대로 보내는 안내라 공유 카드도
   안 그린다(gen:og 도 같은 목록으로 건너뛴다). 도구가 아닌 것을 카드가 없다로 세면
...
/* ★ **못 연 것이 많으면 판정하지 않는다** (2026-08-14 red-walk).
   아직 배포 안 된 새 도구는 한둘이다. 그런데 이 검사는 **몇 개든** 그렇게 봐줘서,
   안 서 있는 주소로 돌리면 122개를 전부 다음 배포 대기로 넘기고
   `0개 도구가 저마다 공유 카드를 갖고 있다` 며 **초록**으로 끝났다.
   못 연 것은 카드가 있다가 아니다. 열에 하나를 넘으면 CANNOT-RUN(2). */
...
if (unreachable.length >= threshold) {
  ...
  console.error('  못 연 것은 카드가 있다가 아니다. 판정하지 않고 지나간다. 주소가 맞는지, 사이트가 서 있는지 보라.');
  process.exit(2);
}
