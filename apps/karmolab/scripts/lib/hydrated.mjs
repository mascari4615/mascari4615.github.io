 ...
 * 127ms 에 교체**였다.
 *
 * 그래서 단추가 보인다로 기다림을 끝내면 그 사이 51ms 에 걸린다. 거기서 적은 글은 교체와
 * 함께 사라지고, 누른 것은 아무 일도 안 난다. 실제로 타임캡슐 검사가 그 틈에서 두 번 헛돌아
 * 바깥 시계에 못 닿았다로 끝났다(도구는 멀쩡했다).
 *
 ...
 *   내려가서, 미리 그린 HTML 에 값을 넣고 값을 넣어 주세요로 끝난다. 화면은 멀쩡한데
...
export async function waitHydrated(page, selector, opts = {}) {
    ...
    .waitForFunction(
      (sel) => {
        ...
        /* ★ **준 그 요소가 단추면 그 단추에게 묻는다** (2026-08-13).
           예전에는 같은 화면 안 아무 단추 하나라도 손이 붙었으면이었다. 그런데 도구 상세
           ...
           두 판본을 모두 넣어 주세요로 끝났다(실사이트 실측). 준 요소가 단추면 그 단추다. */
  ...
  /* 단추가 없어 신호를 못 본 화면(그리고 시간이 다 된 경우)은 가라앉기만 기다린다.
     못 기다린 것을 기다렸다로 삼키지 않으려고 결과를 돌려준다. */
