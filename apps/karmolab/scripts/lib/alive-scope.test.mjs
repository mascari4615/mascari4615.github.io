...
import { 열어볼것 } from './alive-scope.mjs';

test('손댄 도구만 고른다', () => {
  assert.deepEqual(열어볼것(['apps/karmolab/src/widgets/bluemarble/bluemarble.ts']), ['bluemarble']);
});
...
test('껍데기를 건드렸으면 전부 본다 — 어느 도구든 죽을 수 있다', () => {
  for (const f of ['apps/karmolab/src/toolbox.ts', 'apps/karmolab/index.html',
    ...
    assert.equal(열어볼것([f, 'apps/karmolab/src/widgets/x/x.ts']), null, f);
  }
});

test('못 물어봤으면 좁히지 않는다 — 모르는 것을 아는 척하지 않는다', () => {
  assert.equal(열어볼것(null), null);
  assert.equal(열어볼것(undefined), null);
});

test('도구와 상관없는 파일만 바뀌었으면 열 것이 없다', () => {
  assert.deepEqual(열어볼것(['memo/rules/quality.md', 'apps/karmolab/scripts/x.mjs']), []);
});

/* ★ CI 는 갓 꺼낸 체크아웃이라 바뀐 파일 목록이 **빈다**. 그걸 「0개」로 읽으면 이 검사가
   CI 에서 한 번도 안 돈다 — 빈 신호는 「없다」가 아니라 「모른다」다 (2026-08-17). */
test('신호가 아예 없으면 좁히지 않는다(= 전부)', () => {
  assert.equal(열어볼것([]), null);
});
