...
export const WANT = [
  ...
  /* ★ 찾는 표식을 바꿨다 (2026-08-12). 놀이가 앱 셸 위젯으로 옮겨지면서 `id="picks"` 같은
     ...
     주소도 옮겨졌다. `/higher/` 는 이제 661바이트짜리 자리를 옮겼다 안내이고
     진짜 놀이는 `/play/higher/` 에 있다. 옛 주소를 보면 영원히 없다가 나온다. */
  ['놀이. 높은 쪽 고르기', '/play/higher/', /KARMOLAB_ENTRY_STATIC="play"[\s\S]*?높은 쪽 고르기/, '목록에서 링크만 걸리고 열면 없다'],
  ['놀이. 오늘의 문제', '/play/quest/', /KARMOLAB_ENTRY_STATIC="play"[\s\S]*?오늘의 문제/, '목록에서 링크만 걸리고 열면 없다'],
