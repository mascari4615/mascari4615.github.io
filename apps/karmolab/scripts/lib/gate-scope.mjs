 ...
 * 왜 이리 오래 걸려만 물었다. 기다린 시간이 만든 시간을 넘기면 게이트를 안 믿게 된다.
 *
 * ## 안전 기본값. 발판을 안 적은 검사는 **언제나 돈다**
 *
 * 무엇을 딛는지를 157개에 다 적어야 시작할 수 있는 설계였다면 영영 시작 못 한다.
 * 그래서 반대로 뒀다: `볼것` 이 적힌 검사만 건너뛸 수 있고, 안 적힌 것은 그냥 돈다.
 * 처음부터 옳고(빠지는 검사 0), 적을수록 빨라진다.
 *
 * 이건 **개발 중**만 쓰는 길이다. push, CI 는 통짜 그대로다. 내 자리에선 초록이
 * 배포를 빨갛게 만드는 종류의 사고를 여기서 만들지 않는다.
 */

/** 목록 한 줄을 { name, watch } 로 편다. 문자열이면 언제나 돈다다. */
