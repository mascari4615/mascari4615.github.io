/**
 * 「이 검사는 무엇을 딛는가」를 **스스로 알아낸다** (TASK-KAR-231).
 *
 * ## 왜 이게 필요했나
 *
 * `gate-scope.mjs` 가 2026-08-19 에 「바뀐 것에 걸리는 검사만」을 세웠다. 기계는 옳았는데
 ...
 * 돌렸다. 「왜 이리 오래 걸려」가 이틀 연속 나온 이유가 이것이다.
 *
 * 149개를 손으로 적는 설계는 어제도 안 됐고 내일도 안 된다. **적게 하지 말고 알아내게 한다.**
 *
 * ## 무엇을 근거로 알아내나 (셋 다 「실제로 있는 것」만 쓴다)
 *
 ...
 * ## ★ 아무 것도 못 알아내면 **언제나 돈다**
 *
 * `gate-scope.mjs` 의 안전 기본값을 그대로 지킨다. 모르는 것을 「걸릴 것 없다」로 바꾸는 게
 * 이 저장소에서 제일 비싼 고장이다. 그래서 이 파일은 **건너뛸 근거를 만들 뿐**,
 * 「건너뛰어도 된다」를 스스로 단정하지 않는다.
 *
 ...
 * `gate-list.json` 은 「무엇을 돌릴지」인데 그 목록은 이 판이 **매번 새로 읽는다**.
 *
...
export function deriveWatch(gateName, scripts, read = (p) => fs.readFileSync(path.join(APP, p), 'utf8')) {
  ...
  /* ★ 근거가 「자기 자신 + 언제나 자리」뿐이면 **알아낸 게 없는 것**이다.
