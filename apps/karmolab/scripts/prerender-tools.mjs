 ...
 *
 * 왜: 지금 도구 상세 화면의 첫 그림은 HTML → CSS → JS → JS 실행 → 그림이다. 그때까지
...
/* ★ **작업대로 합친 옛 도구는 미리 그릴 게 없다** (2026-08-13). 그 자리는 이제 작업대로
   보냅니다 한 장짜리 안내다. 위젯이 없으니 화면이 안 박히고, 그걸 안 박혔다로 세면
...
/* 서버가 없으면 **못 돈다고 말한다**. 예전에는 여기서 조용히 0 으로 끝냈는데, 배포에서
   그 침묵이 했다로 읽혀 실제 사이트가 내내 빈 채로 나갔다 (2026-08-08). */
...
let done = 0;
let skipped = 0;
/** 못 그린 것. 일부러 건너뜀과 **다르다**. 이게 조용히 넘어가면 배포마다 빈 칸이 쌓인다. */
const failed = [];
...
await browser.close();
/* 끝나고 **결과를 직접 센다**. 몇 장 돌렸나가 아니라 몇 장에 실제로 박혔나.
   이 줄이 없어서, 도중에 끊긴 판이 건너뜀으로 보이고 넘어갔다. */
