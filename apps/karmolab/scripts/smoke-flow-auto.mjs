 ...
 *
 * 자동화라고 적어 놓고 사람이 단계마다 다음을 눌러야 했다. 그 클릭 하나가
 * 자동화와 반자동을 갈랐다. 여기서는 진짜 브라우저에서 그 클릭이 **정말 없어졌는지**를 본다.
 *
 * 서버는 안 쓴다. 서버가 하는 일은 켰다를 기억하는 것뿐이고(도구는 전부 브라우저 안에서
 ...
 *    안 그러면 자동이 됐다가 아니라 원래 넘어가고 있었다를 자동이라 부르게 된다. */
await seed(false);
await fireResult();
 ...
 * 흐름 화면을 떠났나로만 보고, 몇 단계인지는 띠에게 묻는다. */
...
if (stopped !== 0) problems.push(`잠깐을 눌렀는데 ${stopped + 1}단계로 넘어갔다`);
...
console.log('✅ 스스로 이어감. 꺼져 있으면 안 넘어가고, 켜면 초를 세고 넘어가고, 잠깐이면 멈춘다');
