 ...
 * 그건 네 자(실루엣, DBCV, HDBSCAN, H0)가 이미 말한 구획이지 무리가 아니다와 같은 말이다.
 *
 * 그래서 자를 이렇게 고쳐 박는다:
 *  ④-가 **재는 손은 구조를 볼 줄 안다**. 지어낸 갈린 셋에서 우리/섞은 것의 차 ≥ 0.3
 *  ④-나 **문턱을 박지 않는다**. 실린 문턱이 차가 가장 큰 자리인지 곡선으로 다시 고른다
 *  ④-다 **바탕값을 화면에 적는다**. 85% 남았다를 섞어도 64% 는 남는다 없이 적으면 거짓말
 */
...
if (!fs.existsSync(ATLAS)) {
  ...
  if (!cf) {
    ...
    if (cf.comps.length === 1 && cf.comps[0][1] === cf.runs && cf.survival.every((v) => v === 1)) {
      /* 다 1 이면 재고 있는 게 맞나를 의심해야 한다. 실측은 43개 중 8개가 흔들렸다. */
...
if (!chromium || !fs.existsSync(BUNDLE) || !atlas?.skeleton?.confidence) {
  ...
  console.log(`  ③ 화면이 ${cf.runs}판 중 조각 ${cf.mode}개를 적나. ${saysRuns && saysComp ? '적는다' : '**안 적는다**'}`);
  if (!saysRuns || !saysComp) bad.push('화면이 스무 판 중 몇 판에서 같은 조각 수를 안 적는다. 한 판의 답으로 읽힌다');
  /* ④-다 **바탕값을 적나**. 이 자의 핵심이다. 우리 값만 적으면 단단한 뼈대로 읽힌다. */
  ...
  if (!saysBase) bad.push('화면이 **바탕값**을 안 적는다. N% 살아남았다만 적으면 단단한 뼈대로 읽힌다');
