 ...
 * 등고선은 여기가 빽빽하다만 말하고 **여기서부터 남이다는 말하지 않는다.**
 *
 ...
 *
 * ★ 높낮이 0.2 면 지형이 있다로 처음 재려다 **물어 보니 안 물었다**(고르게 흩은 지도를
...
if (!(info?.base > 0)) bad.push('고르게 흩은 점의 바탕값이 안 실려 있다. 그것 없이 높낮이 N은 아무 뜻이 없다');
else {
...
if (!(after.sat < before.sat * 0.85)) {
  bad.push(`지형을 켜도 덩어리 색이 그대로다 (색기 ${before.sat}→${after.sat}). 그림이 아직 여긴 한 무리라고 우긴다`);
}
...
if (!saysBase) bad.push('화면이 **고르게 흩었을 때의 바탕값**을 안 적는다. 높낮이 N이 발견처럼 읽힌다');
console.log(`  ③ 화면. 높이 = 몰린 정도 ${saysHeight ? '○' : '✗'}, 경계는 없다 ${saysNoEdge ? '○' : '✗'}`
  + `, 봉우리 수 ${saysPeaks ? '○' : '✗'}`);
if (!saysHeight) bad.push('화면이 높이 = 몰린 정도를 안 적는다');
