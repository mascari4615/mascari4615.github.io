 ...
 *
 * 지도의 값어치는 어디서 읽나에 있는데, 그 주소가 404 면 그 칸은 빈 칸이다.
 * 내용 검사는 자동으로 못 해도 **주소가 죽었는지**는 기계가 볼 수 있다. 그래서 여기서 본다.
 *
 * HEAD 를 막는 곳이 많아 GET 으로 한 번 더 확인한다. 403/405 는 사람만 받는다는 뜻이라
...
/* ★ **한 번 못 닿았다고 315개를 통째로 버리지 않는다** (2026-08-16, 실측). 아예 못 닿은 것이
   ...
   그때가 진짜 못 닿음이다. 이건 재우고 넘어가는 것이 아니라 **다시 물어보는** 것이다. */
...
/* ★ **5xx 도 죽었다가 아니다** (2026-08-21 실측: `tosdr.org` 가 503 을 냈다).
