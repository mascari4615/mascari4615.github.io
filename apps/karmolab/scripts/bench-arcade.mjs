 ...
 *
 * 51개를 만들면서 한 번도 **재 본 적이 없다.** 해 보니 되더라는 51번 반복할 수 없는 말이라,
 ...
 *  ③ **판 길이**. 진짜 몇 초짜리 놀이인가 (로비에 30초/3분을 적으려면 이 수가 있어야 한다)
...
const STEP = 50;
/** 이만큼 밀어도 안 끝나면 안 끝나는 판으로 적는다 (계약 검사가 따로 막고 있다). */
const CAP = 400000;
...
for (const g of GAMES) {
  ...
  rows.push({
    ...
    /* 씨앗을 바꿔도 판이 하나뿐이면 봇끼리는 **늘 같은 판**이다. 그런 판의 slope는
