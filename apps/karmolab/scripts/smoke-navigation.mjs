 ...
 * 그래서 적혀 있나가 아니라 **실제로 그렇게 됐나** 를 브라우저에 물어본다.
 *
 * 보는 것:
...
/* 이 검사는 **도구 상세 페이지가 이미 만들어져 있어야** 말이 된다 (`npm run gen:tool-pages`).
   없는데 그냥 돌면 링크를 못 찾았다로 죽는데, 그건 제품 고장이 아니라 **검사를 못 돌린 것**이다.
   그 둘을 같은 빨간불로 말했더니 사이트 배포가 몇 시간 섰다 (2026-08-08). */
const toolPagesDir = path.join(repoRoot, 'apps', 'blog', 't');
/* 못 돌린다를 **빨강으로** 말하면 안 된다. 바로 아래 짝 검사(HUB)는 건너뛰는데 이쪽만
 * 죽어서, 세 슬롯의 push 와 배포가 통째로 섰다(2026-08-08). 두 문지기가 같은 일에 서로 다른
 * 답을 내고 있었다. 이제 build 가 이 검사 앞에서 페이지를 찍으므로(`gen:tool-pages`),
 * 여기 걸린다는 건 아직 안 찍었다뿐이다. */
 ...
 * 없는 채로 그냥 돌리면 404 를 제품 고장으로 보고한다: 실제로 그렇게 나가서 build 가
...
try {
  ...
  /* 브라우저가 아예 없으면 검사가 실패했다가 아니라 **검사를 못 돌렸다**이다.
 ...
 * (`PrerenderingDisabledByDevTools`). 그러니 도착해 보니 미리 돌아 있더라로 재려 하면
 ...
 * 그 장치가 사라지면 미리 도는 화면이 전부 방문으로 세인다(그래서 예전에 못 켰다). */
{
  ...
  if (!src.includes('prerenderingchange')) {
    problems.push('계측이 도착했을 때를 안 본다. 미리 도는 화면이 방문으로 세인다');
  }
}
 ...
 * 스스로 끈다**(`BackForwardCacheDisabledForDelegate`). 그래서 되살아났나로 재면 영원히
 ...
 * 위 ④ 는 지금 이 화면만 본다. 다른 화면에 하나 달리면 그 화면만 조용히 잃는다. */
{
  const bad = [];
  const walk = (dir) => {
