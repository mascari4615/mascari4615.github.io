 ...
 *
 * 2026-08-21, 사람이 지도를 열었더니 이 화면을 못 열었어요만 떴다. 그때 이 지도에는
...
const server = http.createServer((req, res) => {
  ...
  /* ★ **앞머리(front matter)를 떼고 낸다.** 실제 사이트는 Jekyll 이 떼고 내보내는데,
     ...
     셸이 안 뜬다. 그러면 진짜 셸에서 안 뜬다가 자료 탓인지 셸 탓인지 못 가른다. */
...
if (state.failed) bad.push('셸이 이 화면을 못 열었어요를 띄웠다. 위젯이 등록을 못 했다');
if (state.stuck) bad.push('장비 꺼내는 중에서 멈췄다. 등록 모양이 셸과 다르다(tabs[].build 인가)');
