 ...
 *   **조용히 그 예비값을 쓴다.** 그래서 테마를 따른다고 적어 놓고 한 번도 안 따를 수 있다.
 *   2026-08-16 실측: 언어 메뉴가 그렇게 **양쪽 판에서 늘 어두웠고**(토큰 이름 4개가 틀림),
 *   도구 아이콘 속 네모는 늘 검정이었다. 어느 검사도 못 잡았다. CI 의 접근성 검사가
 *   밝은 판 글자색 위에 어두운 바탕이라는 이상한 조합을 찍어 준 뒤에야 알았다.
 *
 * 세는 법: `--이름:` 로 값을 주는 자리와 `setProperty('--이름'` 를 전부 모아 있는 것으로 치고,
...
function walk(dir, fn) {
  for (const e of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    /* 해시 붙은 사본(`tools.42ccc66d.css`)은 **캐시용 생성물**이고 .gitignore 대상이다.
       소스가 아니라 옛 판이라 여기서 세면 이미 고친 것이 계속 빚으로 남는다(실측). */
...
/* 주석 안의 `var(--px)` 는 **설명**이지 코드가 아니다. 실제로 남은 4종이 전부
   이렇게 쓰지 마라고 적어 둔 주석이었다(2026-08-16). 세기 전에 주석을 걷어낸다.
