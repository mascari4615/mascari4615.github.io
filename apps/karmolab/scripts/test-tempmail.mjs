 ...
 * ② **지난 주소를 되살리지 않나**. 되살리면 편지가 안 온다로 보인다(고장처럼).
 * ③ **남은 시간이 0에서 끝이라고 말하나**. 곧 사라집니다가 몇 분째 떠 있으면
 *    그건 시계가 아니라 장식이다.
 *
...
/* ★ 지난 주소를 되살리면 사람은 편지가 안 온다로 본다. 고장처럼 보이는 정상이다. */
...
/* **쓰는 자리**를 본다(`localStorage.`). 낱말만 보면 왜 안 쓰는가를 적은 주석이 걸린다.
...
check(/사라졌/.test(M.leftSay(now - 1, now)), `★ 지났으면 끝이라고 말한다 (나온 것 ${M.leftSay(now - 1, now)})`);
