 ...
 *
 * ★ 비교 슬라이더. 겹치는 **방향**. 왼쪽에 전, 오른쪽에 후가 와야 하는데 자르는 쪽을
...
await page.evaluate((cat) => {
  ...
  window.Toolbox = {
    ...
    trackUse() {},
    copyText() {},
    /* 뒷정리를 **붙잡아 둔다**. 이 검사의 절반이 그걸 부르면 진짜 멈추는가다. */
...
const cmp = await page.evaluate(async () => {
  ...
  /* 말 묶음을 받은 뒤에 그려진다. 부르자마자 읽으면 칸이 없다로 헛보인다. */
...
if (cmp.notDrawn === true) fails.push('비교 슬라이더가 8초 안에 안 그려졌다');
  ...
  /* 손잡이가 가운데(0.5)일 때 왼쪽은 전, 오른쪽은 후여야 한다. */
