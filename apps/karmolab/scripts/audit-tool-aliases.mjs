 ...
 * 기기 안 AI를 만들어 놓고 **번역이라고 치면 0건**이었다. 넓혀 보니 북마크, 게시판, 
 * 통계, 도움말, 제비뽑기, 말동무도 전부 0건이었다.
 *
 * 사람은 도구 이름이 아니라 **하려는 일**로 찾는다. 이름에 그 낱말이 없으면 만들어 둔 것이
 * 없는 것과 같다. 그래서 별칭 없는 도구를 세고, 지금보다 늘면 세운다.
 *
 * 0 으로 잠그지 않는 이유: 이름 자체가 이미 찾는 말인 도구가 있다(글자수 세기를 글자수로
...
if (missing.length > was) {
  ...
  console.error('  `data/tool-aliases.json` 에 사람이 칠 법한 말을 적어라. 도구 이름이 아니라 하려는 일로.');
