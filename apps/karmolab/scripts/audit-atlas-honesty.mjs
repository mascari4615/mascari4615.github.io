#!/usr/bin/env node
/**
 * audit-atlas-honesty. **이 자리는 못 믿는다가 정말 못 믿는 자리인가** (TASK-KAR-233).
 *
 * 지도에 붉은 테두리로 여기선 옆에 있어도 남남이다를 그린다. 그 표시가 틀리면
...
const SHUFFLES = 200;     // 믿음 점수를 마구 섞어 우연히 이만큼 갈릴 확률을 낸다
 ...
 * 아무 근거 없이 박아 둔 상수였다. 1.49 는 안 되고 1.51 은 된다고 말할 근거가 없다.
