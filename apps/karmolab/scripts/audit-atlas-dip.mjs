#!/usr/bin/env node
/**
 * audit-atlas-dip — **「갈린다」에 p 값이 붙어 있나** (TASK-KAR-233).
 *
 * 여기까지 「구획이지 무리가 아니다」의 근거는 전부 **문턱을 손으로 고른 자**(실루엣·DBCV·
 ...
 * 「가장 갈라 보이는 방향」에 투영하면 두 봉우리처럼 보인다. 그래서 대조군 둘을 건다:
 *  · **아무 방향** 에 투영했을 때
 ...
 *  ③ 짝마다 p 를 내고 화면이 「짝 N개 중 몇 개가 갈린다」와 **대조군 둘**을 적는다
...
if (!fs.existsSync(ATLAS)) {
  ...
  if (!withDip.length) {
    if (isFake(ATLAS) || !(atlas.levels || []).length) {
      ...
      console.log('[dip] **「갈린다」에 p 값이 없다** (levels[].dip) — 문턱을 손으로 고른 자만 있다');
      process.exit(1);
    }
  } else {
...
if (!chromium || !fs.existsSync(BUNDLE) || !atlas) {
  ...
  if (!saysSplit) bad.push('화면이 「짝 N개 중 몇 개가 갈린다」를 안 적는다');
  if (!saysRuns) bad.push('화면이 **되뽑기 판 수와 p 바닥**을 안 적는다 — p 가 표에서 온 값처럼 읽힌다');
  if (!saysCtrl) bad.push('화면이 **대조군 둘**(아무 방향·거짓 쪼개기)을 안 적는다 — 「갈린다」가 발견처럼 읽힌다');
  await browser.close();
}

if (bad.length) {
  console.log('[dip] **「갈린다」를 제대로 재지도, 제대로 말하지도 않는다**');
