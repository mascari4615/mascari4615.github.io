 ...
 *  - **어떤 값도 「맞는 값」이 아니다** — 여러 값에서 살아남는 구조가 믿을 만한 것이다
 ...
 *
 * 이 자가 보는 것:
 *  ① 손잡이와 **표**가 실려 있나 (「왜 이 값이냐」를 다시 볼 수 있어야 고른 게 된다)
 *  ② 고른 자리가 표에서 **최선**인가
...
if (!fs.existsSync(BUNDLE)) {
  ...
  if (!chromium) {
    ...
    if (!saysWarn) bad.push('화면이 「덩어리 사이 거리는 뜻이 없다」를 안 적는다 — 정본 경고를 사람이 못 본다');
...
console.log('[umap] 손잡이를 재서 골랐고, 표가 실려 있고, 화면이 「거리는 뜻이 없다」를 적는다');
