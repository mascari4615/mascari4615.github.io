...
odejs
ode.exe`)가 두 토막으로 갈려
     명령이 아예 안 뜨고, 그게 조용한 빨강으로 둔갑한다(2026-08-17 시험이 잡음). */
...
let code = run();
/* 2 = 못 돌렸다(CANNOT-RUN). 다시 재도 같은 답이고, 진짜 빨강이다로 적으면 거짓이다. */
if (code !== 0 && code !== 2) {
...
/**
 * ★ **내 코드가 안 올라간 화면을 보고 빨개진 것은 빨강이 아니라 못 돌림이다** (2026-08-17 실측).
 * 그날 실주소 검사가 달리 부르는 이름이 없다. tts, printkit, nettool, protobuf로 빨갰는데,
 ...
 * 초록은 건드리지 않는다. 옛 판이어도 실주소가 성하다는 그 자체로 사실이다.
 */
