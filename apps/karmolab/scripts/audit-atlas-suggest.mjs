#!/usr/bin/env node
/**
 * audit-atlas-suggest. **이어야 할 둘이 정말 맞나** (TASK-KAR-233).
 *
 ...
 * , **후보 폭증**: 우리도 **통틀어 186만 쌍**이다. 순위 없이 찾았다는 무의미하다
...
if (!S) {
  ...
  console.log('[suggest] **이어야 할 둘을 재 본 표가 없다** (suggest)');
  process.exit(1);
}
if (S.skipped) {
...
/* ④ ★ **후보 수가 이 자의 심장**. 안 적으면 찾았다가 발견처럼 읽힌다. */
if (!(S.pool > 100)) bad.push(`한 글당 후보가 ${S.pool}개다. 잘라 낸 수를 적고 있지 않은지 봐라`);
if (S.pairsAll == null) bad.push('통틀어 몇 쌍인지가 없다. 후보 폭증을 안 적으면 찾았다가 발견처럼 읽힌다');
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysPool) bad.push('화면이 **후보 수**를 안 적는다. 찾았다가 발견처럼 읽힌다');
  if (!saysHow) bad.push('화면이 **이만큼 중 몇 등으로 읽어라**를 안 적는다');
  await browser.close();
}

if (bad.length) {
  console.log('[suggest] **이어야 할 둘을 제대로 재지도, 제대로 말하지도 않는다**');
