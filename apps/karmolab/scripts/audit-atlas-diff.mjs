#!/usr/bin/env node
/**
 * audit-atlas-diff. **밀도 차가 진짜 차이인가, 그냥 어느 쪽이 많나인가** (TASK-KAR-233).
 *
 * 갈래 색으로는 어디에 무엇이 있나까지만 보인다. 밀도 **차**는 그 다음을 묻는다:
 ...
 * 모든 칸을 이긴다. 그러면 그 그림은 밀도 차가 아니라 어느 쪽이 더 많나를 다시 그린 것이다.
 *
...
if (isFake(ATLAS)) { console.log('[diff] 가짜 지도다. 밀도 차는 진짜 굽기에서만 잰다'); process.exit(0); }
...
/** 지도를 하나 띄우고 밀도 차를 한 번 켠 뒤 값을 읽는다. */
async function run(mutate, clicks = 1) {
  ...
  /* 재움-의도: 누를 때마다 쌓이라고 두는 틈이다. 값이 멎기를 기다리는 자리가 아니다. */
...
/* 지어낸 자료: A 는 절반이 왼쪽 위, 절반이 가운데, B 는 절반이 오른쪽 아래, 절반이 가운데.
   **가운데 칸은 두 무리에서 각각 절반씩**이라 정규화하면 정확히 반반이 된다.
   (처음엔 A 절반 vs B 전부를 같은 칸에 두고 반반이라 불렀는데, 그건 1:2 다 . 
