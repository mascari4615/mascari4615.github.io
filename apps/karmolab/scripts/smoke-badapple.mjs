 ...
 *
 * 빌드 통과는 화면이 산다는 증거가 아니다. 이 검사는 진짜 영상 파일을 넣고,
...
const server = http.createServer((req, res) => {
  ...
  /* ★ **진짜 사이트가 안 내보내는 것을 내보내면 안 된다** (2026-08-16, 실측). 이 서버는 파일을
     ...
     브라우저가 CSP 가 head 밖에서 왔다. 무시한다고 말한다. 우리 화면 잘못이 아니라
...
/* ★ **못 트는 자리에서는 못 잼으로 물러난다** (2026-08-14). CI 에서 이 기다림이 60초를
   넘겨 검사가 통째로 죽었다(내 자리에서는 통과). 여기서 만든 영상은 `video/webm` 인데,
   러너의 브라우저 빌드가 그 코덱을 못 틀면 **영영 재생 중이 안 된다**. 그건 우리 코드가
   고장 난 게 아니라 이 자리에서 잴 수 없는 것이다. 그걸 빨강으로 읽으면 아무도 안 믿게 된다. */
/* ★ **말로 상태를 읽지 않는다** (2026-08-16, 실측). 여기는 재생 중이라는 **한국어**를 찾고
   있었다. 실주소는 영어 판이라 Playing. 64×48, 27 frames, 15 fps가 떠 있었고,
...
// 그걸 안 그려졌다로 읽으면 멀쩡한 걸 빨강이라 우긴다. 한 바퀴 도는 동안 훑어서
// 어느 시점엔가 그려졌나와 도중에 바뀌었나를 본다. 두 표면도 같은 시각에 읽는다.
