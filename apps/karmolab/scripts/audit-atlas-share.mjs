 ...
 *
 * ⚠ 이 자의 존재 이유: **사생활만 재고 값어치를 안 재면 아무것도 안 내보내면 완벽이
...
if (!S) {
  ...
  console.log('[share] **공유용 일반화 표가 없다** (share). 가려도 새는데 그럼 뭘 할 수 있나를 안 물었다');
  process.exit(1);
}
...
/* ★ ③ **값어치 쪽 우연 수준이 반드시 있어야 한다**. 이게 이 자의 심장.
   굵게 뭉갤수록 닮은 글이 곁에는 저절로 오른다. 우연을 안 적으면 그게 이득처럼 보인다. */
for (const r of S.rows) {
...
/* ★ 곡선이 **바닥에 닿았는지**. 안 닿았으면 어느 k 로도 안 된다는 어림이다. */
const floor = S.rows.find((r) => r.attack <= S.chance * 1.05);
if (!S.usable && !floor) {
  bad.push('공격이 우연 수준까지 내려간 k 가 표에 없다. k 를 더 키워 봐야 어느 k 로도 안 된다고 말할 수 있다');
}
