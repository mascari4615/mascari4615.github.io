 ...
 *
 * ── 2026-08-17: 다시 켰다 ──
 * 한동안 꺼 두면서 「크롬이 `'inline-speculation-rules'` 를 안 받는다」고 적어 뒀는데 **틀렸다.**
 * 실제 원인은 줄바꿈이었다: 이 파일은 CRLF 인데 HTML 을 읽는 단계에서 CRLF 가 LF 로 바뀌어,
 * 파일에서 뜬 지문이 브라우저가 뜨는 지문과 **하나도 안 맞았다** — 그래서 인라인 열두 개가
 * 전부 막혔고(미리읽기 규칙 포함) 첫 화면이 통째로 죽었다. 그걸 「키워드를 안 받는다」로 읽었다.
 * 따로 세운 실험이 갈랐다(meta 로 준 CSP, 세 판): 자물쇠 없음 = 미리받음 1회 · 키워드 없음 = 0회
 * (콘솔에 위반 한 줄) · 키워드 있음 = 1회. **meta 로 줘도 키워드는 듣는다.**
 * ------------------------------------------------------------
