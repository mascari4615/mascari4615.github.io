 ...
 * 두 가지가 겹쳐 있었고 어제는 둘을 하나로 뭉뚱그려 크롬이 키워드를 안 받는다고 적었다.
 ...
 *   Mdd is not defined로 위젯 등록이 끊긴 적이 있다(`smoke-live-pages.mjs` 머리말).
...
if (bodies.length === 0) {
  ...
  console.error('[csp-shell]   이건 걸 게 없다가 아니라 **못 읽었다**는 뜻이다.');
  process.exit(2);
}
...
/* ★ **브라우저는 줄바꿈을 고쳐 읽는다** (2026-08-17, 크롬이 기대한 지문과 대조해 알아냈다).
   ...
   (실측: 첫 화면이 통째로 죽었고, 그때 크롬이 미리읽기 키워드를 안 받는다로 잘못 읽었다).
