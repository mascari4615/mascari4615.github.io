...
const OVER_LIMIT_DEBT = {
  ...
  /* 오락실은 갚았다 (80.2KB → 58.0KB, 2026-09-05). 넘긴 원인은 놀이가 아니라 **P2P 가 도로
     들어온 것**이었다. `net.ts` 의 `OnlineRun` 을 값으로 들여오는 줄 하나가 trystero 55KB 를
     로비 묶음에 다시 붙였다 (2026-09-03). 놀이 조각은 이미 따로 받고 있었다.
     `test:arcade` 가 "로비 묶음에 P2P 없음" 을 지킨다. **갚은 빚은 지운다** (위 karmograph 주석). */
};
