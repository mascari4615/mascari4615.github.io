...
/** 먹 화면을 연다 — **새로고침 뒤에도 같은 손이 필요하다**.
 *  먹은 「이미지」 위젯의 탭으로 합쳐져서, 주소만으로는 묶음 화면까지만 열린다(2026-08-12).
 *  탭이 언제 붙는지 맞히지 않고, 보일 때까지 1초마다 눌러 본다(여러 번 눌러도 되는 동작이다). */
async function openMeok() {
  for (let i = 0; i < 25; i += 1) {
    if (await page.locator('.meok').first().isVisible().catch(() => false)) return;
    const tab = page.locator('#page-image button', { hasText: '먹' }).first();
...
/* ★ **합쳐진 도구는 묶음 안의 탭으로 연다** (2026-08-12).
   먹은 「이미지」 위젯의 탭으로 합쳐졌다(`widgets-lazy-meta.ts` 의 `bundle: 'image'`).
   주소 `#meok` 로는 묶음 화면까지만 열리고 그 탭은 사람이 눌러야 펴진다 — 그림판은 멀쩡한데
   검사만 20초를 기다리다 죽었다(실측: 탭을 누르면 곧바로 뜬다). 아스키 아트도 같은 일을 겪었다. */
try {
  await openMeok();
  await page.waitForSelector('.meok', { timeout: 20000 });
} catch (error) {
  /* 무엇을 보고 못 찾았는지 남긴다 — 「안 떴다」만으로는 탭 이름이 달라진 것인지,
     아예 안 그려진 것인지 구분할 수 없다(실측 2026-08-12, 그것 때문에 한 판을 더 태웠다). */
  const seen = await page.evaluate(() => ({
    묶음: document.getElementById('page-image') ? document.getElementById('page-image').children.length : -1,
    단추: [...document.querySelectorAll('#page-image button')].map((e) => (e.textContent || '').trim().slice(0, 10)).slice(0, 12)
  })).catch(() => null);
  console.error('[smoke-meok] 화면이 안 떴다', errors, JSON.stringify(seen));
  throw error;
}
