 ...
 *
 * 우리는 뼈대 렌즈를 흔들림 → 달라짐 → **얽힘** → 마디 수로 골라 왔다. 그런데
 ...
 *  ② 화면이 **셋을 다** 적는다 (하나만 적으면 좋은 그림으로 읽힌다)
 ...
 * 그래서 자는 셋이 다 무너진다가 아니라 **적어도 하나가 잡는다 + 못 잡은 자를 말한다**
...
if (isFake(ATLAS)) { console.log('[skeleton-draw] 가짜 지도다. 뼈대 그리기는 진짜 굽기에서만 잰다'); process.exit(0); }
...
if (!an) {
  ...
  /* ★ **옮겼다는 표시를 믿지 않는다.** 표시만 지워도 숨겨지기 때문이다(망가뜨림 판이
     그걸 보여 줬다. 자가 못 잡았다). 원래 자리가 같이 실려 있으니 **자가 직접 잰다**. */
  const b0 = an.before?.xy;
  if (!Array.isArray(b0) || b0.length !== P.length) {
    bad.push('원래 자리가 안 실려 있다 (draw.anchored.before.xy). 옮겼다를 자가 못 재고 표시만 믿게 된다');
  } else {
    ...
    if (realMoved > 0.01 && !an.used) {
      bad.push(`마디가 ${(realMoved * 100).toFixed(0)}% 옮겨졌는데 안 옮겼다로 적혀 있다. 화면이 입을 다문다`);
    }
  ...
  if (an.used) {
    /* ★ 자취는 **실제로 줄이는 값**(stress + λ, 앵커)이다. 평범한 stress 만 재면 λ>0 에서
       정당하게 오르는 것을 셈이 틀렸다로 읽는다. 이 자가 한 번 그렇게 헛빨개졌다. */
...
if (!Array.isArray(tb) || !tb.length) {
  bad.push('렌즈 표가 없다. 왜 이 렌즈냐를 다시 볼 수 없다');
} else {
...
/* 마디를 **한 줄에 늘어놓는다.** 이음이 다 한 직선 위에 놓이므로 **얽힘은 그대로 0**
   인데, 그린 거리도 이웃도 그래프와 아무 상관이 없어진다. 이게 같은 자 값, 다른 그림이다. */
...
/* ★ **여기서 내 예상이 틀렸고, 그게 이 자의 핵심이 됐다.**
   stress 도 같이 무너질 것이라 박아 뒀는데 실측은 반대다. 한 줄 그림의 stress 는
   0.097 로 우리 그림(0.214)보다 **좋다**. 우리 뼈대가 거의 사슬이라 걸음 수가 줄 번호와
   같이 늘기 때문이다. 즉 **stress 만 봐도 이 그림을 좋다고 한다.**
   그러니 자는 셋이 다 무너진다를 걸면 안 된다. 걸어야 할 것은
   **적어도 하나는 잡는다와 어느 자가 못 잡았는지 말한다** 이다. */
...
if (!chromium || !fs.existsSync(BUNDLE) || !dr) {
  ...
  if (an?.used) {
    ...
    console.log(`  ⑤ 화면이 자리를 다시 잡았다를 적나. ${saysMoved ? '○' : '✗'}`);
