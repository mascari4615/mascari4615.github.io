...
/* **렌즈도 고른 것인가**. mapper 는 렌즈가 절반이다. 여태 가로축으로 박아 두고
   손잡이만 쓸었는데, 이제 넷을 쓸어 고른다. 왜 이 렌즈냐를 표로 실어야 다시 볼 수 있다. */
...
}
{
  ...
  /* 굽는 쪽과 **같은 순서**: 덜 흔들림 → 덜 달라짐 → **그림 자 셋의 등수 평균** → 마디 많음.
     ...
     그 렌즈를 골랐어야 한다고 했다. 고르는 잣대와 재는 잣대는 하나여야 한다.

     ★ 그런데 한 번 **어긋났다.** 굽는 쪽이 얽힘 하나에서 얽힘, stress, 이웃 지킴의
 ...
 *
 * 여기서 데었다: 굽는 쪽이 렌즈를 쓸어 괴짜성을 골랐는데, 이 자는 여전히 가로축으로
 * 훑고 있었다. 같은 손잡이(16, 0.35)가 가로축에서는 흔들림 2, 25위, 괴짜성에서는 흔들림 0, 1위다.
 * 그래서 성한 지도를 벼랑에 서 있다고 했다. 렌즈가 절반이면 자도 렌즈를 알아야 한다.
 *
...
/* **흔드는 손이 같아야 판정이 같다.** 굽는 쪽은 점마다 다섯 번 뽑아 이 판에 넣을까를
...
/* ★ 실린 손잡이는 **쓸어 본 그리드 안**이어야 한다. 밖이면 잰 값이 아니라 박은 값이다.
   안정만으로는 못 잡는다: 구간을 3 으로 밀면 다 한 조각으로 뭉쳐 폭 0 = **1위**가 된다
