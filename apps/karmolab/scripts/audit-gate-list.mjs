 ...
 *   ① `package.json` 의 `gates` 가 옛 모양(한 줄에 name 120개)으로 되돌아가 있었다. 정본은
 *      `data/gate-list.json`(135개)인데 한 줄짜리가 이기면서 **목록에만 있는 15개가 아무 데서도
 *      안 돌았다.** 초록은 그대로였다. 안 도는 검사는 조용하다.
 *   ② 목록에 이름은 있는데 `package.json` 에 스크립트가 없는 것이 여섯 있었다. `npm run <없는 name>`
 *      은 그냥 죽으므로 묶음이 거기서 멈춘다.
 *
 * 즉 이 감사기가 지키는 것은 검사가 초록인가가 아니라 **검사가 도는 자리에 있는가**다.
 *
 * 무엇을 보나:
 ...
 *
 * 기준선이 없다. 지금 것은 봐준다가 아니라 **짝이 맞아야 한다**라서다.
 */
...
if (!existsSync(PKG) || !existsSync(LIST)) {
  ...
  console.error('[gate-list]   이건 어긋난 데 없음이 아니라 **아무것도 안 봤다**는 뜻이다.');
  process.exit(2);
}
...
console.log(`[gate-list] 묶음 name ${names.length}개, 스크립트 없는 name ${missing.length}개, 어긋남 ${problems.length}건`);
