 ...
 *
 * 왜 있나: 파생물 중에는 커밋본이 곧 서비스본인 것이 있다. 배포가 다시 굽지 않고,
 * 저장소에 들어 있는 파일 그대로 사람과 봇에게 나간다. 그런 파일은 소스가 바뀌어도
 * **아무 데서도 안 걸린 채** 낡는다.
 *
 * 실측(2026-08-13): 봇 씨앗 표가 도구 122개짜리였는데 도구는 138개였다. 8/08 에 구운 표가
 * 8/13 의 도구 목록을 모르고 봇에 심기고 있었다. 그 표는 2026-09-03 에 접었다(사용자 결정).
 *
...
/* 표는 `lib/generated-artifacts.mjs` 한 곳. 새벽 워크플로도 같은 표를 읽는다.
   여기 또 적으면 밤에 굽는다는 약속만 있고 굽는 놈은 없는 자리가 생긴다. */
...
/* ★ **표에 적힌 굽는 명령이 진짜 있나** (2026-08-14). 표는 이제 새벽 워크플로도 읽는다 . 
   여기 없는 이름을 적어 두면 그 파일은 밤에 굽는다는 약속만 남고 새벽 3시에 조용히 죽는다.
...
for (const item of generated) {
  /* **잴 수 없는 것은 안 잰다**. 다시 구우면 늘 다른 것(시각, 최근 N일)을 게이트에 걸면
     영원히 빨갛다. 그건 밤이 굽는다. 그래도 한 줄 남긴다. 안 본다가 보여야 한다. */
  if (item.weight) {
...
/* 매일 스스로 굽는 것은 낡았다가 아니라 곧 구워진다다. 갈라 낸다. */
...
/* 초록도 한 줄 남긴다. 아무 말 없음이 정상인지 안 돈 것인지 구분되게. */
