 ...
 * 여태 장이 안 열리면 `?` 하나 찍고 넘어갔고, 도구 화면이 안 뜨면 문제 0개로 셌다.
 ...
 * 잰 도구 N/M을 적는다. 몇 장을 보고 한 말인지가 판정의 일부다.
 *
...
/* ★ **못 잰 것을 0 으로 세지 않는다** (2026-08-14). 여태 장이 안 열리면 `?` 하나 찍고 넘어갔고,
   위젯이 안 뜨면 문제 0개로 셌다. 그러면 **사이트가 통째로 죽은 날 이 검사는 초록**이다 . 
   이름 없는 칸 0개, 안 이어진 칸 0개이므로. 재려는 것이 안 온 것과 없다는 다르다. */
const unmeasured = [];
let unlinked = 0;
let nameless = 0;
    ...
    while (queue.length) {
      ...
      try {
        ...
        /* ★ **못 잴 게 뻔하면 그만둔다** (2026-08-14). 사이트가 안 서 있으면 122장을 25초씩
           ...
           판정은 이미 못 쟀다로 정해졌으므로 더 두드릴 이유가 없다. */
...
/* **잰 것이 너무 적으면 판정하지 않는다.** 여기서 초록, 빨강 어느 쪽으로도 적으면 거짓이다 . 
   0 은 이름 없는 칸이 없다가 아니라 안 봤다이기 때문이다. 실측 2026-08-14: 138장 중
...
if (unmeasured.length >= threshold) {
  ...
  console.error('  못 잰 것은 이름이 다 붙어 있다가 아니다. 판정하지 않고 지나간다.');
  process.exit(2);
}
...
/* ★ **초록도 나가야 초록이다** (2026-08-21 실측). 위 닫는 데도 시간을 준다 주석은
   5초 안에 안 되면 그냥 나간다고 적어 뒀는데, `race` 는 *기다리기*를 그만둘 뿐
   **나가지는 않는다**. 시각으로 끊고 버린 레인이 창을 쥔 채 남아 node 를 붙잡는다.
   다른 갈래(`--update`, CANNOT-RUN, 빨강)는 전부 `process.exit` 이라 즉시 나갔고,
   **초록 한 갈래만** 빠져 있었다. 그래서 이 검사는 판정을 다 찍고도 안 죽었고,
   실주소 러너의 감싸개가 그 멎음을 판은 그대로였다. 진짜 빨강이다로 적었다.
   즉 **초록을 찍은 검사가 빨강으로 세어졌다.** 판정은 이 줄에서 이미 끝났다. */
process.exit(0);
