 ...
 *
 * 알맹이 시험은 채점이 맞다까지다. 이 검사가 보는 것은 다르다:
 *
 * ★ **답이 화면 안에 없어야 한다.** 초성 맞히기의 답은 화면 어딘가에 글자로 들어 있으면
 *   그 순간 놀이가 끝난다. 개발자 도구를 열 필요도 없이 소스 보기로 보인다. 알맹이가
 ...
 * 부르자마자 읽으면 칸이 없다로 헛보인다(실측: CI 배포 빨강).
 */
...
if (choBuilt === false) {
  ...
  /* ★ 답이 화면 어디에도 없어야 한다. 눈에 보이는 글만이 아니라 **HTML 통째로** 본다 . 
     숨긴 칸, data 속성, value 에 들어 있어도 소스 보기로 다 보인다. */
  /* ★ **우리가 쓴 글까지 샜다고 세면 안 된다** (2026-08-16, 실측). 답은 우리 도구 설명에서
     뽑은 **평범한 낱말**이다(`word-pool.generated.ts`). 그래서 그 낱말이 위젯 제 이름표에
     그냥 들어 있을 수 있다. 오늘 답 글자가 이름표 🟨 한 글자 다름에 박혀 있어,
  ...
  /* **길이 힌트는 답이 아니다.** 화면은 2글자처럼 답의 길이를 알려 준다. 그런데 답 자체가
     글자 같은 평범한 낱말이면 그 안에 통째로 들어가 버린다(오늘 실측). 사람이 2글자를 보고
...
else if (typeOut.say.trim() === '') fails.push('다 쳤어요를 눌러도 화면이 아무 말이 없다');
