...
const 스크립트들 = process.argv.slice(2).filter((a) => !a.startsWith('--'));
...
const { 파생물 } = await import('./lib/generated-artifacts.mjs');
const 산출물 = 스크립트들.flatMap((n) => 파생물.find((x) => x.npm === n)?.outputs ?? []);
...
if (!remote) 못돌림('origin 주소를 못 읽었다');
if (!existsSync(join(appRoot, 'node_modules'))) 못돌림('apps/karmolab/node_modules 가 없다 (npm ci 먼저)');
...
try {
  ...
  if (clone.status !== 0) 못돌림(`사본을 못 만들었다: ${(clone.stderr || '').trim().split('\n').pop()}`);

  const 사본앱 = join(tmp, 'apps/karmolab');
  if (!existsSync(사본앱)) 못돌림('사본에 apps/karmolab 이 없다');
  symlinkSync(join(appRoot, 'node_modules'), join(사본앱, 'node_modules'), 'junction');
  ...
  if (돌린다('npm', ['run', '--silent', 'build:i18n']).status !== 0) 못돌림('사본에서 말 묶음을 못 지었다');
  if (돌린다('node', ['build.mjs']).status !== 0) 못돌림('사본에서 build.mjs 가 죽었다');
  ...
  const 바뀐것 = [];
  for (const rel of 산출물) {
    const 새것 = join(사본앱, rel);
    const 여기 = join(appRoot, rel);
    if (!existsSync(새것)) continue;
    /* 폴더째 나오는 것도 있다(공유 카드 `img/og`) — 그때는 통째로 옮기고 몇 개인지만 적는다. */
    if (statSync(새것).isDirectory()) {
      cpSync(새것, 여기, { recursive: true });
      바뀐것.push(`${rel}/ (폴더째)`);
      continue;
    }
    const a = existsSync(여기) ? readFileSync(여기) : null;
    const b = readFileSync(새것);
    if (a && a.equals(b)) continue;
    cpSync(새것, 여기);
    바뀐것.push(`${rel} (${(statSync(여기).size / 1024).toFixed(1)}KB)`);
  }
  ...
  console.log(`[bake-clean] 바뀐 산출물 ${바뀐것.length}개를 이 자리로 옮겼다:`);
  바뀐것.forEach((x) => console.log('  - ' + x));
