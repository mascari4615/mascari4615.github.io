 ...
 * `will-change` 를 적었으니 합성기가 맡겠지라고 **믿고 안 쟀다**. 채팅 코드에는
 * 이제 주 스레드는 0이라는 **틀린 확신이 주석으로** 박혀 있었고, 실제로는 손을 안 댄
 * 화면에서 초당 48회 스타일을 다시 계산하고 있었다.
 *
 * 이 저장소에는 게이트가 아주 많다(도구 129장 신선도, 부팅 예산, CLS, 접근성, 타이포, 링크...).
 * 그런데 **가만히 둘 때 얼마나 일하나와 만질 때 몇 프레임 나오나를 재는 자리가 없었다.**
...
/* CI 기계는 여기보다 느리고 들쭉날쭉하다. 프레임으로 재는 값은 그만큼 헐겁게 잡는다.
   **스타일 재계산/s 는 안 늘린다**: 그건 매 프레임 일하나를 세는 것이라 기계 속도와
 ...
 * 코드가 사람 화면에 하나도 안 닿았다(조각 다섯이 404). 위쪽 js 가 없으면 못 돌림과 같은
 * 규칙이다: 없는 것을 두고 통과도 실패도 거짓말이고, **막는 자리가 답을 더 나쁘게
...
async function measurePage(url) {
  ...
  /* 배경 도형은 손을 안 대면 4초 뒤부터 잦아든다. 그 뒤에 재야 가만히 있을 때가 된다. */
  ...
  if (process.argv.includes('--regress')) {
    /* ★ **얹는 것이 진짜 무거워야 자기 시험이 뜻을 가진다** (2026-08-16, 실측).
       ...
       예산이 헐겁다로 실패했다. 게이트가 약해진 게 아니라 **가짜 감도가 사라진 것**이다.
