/**
 * 「기기 안에서만 돈다」는 말이 **아직 참인지** 잰다 (TASK-KL-351).
 *
 * 왜 있나: 도구 장 바닥에 「입력한 내용은 브라우저 안에서만 처리되며 어디에도 저장·전송되지
 * 않습니다」가 조건 없이 찍히고 있었다. 그때 이미 `imageedit` 는 그림을 Google Gemini 로,
...
if (asJson) {
  ...
  for (const c of contradicts) {
    console.log(`  ✗ ${c.id} — 「기기 안에서만」이라 적혔는데 바깥 호출이 있다 → ${c.hosts.join(' ') || '(주소는 변수)'}`);
  }
