 ...
 *
 * `smoke-mcp.mjs` 는 저장소 안에서 서버를 띄운다. 그건 우리 컴퓨터에서 된다까지다.
...
/**
 * 이 검사는 **배포 길목**에 있다 (build 사슬). 그래서 못 돌았다와 제품이 틀렸다를
 * 반드시 갈라야 한다. 못 돈 것을 실패로 세면 남의 배포가 통째로 선다.
 *
 * ★ 가르는 방법을 **글자로 하지 않는다.**
 *
 * 처음엔 오류 문구에서 `ENOENT`, `not recognized` 같은 낱말을 찾아 환경 문제로 분류했다.
 ...
 * 못 돌렸다고 말하고 통과. 그 다음부터 나오는 실패는 전부 제품 문제다.
 * 설치도 `--offline` 로 한다. 우리 tarball 은 손안에 있고 의존성이 0개라 레지스트리가 필요 없다.
 * 그러면 망이 안 됐다라는 실패 자리 자체가 사라진다.
 */
const npmUsable = (() => {
  try {
    runNpm(['--version'], root);
...
if (npmUsable === false) {
  ...
  console.log(
    [
      ...
      '  검사가 못 돌았다이지 꾸러미가 틀렸다가 아닙니다. 배포를 세우지 않습니다.'
    ].join('\n')
  );
  process.exit(0);
}
...
try {
   ...
   * --prefer-offline: 캐시를 먼저 뒤져보되, 없으면 인터넷에서 몰래 빨리 받아와 설치한다. 그건 망이 안 됐다라는 실패 자리 자체가 사라지게 한다.
   */
  ...
  // ⓐ 라이선스. 없으면 발행물이 저작권 불명이 된다. `files` 에서 빠지면 조용히 사라진다.
   ...
   * bin 항목이 잘못돼 지웠다고 경고한 적이 있다(경로 앞의 `./`). 그러면 설치는 되는데
   * 명령어만 안 생긴다.
   */
