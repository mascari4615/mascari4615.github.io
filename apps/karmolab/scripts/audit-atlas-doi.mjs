 ...
 * `DOI = D(x,y)` 한 항짜리에, 그것도 화면에서 지우지 않는 판이다. 그 상수도 전부 그리기도
 ...
 * **연결** 부분그래프는 그냥 가까운 60개보다 되찾는 게 적다. 그래서 **안 썼다.**
...
if (!d) {
  ...
  console.log('[doi] **무엇을 남길지 재 본 표가 없다** (doi). 전부 그리고 흐리게가 아직 안 쟨 상수다');
  process.exit(1);
}
...
/* ① **판정이 수와 맞나**. 이 자의 핵심. 졌는데 쓴다고 적으면 빨강. */
...
if (!d.why) bad.push('안 쓴 까닭이 안 적혀 있다. 졌다만으로는 다음 사람이 또 해 본다');
...
console.log(`  ③ 숨은 이웃 수 오차 ${d.hiddenErr}, 확장 top-3 적중 ${d.top3} (아무 방향 ${d.top3Rand}, ${d.top3Of}회)`
  + `, **놓친 것 ${d.missed}개**`);
if (d.hiddenErr) bad.push(`숨은 이웃 수가 틀린 자리가 ${d.hiddenErr}군데 있다`);
if (d.missed == null) bad.push('놓친 것 원장이 없다. 흐리게를 안 그리기로 바꾼 대가를 숨기면 안 된다');
