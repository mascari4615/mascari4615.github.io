 ...
 * 민감도, arXiv 2407.17876), 판 하나를 지도라 부르는 건 표본 하나를 모집단이라 부르는 것이다.
 *
 ...
 * 33.8% 인데 구조 없는 벡터에서는 0.5% 다. **66배**. 그래서 결론은 지도가 쓸모없다가
 ...
 * 그리고 ②도 처음엔 **엉뚱한 걸 재고 있었다**: 앞 k판끼리의 평균 거리는 판 수와 무관한
...
if (!chromium || !fs.existsSync(BUNDLE)) {
  ...
  if (!saysKeep) bad.push('화면이 이웃 유지율을 안 적는다. 믿을 것은 이웃뿐의 근거가 사라진다');
  if (!saysHow) bad.push('화면이 **저기 있다가 아니라 이 옆에 있다로 읽어라**를 안 적는다');
  /**
   * ★ 맨 위 한 자리를 놓고 **자 둘이 싸웠다**. 이 자는 자리는 씨앗의 것을, 고유차원 자는
   * 이 무더기는 2차원이 아니다를 맨 위에 요구했다. 정리 = **원인이 먼저, 증상이 다음.**
