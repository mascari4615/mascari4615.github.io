#!/usr/bin/env node
// audit-workflow-prereqs.mjs. 이 앱을 짓는 워크플로는 전제도 같이 갖췄나를 본다.
//
...
//   ★ `working-directory` 로 보면 짓는가와 자기 자리를 적었는가가 한 번에 지켜진다.
...
/* 모아 둔 채비(composite action)가 **실제로** 어떤 꾸러미를 짓는지 읽어 둔다 . 
   쓴다고 적혀 있으니 되겠지가 아니라 그 파일이 정말 짓는지 본다. */
...
for (const f of files) {
	...
	/* ★ **한 곳에 모아 둔 채비도 인정한다** (2026-08-13).
	   ...
	   짓는 단계가 없다고 했다. 고쳤더니 검사가 거짓 빨강을 낸 셈이다. 모아 둔 곳도 본다. */
	...
	for (const p of required) {
		...
		problems.push(`${f}: ${p.name}(${p.dir}) 를 먼저 짓는 단계가 없다. 새 체크아웃엔 dist 가 없어 typecheck 이 모듈을 못 찾겠다로 선다`);
	}
}
