 ...
 *
 * ★★ 눈금도 한 번 틀렸다. 블록이 뚜렷한 자료를 썼더니 얻는 것이 **−2%** 였다. 알고리즘이
...
if (!S) {
  ...
  console.log('[seriation] **자리 정렬을 재 본 표가 없다** (seriation). 행렬이 나은 그릇인가를 안 물었다');
  process.exit(1);
}
...
if (!rnd) bad.push('마구 정렬이 표에 없다. 바닥이 없으면 좋다가 뜻이 없다');
...
/* ★ **음수는 고장이 아니다** (2026-08-23 정정). 전에는 섞은 자료에서도 2% 는 얻는다를 요구했는데,
...
/* 음수 대조군이 두 배를 거저 통과시키지 않게 **절대 바닥(2%)** 을 같이 깐다 (굽는 쪽과 같은 식). */
