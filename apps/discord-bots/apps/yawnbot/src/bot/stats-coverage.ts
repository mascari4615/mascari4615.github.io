 ...
 * 그런데 0 은 아무도 안 떠들었다와 생김새가 같다. 실제로 그래서 며칠치 대화가 통째로
 ...
 * 메시지 기록 보기는 과거를 읽는 권한이라 실시간 집계와 무관하다.
 */
...
/**
 * 사람 말 한 줄. 기록이 왜 없나에 답이 되는 문장만 돌려주고, 문제 없으면 null.
 *
...
export function coverageNotice(coverage: CoverageReport, totalMessages: number): string | null {
  ...
  if (coverage.visible === 0 && coverage.total > 0) {
    return (
      ...
      '이러면 무슨 말을 해도 안 세집니다. 채널 설정 → 권한에서 욘에게 채널 보기를 켜 주세요.'
    );
  }
  ...
  if (totalMessages === 0) {
    return (
      ...
      '거기 대화는 세지 못합니다. 채널 권한에서 욘에게 채널 보기를 켜 주세요.'
    );
  }
