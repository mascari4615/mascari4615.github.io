...
export function buildTaskSeedBody(
  ...
  const body = [
    ...
    '',
    `> **[INITIATOR-AUTO]** TASK-KAR-018-INIT substrate 발의. status=seed (사용자 가시 윈도우).`,
    '',
    '## 👤 사용자 결정 1개 (디스코드 스레드에서)',
    '',
    '- **✅ approve** → status=ready, 워커 자율 픽업 시작',
    '- **❌ reject** → status=sealed + TASK-OK, INIT 가 같은 신호로 7d 재발의 X',
    '- **💤 무반응** → 14d 후 자동 ready 승격 (worker 자율 픽업)',
    '',
    '_(이 TASK 디스코드 스레드에 답글로 「approve」/「reject」 입력 또는 reaction)_',
    '',
    '## 목표',
    '',
