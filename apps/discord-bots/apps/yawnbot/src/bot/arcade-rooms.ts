 ...
 * 모이는 중」이라는 쪽지 한 장뿐이다. 그래서 봇이 죽어도 이미 열린 방은 그대로 돌아간다.
 *
 * 규율:
 *  ① **올리는 것은 명시적이다.** 사이트의 「같이」는 그대로 비공개(링크 아는 사람만)고,
 *     여기 오르는 것은 「같이 찾기」로 연 방뿐이다. 기본값을 어느 쪽으로 두든 한쪽은 놀라므로,
 *     아예 다른 단추로 가른다.
 *  ② **금방 사라진다**(10분). 안 지우면 목록이 죽은 방으로 덮인다 — 「아무도 없네」보다
 *     「눌렀는데 아무도 없네」가 나쁘다. 방 주인이 계속 살아 있다고 알리면 그만큼 늘어난다.
...
export function registerArcadeRooms(app: Application): void {
  /**
   * 방을 올린다 / 살아 있다고 알린다. 같은 코드면 덮어쓴다 — 알림이 곧 「아직 있다」다.
   */
