...
import { agreesWithTape, setVerifier, verifierReady } from './arcade-verify';
...
describe('패보로 다시 셈하기', () => {
  ...
  it('점수가 같으면 순서가 달라도 맞다. 무승부를 거짓말로 몰면 안 됨', () => {
    setVerifier(() => ({ ok: true, ranks: [0, 1], scores: [3, 3], finished: true }));
    expect(agreesWithTape(TAPE, SEATS, ['b', 'a']).agrees).toBe(true);
  });
