 ...
 * 사라진다. 그런데 무엇을 골랐나는 순위보다 훨씬 많은 걸 말한다. 누구와 취향이 같은지,
 ...
 * 그 4개로 취향 100% 일치라고 말할 수 없다. 겹치는 항목이 적으면 아예 답을 안 한다.
 *
...
/** 이만큼은 겹쳐야 취향이 비슷하다를 말한다. 적으면 우연이 사람 얼굴을 하고 나온다. */
...
export class TasteStore {
   ...
   * 취향은 몇 번이나 그쪽을 골랐나이기 때문이다. 대신 자기 자신과의 대결처럼
   * 말이 안 되는 줄은 버린다.
   */
  ...
  neighbours(who: string, variant: string, limit = 3): { closest: Agreement[]; opposite: Agreement[] } {
     ...
     * 가장 비슷하고 동시에 가장 다른 사람이 된다. 그건 아무 말도 아니다. */
