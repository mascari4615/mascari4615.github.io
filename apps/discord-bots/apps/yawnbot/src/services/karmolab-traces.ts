 ...
 *
 * 왜 있나: Cycle 1 로 내 기록은 남게 됐지만, 사이트에는 여전히 **남의 흔적이 하나도 안 보인다**.
 * 사람이 와도 왔다는 자국이 안 남으면 아무리 와도 빈 곳으로 보인다. 북적북적의 반대.
 *
 * 왜 원장 하나인가: 사용 횟수, 요청, 투표를 각자 저장소로 만들면 다음에 활동 피드, 랭킹을
...
/** 도구 하나의 쓰임. 날짜별로 나눠 둬야 요즘 뜨는 도구를 뒤에 붙일 수 있다. */
interface ToolTrace {
  total: number;
 ...
 * 글 한 편.
 *
 * 게시판(이야기)과 도구 요청을 **한 종류로 둔다**. 둘 다 누가 쓴 글 + 사람들의 반응이고,
 ...
 * 게시판.
 *
 * 커뮤니티는 글 하나 + 댓글이 아니라 **어디에 쓰는 글인지**가 먼저 정해지는 곳이다
...
export interface Gallery {
   ...
   * 다른 쓰임**이다. 글, 답글, 말머리, 좋아요는 그대로고, 달라지는 건 이 글이 끝났는가를
...
export const SEED_GALLERIES: Gallery[] = [
  ...
  // WM 페이지(`/#wm`)가 이 갤러리를 제 이야기판으로 읽는다.
 ...
 *
 * 채팅과 **같은 이름표**를 쓴다. 채팅에서 연보라 수달이던 사람이 글에서도 그렇게 보인다.
...
export interface Post {
  ...
  tag: string | null;
  /**
   * 갤러리 안에서의 번호 (#3). 이슈식 갤러리에서 그 3번 말이야라고 부를 수 있게 한다.
   * **한 번 준 번호는 안 바뀐다**. 글이 지워져도 뒤 글이 당겨 오지 않는다. 번호가 움직이면
   * 어제 남긴 #3 참고가 오늘 다른 글을 가리키게 된다.
   */
  seq: number;
  /** 상태를 바꾸며 남긴 한 줄 (닫는 이유, 만들어진 것). 안 남겼으면 null. */
  statusNote: string | null;
  /** 상태를 바꾼 시각, 사람. 언제 닫혔나가 안 보이면 닫힌 글은 그냥 사라진 글처럼 읽힌다. */
...
export interface Report {
   ...
   * 때쯤이면 무엇을 보고 판단하라는 것인지가 사라져 있다. 그러면 신고는 처리되지 않고
   * 쌓이기만 한다.
   */
  subject: string;
  createdAt: string;
 ...
 * 이 사이트에 사람이 오나다. 도구 열림만 세면 첫 화면은 영원히 아무도 안 온 곳이 된다.
 *
 ...
 * 우리가 알고 싶은 것은 요즘 사람들에게 얼마나 걸리나뿐이라, 날짜별로 **칸에 세기만** 한다.
...
interface TracesState {
   ...
   * 3번 글을 지우는 순간 다음 글이 다시 3번을 받고, 어제 남긴 #3 참고가 다른 글을 가리킨다.
   */
...
function withSeeds(existing: Gallery[]): Gallery[] {
  ...
  const out = existing.map((g) => {
    ...
    // (요청판은 원래 열림 → 만들어짐으로 살아 왔다. 이름만 이제 붙는다).
    const issueStyle =
 ...
 * 새 글은 그 갤러리의 가장 큰 번호 + 1을 받으므로, 여기서 겹치지 않게 매겨 두면 끝이다.
 */
...
/** 지금 보고 있다로 치는 시간. 이보다 오래 소식이 없으면 나간 것으로 본다. */
...
export class KarmolabTraceStore {
  ...
  /** 지금 보고 있는 사람들. 열쇠 → 마지막 소식 시각. **저장하지 않는다** (지금은 과거가 없다). */
   ...
   * 그 수는 사람이 왔다가 아니라 내가 링크를 잘 걸었다밖에 안 말해 준다.
   *
   * @returns 실제로 셌으면 true.
   */
   ...
   *
   * **저장하지 않는다**. 메모리에만 둔다. 지금은 과거가 되면 아무 뜻이 없고,
   ...
   *
   * 값이 없거나 말이 안 되면 그 지표만 건너뛴다. 0 으로 채우면 아주 빠른 판이 하나
   ...
   * (1000ms 이하처럼). 표본이 적으면 수를 같이 준다. 적은 표본의 중앙값은 숫자놀음이다.
   */
   ...
   *
   * 지난 7일과 그 전 7일을 나란히 놓는다. 많다/적다는 혼자서는 뜻이 없고 늘 비교값이 있어야 한다.
   */
  ...
  addPost(
    ...
    const post: Post = {
       ...
       * 나온다. 익명은 화면에서 안 보이기가 아니라 기록에 안 남기여야 한다. */
   ...
   * 대화는 혼자 만든 것이 아니다. 대신 누가 썼는지를 지운다: 이름은 지운 계정이 되고
  ...
  updatePost(
    ...
    if (typeof patch.status === 'string' && allowed.includes(patch.status as Post['status'])) {
      ...
      if (changed) {
        // 언제, 누가가 없으면 닫힌 글은 그냥 사라진 글처럼 읽힌다.
   ...
   * 갤러리를 가리지 않는 검색.
   *
   * 지금까지는 보고 있는 갤러리 안에서만 걸렀다. 그 글 어디 있더라를 못 찾는다.
  ...
  openReports(): Report[] {
    /* 예전에 쌓인 줄에는 `kind`, `subject` 가 없다. 여기서 채워서 내보낸다 . 
       화면이 없을 수도 있는 칸을 매번 방어하게 만들면 그 방어가 곧 빈틈이 된다. */
   ...
   * 빈 번호가 보이는 편이, 남이 남긴 #3 참고가 엉뚱한 글을 가리키는 것보다 낫다.
   */
  ...
  /** 글 하나. 권한을 따질 때 이 글이 어느 갤러리 것인가를 알아야 한다. */
