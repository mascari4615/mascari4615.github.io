 ...
 *
 * 규칙 셋:
 *   ① **읽기가 곧 지우기다.** 내주는 그 순간 지운다. 내주고 나서 지우기로 만들면
...
/**
 * 잠긴 덩어리의 최대 크기(글자). base64 로 3분의 4쯤 부풀므로 실제 파일 5MB 남짓 (TASK-KL-252).
 *
 * 글이든 파일이든 **여기서는 구분하지 않는다**. 서버는 잠긴 덩어리만 보므로 무엇인지 모르고,
 * 규칙(한 번 읽으면 삭제, 7일 만료)도 같다. 구분할 이유가 없는 자리에 구분을 만들면
 * 곳간이 둘, 규칙이 둘, 설명이 둘이 된다.
 */
...
export class BurnNoteStore {
  ...
  /**
   * 꺼내면서 **동시에 지운다**. 없으면 null. 이미 읽혔다와 그런 쪽지 없다를
   * 구분하지 않는다: 구분하면 이 주소에 쪽지가 있었다는 사실이 남에게 새어 나간다.
   */
