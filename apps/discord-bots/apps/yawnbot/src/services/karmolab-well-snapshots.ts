 ...
 *
 * 왜 있나: 우물(KL-153)은 **지금**만 말한다. 그런데 재미있는 건 지금 1등이 아니라
 ...
 * 통째로 빈다. 열릴 때 찍기는 스스로 낫는다(누가 한 번만 열면 그날이 채워진다).
 *
...
/** 며칠까지 들고 있나. 작년 오늘까지 가면 좋지만, 먼저 90일로 시작한다. */
...
export class WellSnapshotStore {
   ...
   * 하루에도 여러 번 열리는데 그때마다 덮으면 그날의 값이 마지막 열람 시각 값이 된다.
   */
   ...
   * 비교할 게 없다로 끝나면 안 된다. 대신 어느 날과 견줬는지 함께 돌려준다.
   */
