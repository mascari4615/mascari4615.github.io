...
/** 얼마나 자주 보낼 때인가를 보나. 정시를 놓쳐도 다음 시각에 따라잡는다. */
...
export function weeklyMessage(
  ...
  // 지난 7일치만 본다. 주간이라면서 통산을 보내면 매주 같은 글이 간다.
  const days: string[] = [];
  for (let i = 1; i <= 7; i += 1) {
  ...
  if (came.length === 0) return null; // 안 온 주에 0일을 보내는 건 알림이 아니라 잔소리다.
...
/**
 * 30분마다 보낼 때인가를 보고, 맞으면 켠 사람들에게 보낸다.
 *
 ...
 *
 * 시각을 기다리는 것 말고 지금 당장 돌려 볼 길이 없으면, 보내지는가를 확인하려면
