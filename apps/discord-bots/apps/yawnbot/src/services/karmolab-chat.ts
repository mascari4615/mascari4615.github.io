...
export interface ChatMessage {
     ...
     * 화면이 내 줄인가와 연달아 말한 같은 사람인가를 판단하는 데만 쓴다.
     ...
     *
     * 채팅은 하루 뒤 사라진다. 좋은 말이 나와도 그걸 남길 길이 손으로 글로 옮기기뿐이면
...
interface ChatState {
     ...
     * 방은 영영 빈다. 실시간 방의 죽는 방식이 정확히 이것이다. 지금 보고 있는 사람이
...
export class KarmolabChatStore {
    ...
    /**
     * 지금 여기를 **사람 단위**로 센다 (사용자 신고 2026-08-08. 숫자가 계속 왔다갔다).
     *
     ...
     * 한 줄 남긴다.
     *
     * 막을 때는 **왜 막혔는지**를 같이 돌려준다. 안 돼요만 주면 사용자는 자기가 도배로
    ...
    post(
        ...
        /* 답하는 상대의 말을 **그때 모습 그대로** 베껴 둔다. 원본은 지워질 수도, 하루 뒤
           사라질 수도 있는데, 그러면 답글만 남아 무엇에 대한 답인지가 없어진다. */
    ...
    /**
     * 밖으로 내보낼 모양. **남의 이름표 목록은 빼고** 내가 눌렀나만 남긴다.
     ...
     * 이름표가 갈리면서 내가 지킨 것이 통째로 남의 것처럼 보인다. 지킨 줄은 하루를
    ...
    subscribe(listener: (event: ChatEvent) => void, who?: string): () => void {
        /* 새로 붙은 사람에게는 몇 명을 **다시 안 보낸다**. 첫 마디(hello)에 이미 들어 있다.
        ...
        return () => {
             ...
             * 없던 일로 칠 수 있다. 이름표 없는 연결은 그냥 그 자리에서 뺀다. */
            if (who === undefined) {
     ...
     * 지금 보고 있는 사람이 아니라 오늘 여기 있던 사람. 빈 방에 남긴 말도 닿게 하는 열쇠다.
     */
     ...
     *
     * 지금 1명만 보이면 죽은 방으로 읽힌다. 오늘 몇 사람이 여기서 말했는지가 같이 보이면
     * 같은 화면이 지금은 조용한 방으로 읽힌다. 남길 마음이 생기는 건 그때다.
