 ...
 * 몇 번 남았는지 한 줄로.
 *
 * 예전엔 0 / 8번째 시도 였다. 0번째 시도는 말이 안 되고, 한 수 둔 뒤의
 * 1 / 8번째 시도도 쓴 수인지 지금 두는 수인지 안 갈린다. 사람이 궁금한 건 **몇 번 남았나**다.
 */
...
/** ㅍㅋㅊ처럼 첫 자음만 친 것인가. 자음 하나만 쳤을 때도 여기에 든다. */
...
// ── 나열형 (TASK-KL-197) ────────────────────────────────────────────────────
// 하나를 맞힌다 옆에 전부 대본다를 둔다. 다른 놀이지만 **표는 그대로**다 . 
...
function conditionsOf(topic) {
  ...
  for (const field of topic.fields ?? []) {
    ...
    if (field.kind === 'number') {
       ...
       * 몸무게 550kg 이상인 포켓몬을 전부는 답이 있어도 사람이 못 댄다. 아무도 몸무게를
...
export function listQuestions(topic, { min = LIST_MIN, max = LIST_MAX } = {}) {
  ...
  /**
   * 표가 크면 조건 하나로는 늘 넘친다. 포켓몬 1025마리에서 불꽃 타입은 100마리가 넘는다.
 ...
 * 오타를 틀렸다로 처리하면 사람은 자기가 뭘 잘못했는지 모른다.
 */
 ...
 *
 * 왜 이게 있나: 맞았나 틀렸나만 보면 나열형은 많이 아는 사람이 이긴다로 끝나고, 한 번 풀면
...
/** 한 칸이 성립하려면 답이 이만큼은 있어야 한다. 하나뿐인 칸은 알거나 모르거나라 재미가 없다. */
const GRID_MIN_PER_CELL = 2;
/** 반대로 이만큼보다 헐거운 칸은 아무거나 넣으면 맞는 칸이라 재미가 죽는다. */
 ...
 * (색이 빨강이면서 파랑인 것은 없다). 그래서 세대 × 타입처럼 갈래가 다른 둘을 건다.
 *
...
export function gridPuzzleOf(topic, at = new Date(), { attempts = 400 } = {}) {
  ...
  for (let n = 0; n < attempts; n += 1) {
     ...
     * 있으면서 원거리 딜러이기도 한은 성립한다. 이걸 막으면 롤처럼 쓸 만한 속성이
     * 하나뿐인 주제는 판이 아예 안 선다.
     */
    ...
    // 같은 속성을 두 축에 걸 때는 값이 겹치면 안 된다. 불꽃이면서 불꽃은 조건이 아니다.
    ...
    if (cells.every((row) => row.every((names) => names.length >= GRID_MIN_PER_CELL && names.length <= tight))) {
      return {
         ...
         * 필드라 학교가 ...인이 세 번 반복되면 읽는 데 방해만 된다. 화면은 짧은 것을
