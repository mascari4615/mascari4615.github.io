 ...
 */
export function dailyIndex(topicId, dayNumber, count) {
  if (count <= 0) throw new Error('빈 표에서는 문제를 못 낸다');
  const cycle = Math.floor(dayNumber / count);
  const rand = mulberry32(hash32(`${topicId}:${cycle}`));
...
/** 오늘의 정답 항목. */
export function answerOf(topic, at = new Date()) {
  return topic.items[dailyIndex(topic.id, kstDayNumber(at), topic.items.length)];
}
