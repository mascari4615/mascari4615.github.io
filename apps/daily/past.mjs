...
try {
  ...
  // 조용히 빈 표를 남기지 않는다. 빈 화면은 고장으로 읽힌다.
 ...
 * 여기서 할 일은 두 가지뿐이다: 배포 뒤로 지나간 날을 위에 얹고, 더 보기로 옛날을 잇는다.
 */
...
root.querySelector('.past-note').textContent =
  `오늘(#${puzzleNumber()}) 답은 여기 없다. 내일 이 자리에 올라온다. 놓친 날은 풀어보기로 지금 풀 수 있어서, ` +
 ...
 *
 * 가려진 동안에는 그림이 아무 뜻도 없어서 주소만 들고 있는다. 그런데 답 모두 보기를
