 ...
 * 단위 시험은 규칙만 본다. 화면에서 실제로 두어진다는 여기서만 갈린다.
 *
 ...
 * 방금 고친 것이 아니라 예전 것을 재고 다 통과라고 말한다. 가장 나쁜 종류의 거짓 초록이다.
...
// 이미 고친 화면을 보고 이상 없음이라 판정하게 된다 (실제로 그랬다).
...
async function playTopic(topicId, { width, height, tag, mode = 'classic' }) {
  ...
  // 끝낸 사람이 지금 할 수 있는 것. 내일 또만 남기면 그대로 나간다.
  check(`[${tag}] 어제 문제로 이어 준다`, (await page.locator('.done .more a[href*="?d="]').count()) === 1);
  if (mode === 'classic') {
    // 기록이 쌓이는 게 보여야 다시 온다. 색만으로 알리지 않게 숫자와 오늘이 글자로 붙는다.
...
/** 실루엣은 처음엔 안 보이다가 틀릴수록 밝아진다가 전부다. 그게 실제로 일어나는지 본다. */
...
async function pastPage(topicId) {
  ...
  /* 오늘 답이라는 **글자**가 어디에도 없다로 재면 안 된다 (TASK-KL-101).
  ...
  // 진짜 규칙은 오늘 줄이 없다이다. 이름으로 보면 과거의 *다른 모드* 답과 우연히 겹쳐
  ...
  check(`[지난:${topicId}] 답 모두 보기로 다 열린다`, allOpen);
...
/**
 * 실루엣 판에서 그림이 끝내 안 오는 경우. 까만 상자만 남으면 원래 이런 놀이와 구분이 안 된다.
 ...
 */
{
  ...
  /**
   * 판 바꾸는 단추는 날짜를 들고 가야 한다. 안 그러면 지난 날을 풀다 실루엣을 누른 순간
   ...
   * 따로 조립해서 제목은 오늘의 포켓몬인데 공유글은 포켓몬으로 나갔었다 . 
...
/**
 * 계측이 실제로 신호를 보내는지. 안 붙으면 조용히 0 이 되고, 우리는 아무도 안 온다로 읽는다.
 ...
 */
{
  ...
  // 방문과 끝남 사이가 비면 열고 그냥 나간 사람을 못 센다.
  ...
  // 연습 방문을 오늘 판과 합치면 오늘 몇 명이 열었나가 부풀어 깔때기를 못 믿는다.
  {
  ...
  // 시작점 단추가 실제로 먹히는지도 계측으로만 알 수 있다. 직접과 갈라 센다.
  {
...
/**
 * 표가 오기 전 몇 초. 그동안 입력칸은 먹통인데 아무 말도 안 했다. 느린 회선에서는 고장으로 읽힌다.
 */
{
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
 ...
 *
 * 이 표는 배포할 때 어제까지로 굳어 박힌다. 다음 배포 전에 날이 바뀌면, 박힌 것의
 ...
 */
{
  ...
  // 얹은 줄도 진짜 줄이어야 한다. 답과 풀어보기가 붙어 있는지 맨 윗줄로 확인한다.
 ...
 */
{
  ...
  // 앱이 돌기 전에 세면 안 치웠다로 헛걸린다. 치워질 때까지 기다린다.
  await page.reload({ waitUntil: 'networkidle' });
  await page
...
/**
 * 검색이 이걸 글 한 장이 아니라 무료로 바로 하는 웹 게임으로 읽어야 한다.
 ...
 */
{
  ...
  /**
   * 펼쳐짐은 정말 펼쳐졌을 때만이어야 한다. 여태 참으로 박혀 있어서, 목록이 비어 있어도
 ...
 */
{
  ...
  const finishHere = async () => {
    ...
    const topic = JSON.parse(readFileSync(join(app, 'data', `${topicId}.json`), 'utf8'));
    // 전부대기, 격자판은 맞히면 끝이 아니다. 그만두는 것이 끝내는 방법이다.
  ...
  // 다음 판 링크를 눌러 두 판 더. 오늘 것만 따라간다(어제 판, 전체 보기는 건너뛴다).
  for (let i = 0; i < 2; i += 1) {
...
/**
 * 표를 못 받는 상황을 일부러 만든다. 예전엔 빈 화면이 떴고, 낯선 사람은 그걸 고장으로 읽는다.
...
// 한 판 끝낸 사람이 허브로 돌아오면, 다 푼 판이 표시돼야 한다 (같은 판을 또 누르는 낭비 차단).
{
  ...
  // 한 판 푼 뒤에는 이어서로 바뀌고, 이미 푼 판을 가리키지 않아야 한다.
...
/**
 * 오늘 판을 다 푼 사람에게도 갈 곳이 있어야 한다. 내일 또만 남기면 그대로 나간다.
 ...
 */
{
   ...
   * 허브에 지난 문제는 <여기>가 있었다. 화면 낭독기로 링크만 훑으면 여기, 여기, 여기로
  ...
  check('★ 뜻 없는 링크 글자가 없다', vague.length === 0, vague.join(', ') || '여기류 0개');
  await ctx.close();
}
