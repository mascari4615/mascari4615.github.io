...
for (const topic of topics) {
   ...
   * 자동완성에도 없고 정답으로도 안 나온다. 사람은 낡은 사이트로 읽고 떠나는데
  ...
  for (const item of topic.items) {
    ...
    for (const f of topic.fields) {
      ...
      // 0 은 없음의 다른 이름인 경우가 많다. 롤 신규 챔피언 여섯이 난이도 0 으로 와서
 ...
 *
 * 판 목록의 `short`/`lede` 는 **탭과 페이지 머리**용이라 짧거나 길다. 카드에서는 무슨 놀이인가가
...
function pagesOf(topic) {
   ...
   * 축 둘을 세울 만큼 갈래가 있는 표에만 붙는다(롤은 자원이 마나로 쏠려 못 선다).
   */
...
/**
 * 검색 로봇에게 이건 무료로 바로 하는 웹 게임이라고 말해 준다.
...
${pagesOf(t)
  .map(
    /* 카드 이름은 **판마다 다르다**. 예전엔 실루엣 아니면 전부 속성 맞히기로 박아 둬서,
